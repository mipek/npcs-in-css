...
void CAI_FuncTankBehavior::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		{
			if ( enginerandom->RandomInt( 0, 3 ) == 0 )
			{
		...
		case TASK_GET_PATH_TO_FUNCTANK:
		{
			...
			goal.pTarget = m_hFuncTank.Get()->BaseEntity();
		...
		case TASK_SCAN_LEFT_FUNCTANK:
		{
			...
			float flScanAmount = enginerandom->RandomFloat( 0, flYawRange );
			...
			float flPitch = enginerandom->RandomFloat( -flPitchRange, flPitchRange );
			...
			*/
			...
			GetOuter()->AddLookTarget_V( vecTarget, 1.0, 2.0, 0.2 );
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_FuncTankBehavior::SetFuncTank( CFuncTank *hFuncTank )
{
	...
	m_hFuncTank.Set((hFuncTank) ? hFuncTank->BaseEntity() : NULL);
...
CBaseEntity *CAI_FuncTankBehavior::BestEnemy( void )
{
	...
	{
		CEntity *pEnemy = CEntity::Instance(pEMemory->hEnemy);
		if ( !pEnemy || !pEnemy->IsAlive() )
			continue;

		CBaseEntity *cbase_pEnemy = pEnemy->BaseEntity();

		// UNDONE: Move relationship checks into IsValidEnemy?
		if ( ( pEnemy->GetFlags() & FL_NOTARGET ) ||
			 ( pNPC->IRelationType( cbase_pEnemy ) != D_HT && pNPC->IRelationType( cbase_pEnemy ) != D_FR ) ||
			 !IsValidEnemy( cbase_pEnemy ) )
			continue;
		...
		if ( GetEnemies()->HasEludedMe( cbase_pEnemy ) )
			continue;

		// Establish the reachability of this enemy
		bUnreachable = pNPC->IsUnreachable( cbase_pEnemy );
		...
		if ( bBestUnreachable && !bUnreachable )
		{
			bBestSeen = ( pNPC->GetSenses()->DidSeeEntity( pEnemy ) || pNPC->FVisible_Entity( cbase_pEnemy ) ); // @TODO (toml 04-02-03): Need to optimize CanSeeEntity() so multiple calls in frame do not recalculate, rather cache
			iBestPriority = pNPC->IRelationPriority( cbase_pEnemy );
			iBestDistSq = (int)(pEnemy->GetAbsOrigin() - GetAbsOrigin()).LengthSqr();
			pBestEnemy = cbase_pEnemy;
			bBestUnreachable = bUnreachable;
		}
			// If both are unreachable or both are reachable, chose enemy based on priority and distance
		else if ( pNPC->IRelationPriority( cbase_pEnemy ) > iBestPriority )
		{
		...
		else if ( pNPC->IRelationPriority( cbase_pEnemy ) == iBestPriority )
		{
			...
			if ( bCloser || !bBestSeen )
			{
				...
				bool fSeen = ( pNPC->GetSenses()->DidSeeEntity( pEnemy ) || pNPC->FVisible_Entity( cbase_pEnemy ) );
