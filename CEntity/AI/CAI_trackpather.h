...
class CAI_TrackPather : public CE_Cycler_Fix
{
public:
	CE_DECLARE_CLASS( CAI_TrackPather, CE_Cycler_Fix );
	DECLARE_DATADESC();
public:
...
protected:	
	...
	virtual CEntity *GetTrackPatherTargetEnt()	{ return NULL; }
	...
	CE_CPathTrack *	GetDestPathTarget()						{ return m_pDestPathTarget;		}
...
protected:
	...
	void SetTrack( CEntity *pGoalEnt );
	...
	CE_CPathTrack *NextAlongCurrentPath( CE_CPathTrack *pPath ) const;
	CE_CPathTrack *PreviousAlongCurrentPath( CE_CPathTrack *pPath ) const;

	// Adjusts a "next"most node based on the current movement direction
	CE_CPathTrack *AdjustForMovementDirection( CE_CPathTrack *pPath ) const;

	// Enemy visibility check
	virtual CEntity *FindTrackBlocker( const Vector &vecViewPoint, const Vector &vecTargetPos );
	...
	void ComputePathDirection( CE_CPathTrack *pPath, Vector *pVecPathDir );
	...
	void MoveToTrackPoint( CE_CPathTrack *pTrack );

	// Sets up a new current path target
	void SetupNewCurrentTarget( CE_CPathTrack *pTrack );
	...
	bool HasLOSToTarget( CE_CPathTrack *pTrack );
...
private:
	...
	CE_CPathTrack		*BestPointOnPath( CE_CPathTrack *pPath, const Vector &targetPos, float avoidRadius, bool visible, bool bFarthestPointOnPath );
	...
	void MoveToClosestTrackPoint( CE_CPathTrack *pTrack );

	// Are the two path tracks connected?
	bool IsOnSameTrack( CE_CPathTrack *pPath1, CE_CPathTrack *pPath2 ) const;

	// Is pPathTest in "front" of pPath on the same path? (Namely, does GetNext() get us there?)
	bool IsForwardAlongPath( CE_CPathTrack *pPath, CE_CPathTrack *pPathTest ) const;

	// Purpose: 
	void UpdateTargetPositionLeading( void );

	// Compute a point n units along a path
	CE_CPathTrack *ComputeLeadingPointAlongPath( const Vector &vecStartPoint, CE_CPathTrack *pFirstTrack, float flDistance, Vector *pTarget );

	// Finds the closest point on the path, returns a signed perpendicular distance
	CE_CPathTrack *FindClosestPointOnPath( CE_CPathTrack *pPath, const Vector &targetPos, Vector *pVecClosestPoint, Vector *pVecPathDir, float *pDistanceFromPath );
	...
	float ComputePathDistance( CE_CPathTrack *pStart, CE_CPathTrack *pDest, bool bForward ) const;

	// Compute the distance to a particular point on the path
	float ComputeDistanceAlongPathToPoint( CE_CPathTrack *pStartTrack, CE_CPathTrack *pDestTrack, const Vector &vecDestPosition, bool bMovingForward );
...
private:
	...
	CEFakeHandle<CE_CPathTrack> m_pCurrentPathTarget;
	CEFakeHandle<CE_CPathTrack> m_pDestPathTarget;
	CEFakeHandle<CE_CPathTrack> m_pLastPathTarget;
	CEFakeHandle<CE_CPathTrack> m_pTargetNearestPath;	// Used only by leading, it specifies the path point *after* where the target is
