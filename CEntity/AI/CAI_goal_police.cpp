...
LINK_ENTITY_TO_CUSTOM_CLASS( ai_goal_police, info_target, CAI_PoliceGoal );
...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
CEntity *CAI_PoliceGoal::GetTarget( void )
{
	if ( m_hTarget == NULL )
	{
		CEntity *pTarget = g_helpfunc.FindEntityByName( (CBaseEntity *) NULL, m_iszTarget );
		...
		m_hTarget.Set(pTarget->BaseEntity());
	}
...
// Input  : *pTarget -
//-----------------------------------------------------------------------------
void CAI_PoliceGoal::KnockOutTarget( CEntity *pTarget )
{
