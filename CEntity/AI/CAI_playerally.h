...
class CAI_AllySpeechManager : public CEntity
{
	CE_DECLARE_CLASS( CAI_AllySpeechManager, CEntity );
public:
...
private:
	CSimpleSimTimer *Get_m_ConceptCategoryTimers(int index);

	DECLARE_DATAMAP(CSimpleSimTimer *, m_ConceptCategoryTimers);
	typedef CUtlMap<string_t,CSimpleSimTimer,char> CAI_AllySpeechManager_m_ConceptTimers;
	DECLARE_DATAMAP(CAI_AllySpeechManager_m_ConceptTimers, m_ConceptTimers);
...
{
public:
	CE_DECLARE_CLASS( CAI_PlayerAlly, CAI_BaseActor );
	...
	// Combat
	//---------------------------------
	void		OnKilledNPC( CBaseEntity *pKilled );
	...
	CEntity *FindSpeechTarget( int flags );
	virtual bool IsValidSpeechTarget( int flags, CEntity *pEntity );

	CEntity		*GetSpeechTarget()								{ return m_hTalkTarget.Get(); }
	void		SetSpeechTarget( CBaseEntity *pSpeechTarget ) 	{ m_hTalkTarget.Set(pSpeechTarget); }

	void		SetSpeechFilter( CE_AI_SpeechFilter *pFilter )	{ m_hSpeechFilter.Set((pFilter)?pFilter->BaseEntity():NULL); }
	CE_AI_SpeechFilter *GetSpeechFilter( void )					{ return m_hSpeechFilter; }
	...
	bool		 SelectQuestionFriend( CEntity *pFriend, AISpeechSelection_t *pSelection );
	bool		 SelectAnswerFriend( CEntity *pFriend, AISpeechSelection_t *pSelection, bool bRespondingToHello );

	//---------------------------------

	bool 		SelectSpeechResponse( AIConcept_t concept, const char *pszModifiers, CEntity *pTarget, AISpeechSelection_t *pSelection );
	...
	void		DeferAllIdleSpeech( float flDelay = -1, CAI_NPC *pIgnore = NULL );
...
private:
	...
	CFakeHandle		m_hTalkTarget;	// who to look at while talking
	float			m_flNextRegenTime;
	float			m_flTimePlayerStartStare;
	CFakeHandle		m_hPotentialSpeechTarget;	// NPC to tell the response rules about when trying to find a response to talk to them with
	...
	CEFakeHandle<CE_AI_SpeechFilter>	m_hSpeechFilter;
