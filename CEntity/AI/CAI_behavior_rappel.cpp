...
//=========================================================
//=========================================================
class CE_CRopeAnchor : public CPointEntity
{
public:
	CE_DECLARE_CLASS( CE_CRopeAnchor, CPointEntity );
...
CE_LINK_ENTITY_TO_CLASS( CRopeAnchor, CE_CRopeAnchor )

//---------------------------------------------------------
//---------------------------------------------------------
#define RAPPEL_ROPE_WIDTH 1
void CE_CRopeAnchor::Spawn()
{
	BaseClass::Spawn();
...
//---------------------------------------------------------
//---------------------------------------------------------
void CE_CRopeAnchor::RemoveThink()
{
	UTIL_Remove( m_hRope );	
...
{
	CEntity::PrecacheModel( "cable/cable.vmt" );
}
...
void CAI_RappelBehavior::SetDescentSpeed()
{
	...
	trace_t tr;
	UTIL_TraceLine( GetOuter()->GetAbsOrigin(), GetOuter()->GetAbsOrigin() - Vector( 0, 0, 8192 ), MASK_SHOT, GetOuter()->BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
void CAI_RappelBehavior::StartTask( const Task_t *pTask )
{
	...
	case TASK_HIT_GROUND:
		...
		if( GetOuter()->GetGroundEntity() != NULL && GetOuter()->GetGroundEntity()->IsNPC() && GetOuter()->GetGroundEntity()->m_iClassname == GetOuter()->m_iClassname )
		{
			...
			variant_t val;
			val.SetFloat( 0 );
			g_CEventQueue->AddEvent( GetOuter()->GetGroundEntity()->BaseEntity(), "sethealth", val, 0, GetOuter()->BaseEntity(), GetOuter()->BaseEntity() );
		}
	...
	case TASK_RAPPEL:
		{
			...
			{
				CEntity *pGroundEnt = GetOuter()->GetGroundEntity();
...
void CAI_RappelBehavior::BeginRappel()
{
	...
	UTIL_TraceEntity( GetOuter(), GetAbsOrigin(), GetAbsOrigin()-Vector(0,0,4096), MASK_SHOT, GetOuter()->BaseEntity(), COLLISION_GROUP_NONE, &tr );

	CEntity *pEnt = CEntity::Instance(tr.m_pEnt);
	if( pEnt != NULL && pEnt->IsNPC() )
	{
		Vector forward;
		GetOuter()->GetVectors( &forward, NULL, NULL );

		g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER, pEnt->EarPosition() - forward * 12.0f, 32.0f, 0.2f, GetOuter()->BaseEntity() );
	}
}
...
void CAI_RappelBehavior::CutZipline()
{
	...
	CEntity *pAnchor = CreateEntityByName( "rope_anchor" );
	pAnchor->SetOwnerEntity( GetOuter()->BaseEntity() ); // Boy, this is a hack!!
...
void CAI_RappelBehavior::CreateZipline()
{
	...
	if( !m_hLine )
	{
		...
		if( attachment > 0 )
		{
			CE_CBeam *pBeam = CE_CBeam::BeamCreate( "cable/cable.vmt", 1 );
			if(pBeam)
			{
