...
void CAI_PlaneSolver::AddObstacle( const Vector &center, float radius, CEntity *pEntity, AI_MoveSuggType_t type )
{
	m_Obstacles.AddToTail( CircleObstacles_t( center, radius, (pEntity)?pEntity->BaseEntity():NULL, type ) );
}
...
float CAI_PlaneSolver::CalcProbeDist( float speed )
{
	...
	CAI_NPC *npc = (CAI_NPC*)CEntity::Instance(m_pNpc);
	float result = GetLookaheadTime() * speed;
	if ( result < npc->GetMoveProbe()->GetHullWidth() )
		return npc->GetMoveProbe()->GetHullWidth();
...
bool CAI_PlaneSolver::MoveLimit( Navigation_t navType, const Vector &target, bool ignoreTransients, bool fCheckStep, int contents, AIMoveTrace_t *pMoveTrace )
{
	...
	CAI_NPC *npc = (CAI_NPC*)CEntity::Instance(m_pNpc);
	CAI_MoveProbe *pProbe = npc->GetMoveProbe();
	CEntity *cent = npc->GetNavTargetEntity();
	return pProbe->MoveLimit( navType, GetLocalOrigin(), target, contents,
							  cent, (fCheckStep) ? 100 : 0,
							  flags,
							  pMoveTrace );
}
	...
	if ( m_Solver.HaveRegulations() )
	{
		...
		if ( m_Solver.Solve( moveSuggestions, nSuggestions, &solution ) )
		{
			*pResult = UTIL_YawToVector( solution.dir );
			...
			CAI_NPC *npc = (CAI_NPC*)CEntity::Instance(m_pNpc);
			Vector curVelocity = npc->GetSmoothedVelocity();
												   ...
												   float arcCenter, float arcSpan, int probeOffset )
{
	...
	{
		Vector vecToOther = CEntity::Instance(moveTrace.pObstruction)->GetLocalOrigin() - GetLocalOrigin();
...
bool CAI_PlaneSolver::GenerateCircleObstacleSuggestions( const AILocalMoveGoal_t &moveGoal, float probeDist )
{
	CAI_NPC *npc = (CAI_NPC*)CEntity::Instance(m_pNpc);
	bool result = false;
	Vector npcLoc = npc->WorldSpaceCenter();
	Vector mins, maxs;

	npc->CollisionProp_Actual()->WorldSpaceSurroundingBounds( &mins, &maxs );
	float radiusNpc = (mins.AsVector2D() - maxs.AsVector2D()).Length() * 0.5;

	for ( int i = 0; i < m_Obstacles.Count(); i++ )
	{
		CBaseEntity *pObstacleEntity = NULL;
		CEntity *cent = CEntity::Instance(m_Obstacles[i].hEntity);
		float zDistTooFar;
		if ( cent && cent->CollisionProp_Actual() )
		{
			pObstacleEntity = m_Obstacles[i].hEntity.Get();
			CEntity *cent_pObstacleEntity = CEntity::Instance(pObstacleEntity);

			if( pObstacleEntity == moveGoal.pMoveTarget && (cent_pObstacleEntity->IsNPC() || cent_pObstacleEntity->IsPlayer()) )
			{
			...
			cent_pObstacleEntity->CollisionProp_Actual()->WorldSpaceSurroundingBounds( &mins, &maxs );
		...
		if ( distToObstacleSq > 0.001 && distToObstacleSq < sq( radius + probeDist ) )
		{
			...
			{
				weight = AdjustRegulationWeight( cent, weight );
			}
...
void CAI_PlaneSolver::GenerateObstacleNpcs( const AILocalMoveGoal_t &goal, float probeDist )
{
	if ( !ProbeForNpcs() )
	{
		...
		CAI_NPC *npc = (CAI_NPC*)CEntity::Instance(m_pNpc);
		npc->CollisionProp_Actual()->WorldSpaceSurroundingBounds( &minsSelf, &maxsSelf );
		float radiusSelf = (minsSelf.AsVector2D() - maxsSelf.AsVector2D()).Length() * 0.5;

		for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
		{
			CAI_NPC *pAI = ppAIs[i];
			if ( pAI != npc && pAI->IsAlive() && ( !goal.pPath || pAI != goal.pPath->GetTarget() ) )
			{
				Vector mins, maxs;

				pAI->CollisionProp_Actual()->WorldSpaceSurroundingBounds( &mins, &maxs );
				if ( mins.z < maxsSelf.z + 12.0 && maxs.z > minsSelf.z - 12.0 )
				{
					float radius = (mins.AsVector2D() - maxs.AsVector2D()).Length() * 0.5;
					float distance = ( pAI->GetAbsOrigin().AsVector2D() - npc->GetAbsOrigin().AsVector2D() ).Length();
		...
		CEntity *pPlayer = UTIL_GetNearestPlayer(npc->GetAbsOrigin());
		if ( pPlayer )
		{
			Vector mins, maxs;

			pPlayer->CollisionProp_Actual()->WorldSpaceSurroundingBounds( &mins, &maxs );
			if ( mins.z < maxsSelf.z + 12.0 && maxs.z > minsSelf.z - 12.0 )
			{
				float radius = (mins.AsVector2D() - maxs.AsVector2D()).Length();
				float distance = ( pPlayer->GetAbsOrigin().AsVector2D() - npc->GetAbsOrigin().AsVector2D() ).Length();
				if ( distance - radius < radiusSelf + probeDist )
				{
...
float CAI_PlaneSolver::CalculateRegulationWeight( const AIMoveTrace_t &moveTrace, float pctBlocked )
{
	...
	else
	{
		...
		weight = AdjustRegulationWeight( CEntity::Instance(moveTrace.pObstruction), weight );
	}
...
float CAI_PlaneSolver::AdjustRegulationWeight( CEntity *pEntity, float weight )
{
	if ( pEntity->MyNPCPointer() != NULL )
	{
		...
		Vector2D velBlocker = ((CAI_NPC *)pEntity)->GetMotor()->GetCurVel().AsVector2D();
...
inline CAI_NPC *CAI_PlaneSolver::GetNpc()
{
	return (CAI_NPC *)CEntity::Instance(m_pNpc);
}