...
{
	m_hPoliceGoal.Set((pGoal)?pGoal->BaseEntity():NULL);
	m_bEnabled = true;
...
void CAI_PolicingBehavior::HostSetBatonState( bool state )
{
	...
	if ( pCop != NULL )
	{
		...
		CEntity *cent = m_hPoliceGoal->GetTarget();
		pCop->SetTarget( (cent)?cent->BaseEntity():NULL);
	}
}
...
void CAI_PolicingBehavior::GatherConditions( void )
{
	...
	CEntity *pTarget = m_hPoliceGoal->GetTarget();
...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
CEntity *CAI_PolicingBehavior::GetGoalTarget( void )
{
...
void CAI_PolicingBehavior::StartTask( const Task_t *pTask )
{
	switch (pTask->iTask)
	{
		...
		case TASK_POLICE_ANNOUNCE_HARASS:
		{
			...
			m_flNextHarassTime = gpGlobals->curtime + enginerandom->RandomInt( 4, 6 );

			// Scatter rubber-neckers
			g_helpfunc.CSoundEnt_InsertSound( SOUND_MOVE_AWAY, GetAbsOrigin(), 256, 2.0f, GetOuter()->BaseEntity() );
		}
			TaskComplete();
			break;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_PolicingBehavior::ShouldKnockOutTarget( CEntity *pTarget )
{
	if ( m_hPoliceGoal == NULL )
	{
		//NOTENOTE: This has been called before the behavior is actually active, or the goal has gone invalid
	...
	bool bVisible = GetOuter()->FVisible_Entity( pTarget->BaseEntity() );
...
// Input  : *pTarget -
//-----------------------------------------------------------------------------
void CAI_PolicingBehavior::KnockOutTarget( CEntity *pTarget )
{
	if ( m_hPoliceGoal == NULL )
	{
		//NOTENOTE: This has been called before the behavior is actually active, or the goal has gone invalid
...
int CAI_PolicingBehavior::SelectSuppressSchedule( void )
{
	...
	if ( m_bTargetIsHostile == false )
	{
		...
		// Attack the target
		GetOuter()->SetEnemy( pTarget->BaseEntity() );
		GetOuter()->SetState( NPC_STATE_COMBAT );
		GetOuter()->UpdateEnemyMemory( pTarget->BaseEntity(), pTarget->GetAbsOrigin() );
...
int CAI_PolicingBehavior::SelectHarassSchedule( void )
{
	...
	GetOuter()->AddLookTarget_E( pTarget->BaseEntity(), 0.5f, 5.0f );

	// Say something if it's been long enough
	if ( m_flNextHarassTime < gpGlobals->curtime )
	{
		// Gesture the player away
		GetOuter()->SetTarget( pTarget->BaseEntity() );
