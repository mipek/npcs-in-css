...
public:
	...
	void	Enable( CE_AI_ActBusyGoal *pGoal, float flRange, bool bVisibleOnly );
	void	OnRestore();
	void	SetBusySearchRange( float flRange );
	void	Disable( void );
	void	ForceActBusy( CE_AI_ActBusyGoal *pGoal, CE_AI_Hint *pHintNode = NULL, float flMaxTime = NO_MAX_TIME, bool bVisibleOnly = false, bool bTeleportToBusy = false, bool bUseNearestBusy = false, CEntity *pSeeEntity = NULL, Activity activity = ACT_INVALID );
	void	ForceActBusyLeave( bool bVisibleOnly = false );
	void	StopBusying( void );
	bool	IsStopBusying();
	CE_AI_Hint *FindActBusyHintNode( void );
	CE_AI_Hint *FindCombatActBusyHintNode( void );
	CE_AI_Hint *FindCombatActBusyTeleportHintNode( void );
	...
	bool	FValidateHintType( CE_AI_Hint *pHint );
	...
	void 	CollectSafeZoneVolumes( CE_AI_ActBusyGoal *pActBusyGoal );
	bool	IsInSafeZone( CEntity *pEntity );
...
private:
	...
	CEFakeHandle<CE_AI_ActBusyGoal>		m_hActBusyGoal;

	bool			m_bNeedToSetBounds;
	CFakeHandle		m_hSeeEntity;
...
class CE_AI_ActBusyGoal : public CAI_FollowGoal_Fix
{
public:
	CE_DECLARE_CLASS( CE_AI_ActBusyGoal, CAI_FollowGoal_Fix );
...
public:
	...
	virtual void NPCMovingToBusy( CAI_NPC *pNPC );
	virtual void NPCAbortedMoveTo( CAI_NPC *pNPC );
	virtual void NPCStartedBusy( CAI_NPC *pNPC );
	virtual void NPCStartedLeavingBusy( CAI_NPC *pNPC );
	virtual void NPCFinishedBusy( CAI_NPC *pNPC );
	virtual void NPCLeft( CAI_NPC *pNPC );
	virtual void NPCLostSeeEntity( CAI_NPC *pNPC );
	virtual void NPCSeeEnemy( CAI_NPC *pNPC );
...
protected:
	...
	void		 EnableGoal( CBaseEntity *pAI );
...
//-----------------------------------------------------------------------------
class CE_AI_ActBusyQueueGoal : public CE_AI_ActBusyGoal
{
	DECLARE_CLASS( CE_AI_ActBusyQueueGoal, CE_AI_ActBusyGoal );
public:
	...
	void		 PushNPCBackInQueue( CAI_NPC *pNPC, int iStartingNode );
	void		 RemoveNPCFromQueue( CAI_NPC *pNPC );
	...
	CAI_NPC			*GetNPCOnNode( int iNode );
	CAI_ActBusyBehavior *GetQueueBehaviorForNPC( CAI_NPC *pNPC );

	DECLARE_DATADESC();

private:
	int							m_iCurrentQueueCount;
	CEFakeHandle<CE_AI_Hint>	m_hNodes[ MAX_QUEUE_NODES ];
	bool						m_bPlayerBlockedNodes[ MAX_QUEUE_NODES ];
	CFakeHandle					m_hExitNode;
	CFakeHandle					m_hExitingNPC;
