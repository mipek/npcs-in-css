...
{
public:
	CActBusyAnimData( void ) : CBaseGameSystem( "CActBusyAnimData" )
	{
	}
...
CAI_ActBusyBehavior::CAI_ActBusyBehavior()
{
	...
	m_bLeaving = false;
	m_bVisibleOnly = false;
	m_flForcedMaxTime = 0.0f;
	m_bBusy = false;
	m_bMovingToBusy = false;
	m_bNeedsToPlayExitAnim = false;
	m_flNextBusySearchTime = 0.0f;
	m_flEndBusyAt = 0.0f;
	m_flBusySearchRange = 0.0f;
	m_bInQueue = false;
	m_iCurrentBusyAnim = 0;
	m_hActBusyGoal.Set(NULL);
	m_bNeedToSetBounds = false;
	m_hSeeEntity.Set(NULL);
	m_fTimeLastSawSeeEntity = 0.0f;
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::Enable( CE_AI_ActBusyGoal *pGoal, float flRange, bool bVisibleOnly )
{
	NotifyBusyEnding();

	if ( pGoal )
	{
		m_hActBusyGoal.Set(pGoal->BaseEntity());
	}
	m_bEnabled = true;
	m_bBusy = false;
	m_bMovingToBusy = false;
	...
	CE_AI_ActBusyGoal *temp = m_hActBusyGoal;
	m_bInQueue = dynamic_cast<CE_AI_ActBusyQueueGoal*>(temp) != NULL;
...
void CAI_ActBusyBehavior::ForceActBusy( CE_AI_ActBusyGoal *pGoal, CE_AI_Hint *pHintNode, float flMaxTime, bool bVisibleOnly, bool bTeleportToBusy, bool bUseNearestBusy, CEntity *pSeeEntity, Activity activity )
{
	...
	m_hSeeEntity.Set((pSeeEntity)?pSeeEntity->BaseEntity():NULL);
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::OnFriendDamaged( CBaseEntity *pSquadmate, CBaseEntity *pAttacker )
{
	CCombatCharacter *combat = (CCombatCharacter *)CEntity::Instance(pSquadmate);

	if( IsCombatActBusy() && combat->IsPlayer() && IsInSafeZone( CEntity::Instance(pAttacker) ) )
	{
...
int CAI_ActBusyBehavior::CountEnemiesInSafeZone()
{
	...
	{
		if( GetOuter()->IRelationType(ppAIs[i]->BaseEntity()) < D_LI )
		{
...
void CAI_ActBusyBehavior::GatherConditions( void )
{
	...
	if ( m_hSeeEntity != NULL && m_bBusy )
	{
		CBaseEntity *seeEntity = m_hSeeEntity->BaseEntity();

		if ( (!bCheckFOV||GetOuter()->FInViewCone_Entity(seeEntity)) && GetOuter()->QuerySeeEntity(seeEntity) && (!bCheckLOS||GetOuter()->FVisible_Entity(seeEntity)) )
		{
	...
	if( m_bAutoFireWeapon && enginerandom->RandomInt(0, 5) <= 3 )
	{
		CCombatWeapon *pWeapon = GetOuter()->GetActiveWeapon();

		if( pWeapon )
		{
			pWeapon->Operator_ForceNPCFire( GetOuter()->BaseEntity(), false );
		}
	}
}
...
int	CAI_ActBusyBehavior::SelectScheduleForLeaving( void )
{
	...
	CE_AI_Hint *pNode = CAI_HintManager::FindHintRandom( GetOuter(), GetOuter()->GetAbsOrigin(), hintCriteria );
	if ( pNode )
	{
		SetHintNode( pNode );
		return SCHED_ACTBUSY_LEAVE;
	...
	if ( m_bForceActBusy || m_flNextBusySearchTime < gpGlobals->curtime )
	{
		...
		if ( !pNode )
		{
			if( IsCombatActBusy() )
			{
				...
				if ( pPlayer && m_hActBusyGoal->IsCombatActBusyTeleportAllowed() && m_iNumConsecutivePathFailures >= 2 && !pPlayer->FInViewCone_Entity(GetOuter()->BaseEntity()) )
				{
			...
			if ( HasAnimForActBusy( iBusyAnim, BA_BUSY ) )
			{
				...
				if ( GetHintNode() && GetHintNode()->Lock( GetOuter()->BaseEntity() ) )
				{
					...
					if( m_hActBusyGoal && m_hActBusyGoal->m_iszSeeEntityName != NULL_STRING )
					{
						...
						CEntity *cent = g_helpfunc.FindEntityByName((CBaseEntity *)NULL, m_hActBusyGoal->m_iszSeeEntityName);
						m_hSeeEntity.Set( (cent) ? cent->BaseEntity() : NULL );
					}
...
void CAI_ActBusyBehavior::CollectSafeZoneVolumes( CE_AI_ActBusyGoal *pActBusyGoal )
{
	...
	{
		CEntity *pVolume = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, pActBusyGoal->m_iszSafeZoneVolume );
		...
		while( pVolume != NULL )
		{
			...
			pVolume = g_helpfunc.FindEntityByName( pVolume, pActBusyGoal->m_iszSafeZoneVolume );
		}
	}
		...
		case TASK_ACTBUSY_VERIFY_EXIT:
		{
			if( GetOuter()->IsWaitFinished() )
			{
				...
				else
				{
					...
					CE_CEntityBlocker *pBlocker = CE_CEntityBlocker::Create( GetOuter()->GetAbsOrigin(), NAI_Hull::Mins( HULL_HUMAN ), NAI_Hull::Maxs( HULL_HUMAN ), GetOuter()->BaseEntity(), true );
					g_CEventQueue->AddEvent( pBlocker->BaseEntity(), "Kill", 1.0, GetOuter()->BaseEntity(), GetOuter()->BaseEntity() );
					TaskComplete();
				}
			}
		}
...
void CAI_ActBusyBehavior::NotifyBusyEnding( void )
{
	...
	if ( GetHintNode() )
	{
		...
		if( IsCombatActBusy() )
		{
			...
			GetHintNode()->DisableForSeconds( enginerandom->RandomFloat( 10, 15) );
		}
...
//-----------------------------------------------------------------------------
LINK_ENTITY_TO_CUSTOM_CLASS( ai_goal_actbusy, ai_goal_follow, CE_AI_ActBusyGoal );

BEGIN_DATADESC( CE_AI_ActBusyGoal )
...
// Purpose:
//-----------------------------------------------------------------------------
CAI_ActBusyBehavior *CE_AI_ActBusyGoal::GetBusyBehaviorForNPC( CBaseEntity *pEntity, const char *sInputName )
{
	CAI_NPC *pActor = dynamic_cast<CAI_NPC*>(CEntity::Instance(pEntity));
	if ( !pActor )
	{
...
// Purpose:
//-----------------------------------------------------------------------------
CAI_ActBusyBehavior *CE_AI_ActBusyGoal::GetBusyBehaviorForNPC( const char *pszActorName, CBaseEntity *pActivator, CBaseEntity *pCaller, const char *sInputName )
{
	CEntity *pEntity = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, MAKE_STRING(pszActorName), NULL, pActivator, pCaller );
	if ( !pEntity )
	{
	...
	return GetBusyBehaviorForNPC( pEntity->BaseEntity(), sInputName );
}
...
// Input  : &inputdata -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::EnableGoal( CBaseEntity *pAI )
{
	BaseClass::EnableGoal( pAI );

	// Now use this actor to lookup the Behavior
	CAI_ActBusyBehavior *pBehavior;
	CAI_NPC *npc = (CAI_NPC *)CEntity::Instance(pAI);

	if ( npc->GetBehavior( &pBehavior ) )
	{
...
// Input  : &inputdata -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::InputActivate( inputdata_t &inputdata )
{
	if ( ai_debug_actbusy.GetInt() == 4 )
	{
...
// Input  : &inputdata -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::InputDeactivate( inputdata_t &inputdata )
{
	if ( ai_debug_actbusy.GetInt() == 4 )
	{
...
// Purpose:
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::InputSetBusySearchRange( inputdata_t &inputdata )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::InputForceNPCToActBusy( inputdata_t &inputdata )
{
	char parseString[255];
	Q_strncpy(parseString, inputdata.value.String(), sizeof(parseString));

	CE_AI_Hint *pHintNode = NULL;
	float flMaxTime = NO_MAX_TIME;
	bool bTeleport = false;
	bool bUseNearestBusy = false;
	CEntity *pSeeEntity = NULL;
	...
	if ( pszParam )
	{
		...
		CEntity *pEntity = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, pszParam, NULL, inputdata.pActivator, inputdata.pCaller );
		if ( pEntity )
		{
			pHintNode = dynamic_cast<CE_AI_Hint*>(pEntity);
			if ( !pHintNode )
			{
	...
	while ( pszParam )
	{
		...
		{
			pSeeEntity = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, pszParam+4 );
		}
		else if ( pszParam[0] == '$' )
		{
			...
			activity = (Activity)g_helpfunc.ActivityList_IndexForName( pAnimName );
...
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::InputForceThisNPCToActBusy( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::InputForceThisNPCToLeave( inputdata_t &inputdata )
{
...
// Input  : *pNPC -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::NPCMovingToBusy( CAI_NPC *pNPC )
{
}
...
// Input  : *pNPC -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::NPCStartedBusy( CAI_NPC *pNPC )
{
	CBaseEntity *npc = (pNPC)?pNPC->BaseEntity():NULL;
	m_OnNPCStartedBusy.Set( npc, npc, BaseEntity() );
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::NPCStartedLeavingBusy( CAI_NPC *pNPC )
{
}
...
// Input  : *pNPC -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::NPCAbortedMoveTo( CAI_NPC *pNPC )
{
}
...
// Input  : *pNPC -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::NPCFinishedBusy( CAI_NPC *pNPC )
{
	CBaseEntity *npc = (pNPC)?pNPC->BaseEntity():NULL;
	m_OnNPCFinishedBusy.Set( npc, npc, BaseEntity() );
}
...
// Input  : *pNPC -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::NPCLeft( CAI_NPC *pNPC )
{
	CBaseEntity *npc = (pNPC)?pNPC->BaseEntity():NULL;
	m_OnNPCLeft.Set( npc, npc, BaseEntity() );
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::NPCLostSeeEntity( CAI_NPC *pNPC )
{
	CBaseEntity *npc = (pNPC)?pNPC->BaseEntity():NULL;
	m_OnNPCLostSeeEntity.Set( npc, npc, BaseEntity() );
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CE_AI_ActBusyGoal::NPCSeeEnemy( CAI_NPC *pNPC )
{
	CBaseEntity *npc = (pNPC)?pNPC->BaseEntity():NULL;
	m_OnNPCSeeEnemy.Set( npc, npc, BaseEntity() );
}
...
//-----------------------------------------------------------------------------
LINK_ENTITY_TO_CUSTOM_CLASS( ai_goal_actbusy_queue, ai_goal_follow, CE_AI_ActBusyQueueGoal );


BEGIN_DATADESC( CE_AI_ActBusyQueueGoal )
					// Keys
...
// Purpose:
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::Spawn( void )
{
	BaseClass::Spawn();
...
// Purpose:
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::InputActivate( inputdata_t &inputdata )
{
	if ( !IsActive() )
	{
		// Find all our nodes
		for ( int i = 0; i < MAX_QUEUE_NODES; i++ )
		{
			...
			CEntity *pEntity = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_iszNodes[i] );
			if ( !pEntity )
			{
				Warning( "Unable to find ai_goal_actbusy_queue %s's node %d: %s\n", GetEntityName(), i, STRING(m_iszNodes[i]) );
				UTIL_Remove( this );
				return;
			}

			CE_AI_Hint *cent = dynamic_cast<CE_AI_Hint*>(pEntity);
			if ( !cent )
			{
				m_hNodes[i].Set(NULL);
			...
			m_hNodes[i].Set(cent->BaseEntity());
		...
		// Find the exit node
		CEntity *cent = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_iszExitNode );
		if ( !m_hExitNode )
		{
			m_hExitNode.Set(NULL);
			Warning( "Unable to find ai_goal_actbusy_queue %s's exit node: %s\n", GetEntityName(), STRING(m_iszExitNode) );
			UTIL_Remove( this );
			return;
		}
		m_hExitNode.Set(cent->BaseEntity());

		RecalculateQueueCount();

		SetContextThink( &CE_AI_ActBusyQueueGoal::QueueThink, gpGlobals->curtime + 5, QUEUE_THINK_CONTEXT );
	}
...
// Input  : iCount -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::RecalculateQueueCount( void )
{
	...
	m_OnQueueMoved.Set( m_iCurrentQueueCount, BaseEntity(), BaseEntity());
}
...
void CE_AI_ActBusyQueueGoal::InputPlayerStartedBlocking( inputdata_t &inputdata )
{
	...
	*/
	...
	CAI_NPC *pNPC = GetNPCOnNode(iNode);
...
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::PushNPCBackInQueue( CAI_NPC *pNPC, int iStartingNode )
{
	...
	while ( pNPC )
	{
		...
		for ( int iNext = iStartingNode+1; iNext < MAX_QUEUE_NODES; iNext++ )
		{
			if ( !m_bPlayerBlockedNodes[iNext] )
			{
				...
				CAI_NPC *pTargetNPC = GetNPCOnNode(iNext);
				if ( pTargetNPC )
				{
...
// Input  : &inputdata -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::InputMoveQueueUp( inputdata_t &inputdata )
{
	...
	if ( pNPC )
	{
		...
		CAI_ActBusyBehavior *pBehavior = GetQueueBehaviorForNPC( pNPC );
		pBehavior->Disable();
		m_hExitingNPC.Set(pNPC->BaseEntity());;
	}
}
...
// Input  : *pNPC -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::NPCMovingToBusy( CAI_NPC *pNPC )
{
...
// Input  : *pNPC -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::NPCStartedBusy( CAI_NPC *pNPC )
{
...
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::MoveQueueUp( void )
{
	...
	if ( GetNextThink( QUEUE_MOVEUP_THINK_CONTEXT ) < gpGlobals->curtime )
	{
		...
		SetContextThink( &CE_AI_ActBusyQueueGoal::MoveQueueUpThink, flTime, QUEUE_MOVEUP_THINK_CONTEXT );
	}
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::MoveQueueUpThink( void )
{
	...
	for ( int iEmptyNode = 0; iEmptyNode < (MAX_QUEUE_NODES-1); iEmptyNode++ )
	{
		if ( !NodeIsOccupied(iEmptyNode) && !m_bPlayerBlockedNodes[iEmptyNode] )
		{
			...
			for ( int iNext = iEmptyNode+1; iNext < MAX_QUEUE_NODES; iNext++ )
			{
				...
				CAI_NPC *pNPC = GetNPCOnNode(iNext);
				if ( pNPC )
				{
					CAI_ActBusyBehavior *pBehavior = GetQueueBehaviorForNPC( pNPC );
...
// Input  : *pNPC -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::NPCAbortedMoveTo( CAI_NPC *pNPC )
{
...
// Input  : *pNPC -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::NPCFinishedBusy( CAI_NPC *pNPC )
{
	...
	{
		pNPC->ScheduledMoveToGoalEntity( SCHED_IDLE_WALK, (m_hExitNode)?m_hExitNode->BaseEntity():NULL, ACT_WALK );
		CBaseEntity *npc = (pNPC)?pNPC->BaseEntity():NULL;
		m_OnNPCLeftQueue.Set( npc, npc, BaseEntity() );
		m_hExitingNPC.Set(NULL);
	}
...
// Input  : *pNPC -
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::NPCStartedLeavingBusy( CAI_NPC *pNPC )
{
	...
	{
		CBaseEntity *npc = (pNPC)?pNPC->BaseEntity():NULL;
		m_OnNPCStartedLeavingQueue.Set( npc, npc, BaseEntity() );
	}
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::RemoveNPCFromQueue( CAI_NPC *pNPC )
{
	RecalculateQueueCount();
...
//-----------------------------------------------------------------------------
void CE_AI_ActBusyQueueGoal::QueueThink( void )
{
	if ( !GetNPCOnNode(0) )
	{
		MoveQueueUp();
...
// Purpose:
//-----------------------------------------------------------------------------
inline bool	CE_AI_ActBusyQueueGoal::NodeIsOccupied( int i )
{
...
// Output : CAI_BaseNPC
//-----------------------------------------------------------------------------
CAI_NPC *CE_AI_ActBusyQueueGoal::GetNPCOnNode( int iNode )
{
	if ( !m_hNodes[iNode] )
		return NULL;

	return dynamic_cast<CAI_NPC *>(m_hNodes[iNode]->User());
}
...
//-----------------------------------------------------------------------------
CAI_ActBusyBehavior *CE_AI_ActBusyQueueGoal::GetQueueBehaviorForNPC( CAI_NPC *pNPC )
{
	CAI_ActBusyBehavior *pBehavior;
