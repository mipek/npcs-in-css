...
class CE_AI_BattleLine : public CEntity
{
public:
	CE_DECLARE_CLASS( CE_AI_BattleLine, CEntity );
	
public: //these are 1-to-1 copies from source sdk
	bool Affects( CAI_NPC *pNpc )
	{
		string_t str = m_iszActor;
		const char *pszNamedActor = STRING( str );
...
public:
	...
	DECLARE_DATAMAP(CAI_MoveMonitor, m_SelfMoveMonitor);
	
	DECLARE_DATADESC();
};

DEFINE_PROP(m_iszActor, CE_AI_BattleLine);
DEFINE_PROP(m_fActive, CE_AI_BattleLine);
DEFINE_PROP(m_fStrict, CE_AI_BattleLine);
DEFINE_PROP(m_SelfMoveMonitor, CE_AI_BattleLine);

//-------------------------------------

CE_LINK_ENTITY_TO_CLASS( CAI_BattleLine, CE_AI_BattleLine );

BEGIN_DATADESC( CE_AI_BattleLine )
...
{
	m_params = params;
	m_hStandoffGoal.Set(pGoalEntity ? pGoalEntity->BaseEntity() : NULL);
...
void CAI_StandoffBehavior::PrescheduleThink()
{
	...
	if( DrawBattleLines.GetInt() != 0 )
	{
		...
		while ((pEntity = g_helpfunc.FindEntityByClassname( pEntity, "ai_battle_line" )) != NULL)
		{
			// Visualize the battle line and its normal.
			CE_AI_BattleLine *pLine = dynamic_cast<CE_AI_BattleLine *>(pEntity);
	...
	if ( GetEnemy() )
	{
			     ...
			     gpGlobals->curtime  - pEnemyInfo->timeFirstSeen > m_params.flAbandonTimeLimit * 2 ) ) )
		{
			...
			if ( pSquad )
			{
				...
				{
					CAI_NPC *pNPC = CEntity::Instance(pSquadMate)->MyNPCPointer();
					if ( pNPC && pNPC->IsAlive() && pNPC != GetOuter() )
					{
						CAI_StandoffBehavior *pSquadmateStandoff;
						pNPC->GetBehavior( &pSquadmateStandoff );
...
int CAI_StandoffBehavior::SelectScheduleAttack( void )
{
	if ( GetPosture() == AIP_PEEKING || GetPosture() == AIP_STANDING )
	{
			  ...
			  HasCondition( COND_TOO_FAR_TO_ATTACK ) )
		{
			...
			if ( pWeapon && ( pWeapon->CapabilitiesGet() & bits_CAP_WEAPON_RANGE_ATTACK1 ) )
			{
...
Vector CAI_StandoffBehavior::GetStandoffGoalPosition() 
{
	...
	{
		CE_AI_BattleLine *pBattleLine = NULL;
		for (;;)
		{
			pBattleLine = (CE_AI_BattleLine *)g_helpfunc.FindEntityByClassname( pBattleLine, "ai_battle_line" );
	...
	if ( m_UpdateBattleLinesSemaphore.EnterThink() )
	{
		...
		else if ( PlayerIsLeading() && GetEnemy() )
		{
			if ( m_params.fPlayerIsBattleline )
			{
				...
				if ( pPlayer && GetDirectionOfStandoff( &playerLine.normal ) )
				{
...
bool CAI_StandoffBehavior::IsValidShootPosition( const Vector &vLocation, CAI_Node *pNode, const CBaseEntity *pHint )
{
...
void CAI_MappedActivityBehavior_Temporary::OnChangeActiveWeapon( CCombatWeapon *pOldWeapon, CCombatWeapon *pNewWeapon )
{
	UpdateTranslateActivityMap();
}
...
//-------------------------------------
class CE_AI_StandoffGoal : public CE_AI_GoalEntity
{
public:
	CE_DECLARE_CLASS( CE_AI_StandoffGoal, CE_AI_GoalEntity );
...
CE_LINK_ENTITY_TO_CLASS( CAI_StandoffGoal, CE_AI_StandoffGoal );
