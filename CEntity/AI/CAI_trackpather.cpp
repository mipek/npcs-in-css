...
{
	m_vecDesiredPosition.Init();
	m_vecGoalOrientation.Init();
	m_strCurrentPathName = NULL_STRING;
	m_strDestPathName = NULL_STRING;
	m_strLastPathName = NULL_STRING;
	m_strTargetNearestPathName = NULL_STRING;
	m_vecLastGoalCheckPosition.Init();
	m_flEnemyPathUpdateTime = 0.0f;
	m_bForcedMove = false;
	m_bPatrolling = false;
	m_bPatrolBreakable = false;
	m_bLeading = false;
	m_flTargetDistanceThreshold = 0.0f;
	m_flAvoidDistance = 0.0f;
	
	m_flTargetTolerance = 0.0f;
	m_vecSegmentStartPoint.Init();
	m_vecSegmentStartSplinePoint.Init();
	m_bMovingForward = false;
	m_bChooseFarthestPoint = false;
	m_flFarthestPathDist = 0.0f;
	m_flPathMaxSpeed = 0.0f;
	m_flTargetDistFromPath = 0.0f;
	m_flLeadDistance = 0.0f;
	m_vecTargetPathDir.Init();
	m_vecTargetPathPoint.Init();
...
void CAI_TrackPather::OnRestore( void )
{
	...
	{
		CEntity *cent = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_strCurrentPathName );
		if(cent != NULL)
			m_pCurrentPathTarget.Set(cent->BaseEntity());
	}
	else
	{
		m_pCurrentPathTarget.Set(NULL);
	}

	// Restore destination path
	if ( m_strDestPathName != NULL_STRING )
	{
		CEntity *cent = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_strDestPathName );
		if(cent != NULL)
			m_pDestPathTarget.Set(cent->BaseEntity());
	}
	else
	{
		m_pDestPathTarget.Set(NULL);
	}

	// Restore last path
	if ( m_strLastPathName != NULL_STRING )
	{
		CEntity *cent = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_strLastPathName );
		if(cent != NULL)
			m_pLastPathTarget.Set(cent->BaseEntity());
	}
	else
	{
		m_pLastPathTarget.Set(NULL);
	}

	// Restore target nearest path
	if ( m_strTargetNearestPathName != NULL_STRING )
	{
		CEntity *cent = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_strTargetNearestPathName );
		if(cent != NULL)
			m_pTargetNearestPath.Set(cent->BaseEntity());
	}
	else
	{
		m_pTargetNearestPath.Set(NULL);
	}
}
...
//-----------------------------------------------------------------------------
inline CE_CPathTrack *CAI_TrackPather::NextAlongCurrentPath( CE_CPathTrack *pPath ) const
{
	return CE_CPathTrack::ValidPath( m_bMovingForward ? pPath->GetNext() : pPath->GetPrevious() );
}

inline CE_CPathTrack *CAI_TrackPather::PreviousAlongCurrentPath( CE_CPathTrack *pPath ) const
{
	return CE_CPathTrack::ValidPath( m_bMovingForward ? pPath->GetPrevious() : pPath->GetNext() );
}

inline CE_CPathTrack *CAI_TrackPather::AdjustForMovementDirection( CE_CPathTrack *pPath ) const
{
	if ( !m_bMovingForward && CE_CPathTrack::ValidPath( pPath->GetPrevious( ) ) )
	{
		pPath = CE_CPathTrack::ValidPath( pPath->GetPrevious() );
	}
	return pPath;
}
...
//-----------------------------------------------------------------------------
CEntity *CAI_TrackPather::FindTrackBlocker( const Vector &vecViewPoint, const Vector &vecTargetPos )
{
	trace_t	tr;
	UTIL_TraceHull( vecViewPoint, vecTargetPos, -Vector(4,4,4), Vector(4,4,4), MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
CE_CPathTrack *CAI_TrackPather::BestPointOnPath( CE_CPathTrack *pPath, const Vector &targetPos, float flAvoidRadius, bool visible, bool bFarthestPoint )
{
	...
	CEntity *pTargetEnt = GetTrackPatherTargetEnt();	
	if ( pTargetEnt != NULL )
	{
		CCombatCharacter *pCCTarget = pTargetEnt->MyCombatCharacterPointer();
		if ( pCCTarget != NULL && pCCTarget->IsInAVehicle() )
		{
			pVehicle = CEntity::Instance(pCCTarget->GetVehicleEntity());
		}
	}
	...
	CE_CPathTrack *pNearestPath	= NULL;
	...
	{
		int loopCheck = 0;
		CE_CPathTrack *pTravPath = pPath;
		CE_CPathTrack *pNextPath;

		BEGIN_PATH_TRACK_ITERATION();
		for ( ; CE_CPathTrack::ValidPath( pTravPath ); pTravPath = pNextPath, loopCheck++ )
		{
			...
			if ( visible )
			{
				...
				CEntity *pBlocker = FindTrackBlocker( pTravPath->GetAbsOrigin(), targetPos );
...
//-----------------------------------------------------------------------------
CE_CPathTrack *CAI_TrackPather::ComputeLeadingPointAlongPath( const Vector &vecStartPoint, 
				CE_CPathTrack *pFirstTrack, float flDistance, Vector *pTarget )
{
	bool bMovingForward = (flDistance > 0.0f);
	flDistance = fabs(flDistance);

	CE_CPathTrack *pTravPath = pFirstTrack;
	...
	*pTarget = vecStartPoint;
	...
	for ( ; CE_CPathTrack::ValidPath( pTravPath ); pTravPath = pNextPath )
	{
		...
		if ( flPathDist <= flDistance )
		{
			...
			*pTarget = pTravPath->GetAbsOrigin();
			if ( !CE_CPathTrack::ValidPath(pNextPath) )
...
//-----------------------------------------------------------------------------
float CAI_TrackPather::ComputeDistanceAlongPathToPoint( CE_CPathTrack *pStartTrack, 
	CE_CPathTrack *pDestTrack, const Vector &vecDestPosition, bool bMovingForward )
{													  
	...
	CE_CPathTrack *pTravPath = pStartTrack;
	CE_CPathTrack *pNextPath, *pTestPath;
	BEGIN_PATH_TRACK_ITERATION();
	for ( ; CE_CPathTrack::ValidPath( pTravPath ); pTravPath = pNextPath )
	{
		// Circular loop checking
...
//------------------------------------------------------------------------------
bool CAI_TrackPather::HasLOSToTarget( CE_CPathTrack *pTrack )
{
	CEntity *pTargetEnt = GetTrackPatherTargetEnt();
	if ( !pTargetEnt )
		return true;
	...
	// If it has to be visible, run those checks
	CEntity *pBlocker = FindTrackBlocker( pTrack->GetAbsOrigin(), targetPos );
...
void CAI_TrackPather::UpdateCurrentTarget()
{
	...
	if ( m_pCurrentPathTarget != m_pLastPathTarget )
	{
		...
		m_pCurrentPathTarget->CustomAcceptInput( "InPass", BaseEntity(), BaseEntity(), emptyVariant, 0 );
	...
	if ( m_pCurrentPathTarget != m_pDestPathTarget )
	{
		...
		CE_CPathTrack *cent = NextAlongCurrentPath( m_pCurrentPathTarget );
		if ( cent == NULL)
		{
...
float CAI_TrackPather::ComputeDistanceToTargetPosition()
{
	...
	CE_CPathTrack *pStart = m_pCurrentPathTarget;
...
//-----------------------------------------------------------------------------
void CAI_TrackPather::ComputePathDirection( CE_CPathTrack *pPath, Vector *pVecPathDir )
{
...
void CAI_TrackPather::ComputePointAlongCurrentPath( float flDistance, float flPerpDist, Vector *pTarget )
{
	...
	*pTarget = vecStartPoint;
	...
	if ( flDistance != 0.0f )
	{
		...
		CE_CPathTrack *pTravPath = m_pCurrentPathTarget;
		CE_CPathTrack *pAdjustedDest = AdjustForMovementDirection( m_pDestPathTarget );
		for ( ; CE_CPathTrack::ValidPath( pTravPath ); pTravPath = NextAlongCurrentPath( pTravPath ) )
		{
...
//-----------------------------------------------------------------------------
CE_CPathTrack *CAI_TrackPather::FindClosestPointOnPath( CE_CPathTrack *pPath, 
...
#endif
	{
		CE_CPathTrack *pOldDest = m_pDestPathTarget;
		...
		{
 			BEGIN_PATH_TRACK_ITERATION();
			CE_CPathTrack *pTravPath = m_pDestPathTarget;
 			while( CE_CPathTrack::ValidPath( pTravPath ) )
			{
				// Circular loop checking
				if ( pTravPath->HasBeenVisited() )
				{
		...
		while ( true )
		{
			CE_CPathTrack *pNextTrack = NextAlongCurrentPath( m_pDestPathTarget );
			if ( !pNextTrack || (pNextTrack == pOldDest) || pNextTrack->HasBeenVisited() )
				break;

			pNextTrack->Visit();
			m_pDestPathTarget.Set(pNextTrack->BaseEntity());
		}
	}
	else
	{
...
void CAI_TrackPather::UpdateCurrentTargetLeading()
{
	...
	if ( bRestingAtDest )
	{
		...
		CE_CPathTrack *pSegmentStart = PreviousAlongCurrentPath( m_pCurrentPathTarget ); 
		if ( !pSegmentStart )
		{
...
void CAI_TrackPather::UpdateTargetPositionLeading( void )
{
	...
	CE_CPathTrack *pNextPath = FindClosestPointOnPath( m_pCurrentPathTarget, 
	...
	if ( CE_CPathTrack::ValidPath( pNextPath ) == NULL )
		return;
...
//	NDebugOverlay::Cross3D( pNextPath->GetAbsOrigin(), -Vector(24,24,24), Vector(24,24,24), 255, 255, 0, true, 0.1f );
	...
	m_pTargetNearestPath.Set(pNextPath->BaseEntity());
	...
	CE_CPathTrack *pDest = ComputeLeadingPointAlongPath( vecClosestPoint, pNextPath, m_flLeadDistance, &targetPos );
	...
	if ( pDest != m_pDestPathTarget )
	{
		...
		CE_CPathTrack *pAdjustedDest = AdjustForMovementDirection( pDest );
		if ( m_pCurrentPathTarget != pAdjustedDest )
		{
		...
		if ( bMovingForward != m_bMovingForward )
		{
			...
			CE_CPathTrack *cent = NextAlongCurrentPath( m_pCurrentPathTarget );
			m_pCurrentPathTarget.Set((cent)?cent->BaseEntity():NULL);
		}
		m_pDestPathTarget.Set(pDest->BaseEntity());
	}
...
void CAI_TrackPather::UpdateTargetPosition( void )
{
	...
	// Find the best position to be on our path
	CE_CPathTrack *pDest = BestPointOnPath( m_pCurrentPathTarget, targetPos, m_flAvoidDistance, true, m_bChooseFarthestPoint );
...
float CAI_TrackPather::MaxDistanceFromCurrentPath() const
{
	...
	CE_CPathTrack *pPrevPath = PreviousAlongCurrentPath( m_pCurrentPathTarget );
	if ( !pPrevPath )
	{
...
void CAI_TrackPather::UpdateTrackNavigation( void )
{
	...
	if ( ( CE_CPathTrack::ValidPath( m_pDestPathTarget ) == NULL ) && ( (*m_target) != NULL_STRING ) )
	{
...
//------------------------------------------------------------------------------
void CAI_TrackPather::MoveToTrackPoint( CE_CPathTrack *pTrack )
{
	if ( IsOnSameTrack( pTrack, m_pDestPathTarget ) )
	{
		// The track must be valid
		if ( CE_CPathTrack::ValidPath( pTrack ) == NULL )
			return;

		m_pDestPathTarget.Set(pTrack->BaseEntity());
		m_bMovingForward = IsForwardAlongPath( m_pCurrentPathTarget, pTrack );
		m_bForcedMove = true;
	}
	else
	{
		CE_CPathTrack *pClosestTrack = BestPointOnPath( pTrack, WorldSpaceCenter(), 0.0f, false, false );

		// The track must be valid
		if ( CE_CPathTrack::ValidPath( pClosestTrack ) == NULL )
			return;

		SetupNewCurrentTarget( pClosestTrack );
		m_pDestPathTarget.Set((pTrack)?pTrack->BaseEntity():NULL);
...
//------------------------------------------------------------------------------
void CAI_TrackPather::MoveToClosestTrackPoint( CE_CPathTrack *pTrack )
{
	if ( IsOnSameTrack( pTrack, m_pDestPathTarget ) )
		return;
	...
	SetupNewCurrentTarget( pClosestTrack );
	m_pDestPathTarget.Set(pClosestTrack->BaseEntity());
	m_bMovingForward = true;
...
//-----------------------------------------------------------------------------
bool CAI_TrackPather::IsOnSameTrack( CE_CPathTrack *pPath1, CE_CPathTrack *pPath2 ) const
{
	if ( pPath1 == pPath2 )
		return true;
	...
	{
		BEGIN_PATH_TRACK_ITERATION();
		CE_CPathTrack *pTravPath = pPath1->GetPrevious();
		while( CE_CPathTrack::ValidPath( pTravPath ) && (pTravPath != pPath1) )
		{
			// Circular loop checking
			if ( pTravPath->HasBeenVisited() )
				break;
	...
	{
		BEGIN_PATH_TRACK_ITERATION();
		CE_CPathTrack *pTravPath = pPath1->GetNext();
		while( CE_CPathTrack::ValidPath( pTravPath ) && (pTravPath != pPath1) )
		{
			// Circular loop checking
			if ( pTravPath->HasBeenVisited() )
...
void CAI_TrackPather::Teleported()
{
	...
	CE_CPathTrack *pClosestTrack = BestPointOnPath( GetDestPathTarget(), WorldSpaceCenter(), 0.0f, false, false );
	m_pDestPathTarget.Set(NULL);
...
//-----------------------------------------------------------------------------
float CAI_TrackPather::ComputePathDistance( CE_CPathTrack *pPath, CE_CPathTrack *pDest, bool bForward ) const
{
	float flDist = 0.0f;
	CE_CPathTrack *pLast = pPath;

	BEGIN_PATH_TRACK_ITERATION();
	while ( CE_CPathTrack::ValidPath( pPath ) )
	{
...
//-----------------------------------------------------------------------------
bool CAI_TrackPather::IsForwardAlongPath( CE_CPathTrack *pPath, CE_CPathTrack *pPathTest ) const
{
...
{
	CE_CPathTrack *pNextTrack = NextAlongCurrentPath(m_pCurrentPathTarget);
	if ( !pNextTrack )
	{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CAI_TrackPather::SetTrack( CEntity *pGoalEnt )
{
	// Ignore this input if we're *already* on that path.
	CE_CPathTrack *pTrack = dynamic_cast<CE_CPathTrack *>(pGoalEnt);
	if ( !pTrack )
	{
...
void CAI_TrackPather::SetTrack( string_t strTrackName )
{
	...
	CEntity *pGoalEnt = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, strTrackName );
	if ( pGoalEnt == NULL )
	{
