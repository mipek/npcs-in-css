...
// Purpose: 
//-----------------------------------------------------------------------------
void CAI_Spotlight::Init( CAI_NPC *pOuter, int nFlags, float flConstraintAngle, float flMaxLength )
{
...
void CAI_Spotlight::ComputeEndpoint( const Vector &vecStartPoint, Vector *pEndPoint )
{
	// Create the endpoint
	trace_t tr;
	UTIL_TraceLine( vecStartPoint, vecStartPoint + m_vSpotlightDir * 2 * m_flSpotlightMaxLength, MASK_OPAQUE, GetOuter()->BaseEntity(), COLLISION_GROUP_NONE, &tr );
	*pEndPoint = tr.endpos;
}
...
void CAI_Spotlight::CreateSpotlightEntities( void )
{
	...
	CE_CSpotlightEnd *light = (CE_CSpotlightEnd*)CreateEntityByName( "spotlight_end" );
	m_hSpotlightTarget.Set(light->BaseEntity());

	light->Spawn();
	light->SetAbsOrigin( vecEndPoint );
	light->SetOwnerEntity( GetOuter()->BaseEntity() );
	light->SetRenderColor( 255, 255, 255 );
	light->m_Radius = m_flSpotlightMaxLength;
	...
	CE_CBeam *beam = CE_CBeam::BeamCreate( "sprites/glow_test02.vmt", SPOTLIGHT_WIDTH );
	m_hSpotlight.Set(beam->BaseEntity());

	// Set the temporary spawnflag on the beam so it doesn't save (we'll recreate it on restore)
	beam->AddSpawnFlags( SF_BEAM_TEMPORARY );
	beam->SetColor( 255, 255, 255 ); 
	beam->SetHaloTexture( m_nHaloSprite );
	beam->SetHaloScale( 32 );
	beam->SetEndWidth( m_hSpotlight->GetWidth() );
	beam->SetBeamFlags( (FBEAM_SHADEOUT|FBEAM_NOTILE) );
	beam->SetBrightness( 32 );
	beam->SetNoise( 0 );
	beam->EntsInit( GetOuter(), m_hSpotlightTarget );
	beam->SetStartAttachment( m_nSpotlightAttachment );
}
