...
/*void CAI_LeadBehavior::LeadPlayer( const AI_LeadArgs_t &leadArgs, CAI_LeadBehaviorHandler *pSink )
{
	...
	if ( pOuter )
	{
		CPlayer *pPlayer = UTIL_GetNearestVisiblePlayer(pOuter);
		pOuter->SetSpeechTarget( (pPlayer)?pPlayer->BaseEntity():NULL );
	}
...
void CAI_LeadBehavior::BeginScheduleSelection()
{
	...
	SetTarget( (pPlayer)?pPlayer->BaseEntity():NULL );
...
{
	CEntity *pGoalEnt;
	pGoalEnt = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, args.pszGoal );
	...
	{
		CEntity *pWaitPoint = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, args.pszWaitPoint );
		if ( pWaitPoint )
		{
...
void CAI_LeadBehavior::GatherConditions( void )
{
	...
	if ( HasGoal() )
	{
		...
		CEntity *pFollower = UTIL_GetNearestPlayer(GetAbsOrigin());
		...
		if( pFollower )
		{
			...
			if ( GetOuter()->FVisible_Entity( pFollower->BaseEntity() ) && GetOuter()->GetSenses()->ShouldSeeEntity( pFollower->BaseEntity() ) )
			{
...
int CAI_LeadBehavior::SelectSchedule()
{
	if ( HasGoal() )
	{
		...
		{
			CPlayer *pFollower = UTIL_GetNearestPlayer(GetAbsOrigin());
		...
		if( HasCondition( COND_LEAD_FOLLOWER_LOST ) )
		{
			if( m_args.iRetrievePlayer )
			{
				...
				DevMsg( "Follower lost. Spoke COMING_BACK.\n");
		...
		if( HasCondition( COND_LEAD_FOLLOWER_LAGGING ) )
		{
			DevMsg( "Follower lagging. Spoke CATCHUP.\n");
...
void CAI_LeadBehavior::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		{
			float flAvailableTime = GetOuter()->GetExpresser()->GetSemaphoreAvailableTime( GetOuter()->BaseEntity() );
...
bool CAI_LeadBehavior::Speak( AIConcept_t concept )
{
	...
	if ( bNag )
	{
		...
		{
			DevMsg( "Leader didn't speak due to Nag timer.\n");
			return false;
		}
	}
