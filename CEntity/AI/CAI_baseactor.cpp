	...
	switch ( info->m_pEvent->GetType() )
	{
		...
		case CChoreoEvent::GENERIC:
		{
			if (stricmp( event->GetParameters(), "AI_BLINK") == 0)
			{
				...
				// blink eyes
				Blink();
				// don't blink for duration, or next enginerandom blink time
...
bool CAI_BaseActor::RandomFaceFlex( CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event )
{
	if (info->m_flNext < gpGlobals->curtime)
	{
		...
		if (pSettinghdr == NULL)
		{
			pSettinghdr = ( const flexsettinghdr_t * )FindSceneFile( "enginerandom" );
		}
		if ( pSettinghdr )
		{
...
float CAI_BaseActor::HeadTargetValidity(const Vector &lookTargetPos)
{
	...
	float flInterest = clamp( 3.4142f + 3.4142f * dotPr, 0.0f, 1.0f );
...
void CAI_BaseActor::ClearLookTarget( CBaseEntity *pTarget )
{
	...
	if ( iIndex != m_randomLookQueue.InvalidIndex() )
	{
		...
		// Figure out the new enginerandom look time
...
#else
		// No, for now, give all targets enginerandom priority (as long as they're in front)
...
void CAI_BaseActor::MaintainLookTargets( float flInterval )
{
	...
	// clean up enginerandom look targets
	m_randomLookQueue.Cleanup();

	// clean up synthetic look targets
	m_syntheticLookQueue.Cleanup();

	// if there's real things to look at, turn off the enginerandom targets
	...
	// Time to finish the current enginerandom expression? Or time to pick a new one?
...
void CAI_BaseActor::PlayExpressionForState( NPC_STATE state )
{
	...
	// If we have a enginerandom expression, use that
	...
	{
		// Stop looking for enginerandom expressions for this state
...
//-----------------------------------------------------------------------------
// Purpose: Return a enginerandom expression for the specified state to play over 
