...
extern ConVar *xbox_throttlebias;
extern ConVar *xbox_throttlespoof;
extern ConVar *xbox_autothrottle;
extern ConVar *xbox_steering_deadzone;
...
// Constructor
//-----------------------------------------------------------------------------
CFourWheelVehiclePhysics::CFourWheelVehiclePhysics( CAnimating *pOuter )
{
...
{
	return GetOuter()->LookupPoseParameter( szName );
}

inline float CFourWheelVehiclePhysics::GetPoseParameter( int iParameter )
{
	return GetOuter()->GetPoseParameter( iParameter );
}
...
{
	return GetOuter()->GetAttachment( szName, origin, angles );
}
...
{
	Msg("ParseVehicleScript %s\n", pScriptName);

	// Physics keeps a cache of these to share among spawns of vehicles or flush for debugging
	if (!PhysFindOrAddVehicleScript( pScriptName, &vehicle, NULL ))
	{
		META_CONPRINTF("[Monster] %s: failed on PhysFindOrAddVehicleScript\n", __func__);
	}

	m_debugRadius = vehicle.axles[0].wheels.radius;
	CalcWheelData( vehicle );

	if (!PhysModelParseSolid( solid, GetOuter()->BaseEntity(), GetOuter()->GetModelIndex() ))
	{
		META_CONPRINTF("[Monster] %s: failed on PhysModelParseSolid\n", __func__);
	}
...
void CFourWheelVehiclePhysics::CalcWheelData( vehicleparams_t &vehicle )
{
	...
	{
		VectorITransform( left, GetOuter()->EntityToWorldTransform(), left );
		VectorITransform( right, GetOuter()->EntityToWorldTransform(), right );
...
bool CFourWheelVehiclePhysics::Initialize( const char *pVehicleScript, unsigned int nVehicleType )
{
	...
	{
		GetOuter()->VPhysicsGetObject()->EnableCollisions(false);
	}
	GetOuter()->VPhysicsDestroyObject();
	...
	IPhysicsObject *pBody = GetOuter()->VPhysicsInitNormal( SOLID_VPHYSICS, 0, false, &solid );
...
void CFourWheelVehiclePhysics::PlaceWheelDust( int wheelIndex, bool ignoreSpeed )
{
	...
	vecVel.z = enginerandom->RandomFloat( 0.3f, 1.0f );
	...
	if ( flSize )
	{
		...
		g_helpfunc.DispatchEffect( "WheelDust", data );
	}
}
...
bool CFourWheelVehiclePhysics::Think()
{
	...
	if ( GetOuter()->VPhysicsGetObject() && !GetOuter()->VPhysicsGetObject()->GetShadowController() )
	{
...
{
	// must be a wheel
	if ( pPhysics == GetOuter()->VPhysicsGetObject() )
		return true;
	...
	for ( int i = 0; i < m_wheelCount; i++ )
	{
		if ( pPhysics == m_pWheels[i] )
		{
			...
			VectorITransform( m_wheelPosition[i], GetOuter()->EntityToWorldTransform(), tmp );
...
void CFourWheelVehiclePhysics::UpdateDriverControls( CUserCmd *cmd, float flFrameTime )
{
	...
	//-------------------------------------------------------------------------
	CEntity *pDriver = CEntity::Instance(m_pOuterServerVehicle->GetDriver());
	CPlayer *pPlayerDriver;
	float flBiasThreshold = xbox_throttlebias->GetFloat();

	if( pDriver && pDriver->IsPlayer() )
	{
		pPlayerDriver = dynamic_cast<CPlayer*>(pDriver);
	...
	float flSpeedPercentage = clamp( (float) m_nSpeed / m_flMaxSpeed, 0.0f, 1.0f );
...
int CFourWheelVehiclePhysics::VPhysicsGetObjectList( IPhysicsObject **pList, int listMax )
{
	int count = 0;
	// add the body
	count = AddPhysToList( pList, listMax, count, GetOuter()->VPhysicsGetObject() );
	for ( int i = 0; i < 4; i++ )
	{
