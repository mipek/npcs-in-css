...
void SharedVehicleViewSmoothing(CPlayer *pPlayer,
								...
								float *pFOV )
{
	CEntity *cent = CEntity::Instance(pData->pVehicle);
	CAnimating *anim = (CAnimating *)cent;

	int eyeAttachmentIndex = anim->LookupAttachment( "vehicle_driver_eyes" );
	matrix3x4_t vehicleEyePosToWorld;
	Vector vehicleEyeOrigin;
	QAngle vehicleEyeAngles;
	anim->GetAttachment( eyeAttachmentIndex, vehicleEyeOrigin, vehicleEyeAngles );
	...
	*pAbsAngles = pPlayer->EyeAngles();
	...
	{
		CE_CPropVehicleDriveable *pDriveable = assert_cast<CE_CPropVehicleDriveable*>(cent);
	...
	if ( bRunningAnim && !pData->bWasRunningAnim )
	{
		...
		pData->flEnterExitDuration = anim->SequenceDuration( anim->GetSequence() );

		if(pData->flEnterExitDuration == 0.0f)
			pData->flEnterExitDuration = 1.0f;
		...
		*pAbsAngles = vehicleEyeAngles;
		...
		else
		{
			...
			if ( !bExitAnimOn )
			{
				...
				anim->GetAttachmentLocal( eyeAttachmentIndex, localEyeOrigin, localEyeAngles );
#ifdef CLIENT_DLL
	...
	if ( bRunningAnim || pData->bRunningEnterExit )
	{
		float flSplineFrac = clamp( SimpleSpline( frac ), 0.0f, 1.0f );
