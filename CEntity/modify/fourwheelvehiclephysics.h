...
// Purpose:
//-----------------------------------------------------------------------------
class CFourWheelVehiclePhysics // CE_TODO: we currently have no control over this call (no hooks).
{
public:
	DECLARE_DATADESC();

	CFourWheelVehiclePhysics( CAnimating *pOuter );
	~CFourWheelVehiclePhysics ();

	// Call Precache + Spawn from the containing entity's Precache + Spawn methods
	void Spawn();
	void SetOuter( CAnimating *pOuter, CFourWheelServerVehicle *pServerVehicle );
	...
	CAnimating *GetOuter() { return (CAnimating *)CEntity::Instance(m_pOuter); }
...
inline void CFourWheelVehiclePhysics::SetOuter( CAnimating *pOuter, CFourWheelServerVehicle *pServerVehicle )
{
	m_pOuter.Set((pOuter)?pOuter->BaseEntity():NULL);
