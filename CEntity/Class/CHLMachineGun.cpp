...
//=========================================================
BEGIN_DATADESC_CENTITY( CHLMachineGun )
...
void CHLMachineGun::PrimaryAttack( void )
{
	...
	info.m_vecDirShooting = pPlayer->GetAutoaimVector_Float( AUTOAIM_SCALE_DEFAULT );
	info.m_vecSpread = pPlayer->GetAttackSpread( BaseEntity(), NULL );
	...
	//AddViewKick(); // CE_TODO addViewKick
	
	g_helpfunc.CSoundEnt_InsertSound( SOUND_COMBAT, GetAbsOrigin(), SOUNDENT_VOLUME_MACHINEGUN, 0.2, pPlayer->BaseEntity() );
...
{
	if(CPlayer *pPlayer = ToBasePlayer ( GetOwnerEntity() ) )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHLMachineGun::DoMachineGunKick( CPlayer *pPlayer, float dampEasy, float maxVerticleKickAngle, float fireDurationTime, float slideLimitTime )
{
	...
	if ( enginerandom->RandomInt( -1, 1 ) >= 0 )
		vecScratch.y *= -1;
...
int CHLMachineGun::WeaponSoundRealtime( WeaponSound_t shoot_type )
{
	...
	float dt = clamp( *m_flAnimTime.ptr - *m_flPrevAnimTime.ptr, 0, 0.2 );
...
void CHLMachineGun::CE_MuzzleFlash(float scale, int type)
{
	Vector vecMuzzle;
	QAngle angShootDir;
	if (GetAttachment( "muzzle", vecMuzzle, angShootDir ))
	{
		CBroadcastRecipientFilter filter;
		te->MuzzleFlash(filter, 0.0f, vecMuzzle, angShootDir, scale, type);
	}
}
