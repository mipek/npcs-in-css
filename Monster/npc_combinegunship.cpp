...
// Purpose: 
//-----------------------------------------------------------------------------
class CTargetGunshipCrash : public CE_InfoTarget_Fix
{
public:
	CE_DECLARE_CLASS( CTargetGunshipCrash, CE_InfoTarget_Fix );
...
LINK_ENTITY_TO_CUSTOM_CLASS(info_target_gunshipcrash, info_target, CTargetGunshipCrash);
...
{
public:
	CE_DECLARE_CLASS( CNPC_CombineGunship, CBaseHelicopter );
...
private:
	...
	CEFakeHandle<CSmokeTrail>	m_pSmokeTrail;
	CFakeHandle			m_hGroundAttackTarget;

	CSoundPatch		*m_pAirExhaustSound;
	CSoundPatch		*m_pAirBlastSound;
	CSoundPatch		*m_pCannonSound;

	CEntity		*m_pRotorWashModel;
	...
	CFakeHandle							m_hRagdoll;
	CEFakeHandle<CTargetGunshipCrash>	m_hCrashTarget;
	float							m_flNextGunshipCrashFind;

	//CE_TODO
	//CEFakeHandle<CCitadelEnergyCore>		m_hEnergyCore;
...
private:
	...
	void CreateFireBeam(const Vector &endpos);
...
{
	m_hGroundAttackTarget.Set(NULL);
	m_pSmokeTrail.Set(NULL);
	m_iAmmoType		= -1;
	m_pCrashingController = NULL;
	m_hRagdoll.Set(NULL);
	m_hCrashTarget.Set(NULL);
}
...
Vector CNPC_CombineGunship::GetGroundAttackHitPosition( void )
{
	...
	UTIL_TraceLine( vecShootPos, vecShootPos + Vector( 0, 0, -MAX_TRACE_LENGTH ), MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
void CNPC_CombineGunship::StartGroundAttack( void )
{
	...
	g_helpfunc.CSoundEnt_InsertSound ( SOUND_DANGER, endpos, 1024, 0.5f );
...
void CNPC_CombineGunship::ManageWarningBeam( void )
{
	...
	trace_t	tr;
	CTraceFilterSkipTwoEntities filter( (m_hGroundAttackTarget)?m_hGroundAttackTarget->BaseEntity():NULL, BaseEntity(), COLLISION_GROUP_NONE );
	...
	while ( tr.endpos != m_vecHitPos )
	{
		...
		if ( tr.fraction != 1.0 )
		{
			if ( tr.m_pEnt )
			{
				...
				CTakeDamageInfo	info( BaseEntity(), BaseEntity(), 1.0f, DMG_ENERGYBEAM );

				Vector vTargetDir = cent->BodyTarget( tr.endpos, false ) - tr.endpos;
			...
			if ( tr.startsolid )
			{
				...
				g_pEffects->Sparks( tr2.endpos, (int)(3.0f - (m_flGroundAttackTime-gpGlobals->curtime)), (int)(3.5f - (m_flGroundAttackTime-gpGlobals->curtime)), &tr2.plane.normal );
...
void CNPC_CombineGunship::DoBellyBlastDamage( trace_t &tr, Vector vMins, Vector vMaxs )
{
	...
	for ( int i = 0; i < count; i++ )
	{
		...
		CTakeDamageInfo	info( BaseEntity(), BaseEntity(), damage, DMG_DISSOLVE );
		...
		trace_t	groundTrace;
		UTIL_TraceLine( pEntity->GetAbsOrigin(), pEntity->GetAbsOrigin() - Vector( 0, 0, 256 ), MASK_SOLID, pEntity->BaseEntity(), COLLISION_GROUP_NONE, &groundTrace );
		...
		if ( tr.fraction < 1.0f )
		{
			...
			g_helpfunc.DispatchEffect( "Explosion", data );
		}
...
void CNPC_CombineGunship::DoGroundAttackExplosion( void )
{
	...
	trace_t	tr;
	UTIL_TraceLine( vecSrc, impactPoint, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
void CNPC_CombineGunship::DoCombat( void )
{
	...
	{
		AddEntityRelationship( GetEnemy()->BaseEntity(), D_NU, 5 );
...
void CNPC_CombineGunship::PrescheduleThink( void )
{
	...
	else if( m_lifeState == LIFE_DYING )
	{
		...
		if ( flTimeLeft > 1.5 )
		{
			bCreateExplosion = (enginerandom->RandomInt( 0, 3 ) == 0);
		}
		else
		{
			bCreateExplosion = (enginerandom->RandomInt( 0, 2 ) == 0);
		}
		...
		if ( bCreateExplosion )
		{
			...
			ExplosionCreate( explodePoint, QAngle(0,0,1), BaseEntity(), 100, 128, false );
		}
	...
	SetPoseParameter( m_posePitch, enginerandom->RandomFloat( GUNSHIP_HEAD_MAX_LEFT, GUNSHIP_HEAD_MAX_RIGHT ) );
	SetPoseParameter( m_poseYaw, enginerandom->RandomFloat( GUNSHIP_HEAD_MAX_UP, GUNSHIP_HEAD_MAX_DOWN ) );
#endif
...
void CNPC_CombineGunship::FireCannonRound( void )
{
	...
	{
		// Fire a fake shot
		CEntity::FireBullets( 1, vecMuzzle, vecToEnemy, VECTOR_CONE_5DEGREES, 8192, m_iAmmoType, 1 );

		CEntity *pMissile = GetEnemy();
		...
		if ( ( DotProduct( threatDir, missileDir ) > 0.95f ) && ( threatDist < 1024.0f ) )
		{
			...
			{
				CTakeDamageInfo info( BaseEntity(), BaseEntity(), 200, DMG_MISSILEDEFENSE );
	...
	//CTraceFilterSkipTwoEntities traceFilter( BaseEntity(), m_hFireSprite?m_hFireSprite->BaseEntity():nullptr, COLLISION_GROUP_NONE );
	CE_CTraceFilterSimple traceFilter( BaseEntity(), COLLISION_GROUP_NONE );
	trace_t tr;
	UTIL_TraceLine(vecMuzzle, vecMuzzle + vecAimDir * MAX_TRACE_LENGTH, MASK_SHOT, &traceFilter, &tr);

	CBroadcastRecipientFilter filter;
	te->BeamPoints(filter, 0, &vecMuzzle, &tr.endpos, g_iGunshipEffectIndex,
				   0, 0, 0, 0.1f, 2.0f, 1.0f, 0, 0.1f,
				   100, 120, 230+enginerandom->RandomInt( 0, 25 ), 90, 10);
}
...
void CNPC_CombineGunship::DoMuzzleFlash( void )
{
	...
	g_helpfunc.DispatchEffect( "GunshipMuzzleFlash", data );
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_CombineGunship::FVisible_Entity( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	bool fReturn = BaseClass::FVisible_Entity( pEntity, traceMask, ppBlocker );
...
void CNPC_CombineGunship::Event_Killed( const CTakeDamageInfo &info )
{
	...
	m_pRotorSound = g_SoundController->SoundCreate( filter2, entindex(), "NPC_CombineGunship.DyingSound" );
	if(m_pRotorSound) {
...
bool CNPC_CombineGunship::FindNearestGunshipCrash( void )
{
	...
	while( (pEnt = g_helpfunc.FindEntityByClassname(pEnt, "info_target_gunshipcrash")) != NULL )
	{
	...
	m_hCrashTarget.Set(pNearest->BaseEntity());
	m_flNextGunshipCrashFind = gpGlobals->curtime + 0.5;
	m_flEndDestructTime = 0;
...
void CNPC_CombineGunship::BeginDestruct( void )
{
	...
	// Create the ragdoll
	CEntity *cent = g_helpfunc.CreateServerRagdoll( BaseEntity(), 0, info, COLLISION_GROUP_NONE );
	if ( !cent )
	{
		m_hRagdoll.Set(NULL);
		// Failed, just explode
	...
	m_hRagdoll.Set(cent->BaseEntity());
	m_hRagdoll->SetName( UTIL_VarArgs("%s_ragdoll", GetEntityName()) );
...
void CNPC_CombineGunship::CreateSmokeTrail( void )
{
	...
	if ( smoke )
	{
		m_pSmokeTrail.Set(smoke->BaseEntity());

		smoke->m_SpawnRate			= 48;
		smoke->m_ParticleLifetime	= 2.5f;

		smoke->m_StartColor->Init( 0.25f, 0.25f, 0.25f );
		smoke->m_EndColor->Init( 0.0, 0.0, 0.0 );

		smoke->m_StartSize		= 24;
		smoke->m_EndSize		= 128;
		smoke->m_SpawnRadius	= 4;
		smoke->m_MinSpeed		= 8;
		smoke->m_MaxSpeed		= 64;
		smoke->m_Opacity		= 0.2f;
...
void CNPC_CombineGunship::InitializeRotorSound( void )
{
	...
	m_pCannonSound		= g_SoundController->SoundCreate( filter, entindex(), "NPC_CombineGunship.CannonSound" );
	m_pRotorSound		= g_SoundController->SoundCreate( filter, entindex(), "NPC_CombineGunship.RotorSound" );
	m_pAirExhaustSound	= g_SoundController->SoundCreate( filter, entindex(), "NPC_CombineGunship.ExhaustSound" );
	m_pAirBlastSound	= g_SoundController->SoundCreate( filter, entindex(), "NPC_CombineGunship.RotorBlastSound" );

	META_CONPRINTF("InitializeRotorSound %d %f %p %p %p %p\n", entindex(), gpGlobals->curtime, m_pCannonSound, m_pRotorSound, m_pAirExhaustSound, m_pAirBlastSound);
	...
	if(m_pAirExhaustSound) {
		g_SoundController->Play( m_pAirExhaustSound, 0.0, 100 );
	}

	if(m_pAirBlastSound) {
		g_SoundController->Play( m_pAirBlastSound, 0.0, 100 );
	}
...
void CNPC_CombineGunship::UpdateRotorSoundPitch( int iPitch )
{
	...
	if(m_pAirExhaustSound)
	{
		g_SoundController->SoundChangePitch( m_pAirExhaustSound, iPitch, 0.1 );
	}
	...
	if (pPlayer)
	{
		...
		float flVolume = clamp( RemapVal( flDistance, (900*900), (1800*1800), 1, 0 ), 0.0f, 1.0f );
		if(m_pAirExhaustSound)
		{
			g_SoundController->SoundChangeVolume( m_pAirExhaustSound, flVolume * GetRotorVolume(), 0.1 );
		}
		// Fade in the blast when it's close to the player (in 2D)
		flVolume = clamp( RemapVal( flDistance, (600*600), (700*700), 1, 0 ), 0.0f, 1.0f );
		if(m_pAirBlastSound)
		{
			g_SoundController->SoundChangeVolume( m_pAirBlastSound, flVolume * GetRotorVolume(), 0.1 );
		}
	}
...
void CNPC_CombineGunship::SelfDestruct( void )
{
	...
	for( int i = 0 ; i < 6 ; i++ )
	{
		...
		ExplosionCreate( vecOrigin + vecDelta, QAngle( -90, 0, 0 ), BaseEntity(), 10, 10, false );
	}
	...
	if ( !m_hCrashTarget )
	{
		...
		PropBreakableCreateAll( pBreakEnt->GetModelIndex(), pBreakEnt->VPhysicsGetObject(), pBreakEnt->GetAbsOrigin(), pBreakEnt->GetAbsAngles(), pBreakEnt->GetAbsVelocity(), angVelocity, 1.0, 800, COLLISION_GROUP_NPC, pBreakEnt->BaseEntity() );
...
void CNPC_CombineGunship::InputSetDockingBBox( inputdata_t &inputdata )
{
	...
	UTIL_SetSize( BaseEntity(), vecSize * -1, vecSize );
}
...
#define GUNSHIP_TRIM_BOX 38
	...
	UTIL_SetSize( BaseEntity(), vecBBMin, vecBBMax );
}
...
void CNPC_CombineGunship::InputDoGroundAttack( inputdata_t &inputdata )
{
	...
	CEntity *pEntity = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, inputdata.value.StringID(), NULL, inputdata.pActivator, inputdata.pCaller );
	if ( pEntity )
	{
		// Mapmaker wants us to ground attack a specific target
		m_hGroundAttackTarget.Set(pEntity->BaseEntity());
	}
	else
	{
		StartGroundAttack();
...
// Output : int
//-----------------------------------------------------------------------------
void CNPC_CombineGunship::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
	// Reflect bullets
	CTakeDamageInfo newinfo = info;
	if ( info.GetDamageType() & DMG_BULLET )
	{
		newinfo.SetDamage(newinfo.GetDamage()/10.0f);

		if ( enginerandom->RandomInt( 0, 2 ) == 0 )
		{
			Vector vecRicochetDir = vecDir * -1;

			vecRicochetDir.x += enginerandom->RandomFloat( -0.5, 0.5 );
			vecRicochetDir.y += enginerandom->RandomFloat( -0.5, 0.5 );
			vecRicochetDir.z += enginerandom->RandomFloat( -0.5, 0.5 );
		...
		if ( CEntity::Instance(info.GetAttacker())->IsPlayer() )
		{
			// CE_TODO: citizen
			/*if ( GetCitizenResponse() )
			{
	...
	BaseClass::TraceAttack( newinfo, vecDir, ptr, dump );
}
...
int	CNPC_CombineGunship::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if ( m_bInvulnerable == false )
	{
		...
		{
			iHealthIncrements = (int)ceil( iHealthIncrements * 0.5 );
		}
		else if ( g_helpfunc.GameRules_IsSkillLevel( SKILL_HARD ) )
		{
			iHealthIncrements = (int)floor( iHealthIncrements * 1.5 );
		}
...
void CNPC_CombineGunship::StartCannonBurst( int iBurstSize )
{
	...
	g_SoundController->SoundChangeVolume( m_pCannonSound, 1.0, 0 );
	...
	if ( GetEnemy() )
	{
		...
		int angleOffset = enginerandom->RandomInt( 15, 30 );
		...
		g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER | SOUND_CONTEXT_REACT_TO_SOURCE, enemyPos, 512, 0.2f, BaseEntity() );
	}
}
...
void CNPC_CombineGunship::StopCannonBurst( void )
{
	...
	flPerc = 1.0f - ((float)m_iBurstSize / sk_gunship_burst_size.GetFloat());
	flDelay *= flPerc;
	...
	if ( m_pCannonSound )
	{
		g_SoundController->SoundChangeVolume( m_pCannonSound, 0.0, 0.05 );
	}
...
{
	META_CONPRINTF("StopLoopingSounds %d %f %p %p %p %p\n", entindex(), gpGlobals->curtime, m_pCannonSound, m_pRotorSound, m_pAirExhaustSound, m_pAirBlastSound);
	...
	if ( m_pAirExhaustSound )
	{
		g_SoundController->SoundDestroy( m_pAirExhaustSound );
		m_pAirExhaustSound = NULL;
	}

	if ( m_pAirBlastSound )
	{
		g_SoundController->SoundDestroy( m_pAirBlastSound );
...
bool CNPC_CombineGunship::IsValidEnemy( CBaseEntity *pEnemy )
{
	...
	if ( cent->IsAlive() && !cent->MyNPCPointer() && FClassnameIs( cent, "rpg_missile" ) )
		return true;

	// If we're shooting off a burst, don't pick up a new enemy
	if ( ( m_bIsFiring ) && ( ( GetEnemy() == NULL ) || ( GetEnemy() != cent ) ) )
		return false;
...
bool CNPC_CombineGunship::IsTargettingMissile( void )
{
	...
	if (!FClassnameIs(GetEnemy(), "rpg_missile"))
		return false;
...
void CNPC_CombineGunship::UpdateOnRemove()
{
	RemoveFireSprite();
...
void CNPC_CombineGunship::CreateFireSprite()
{
	Vector startpos;
	GetAttachment( "BellyGun", startpos, NULL, NULL, NULL );

	CE_CSprite *sprite = CE_CSprite::SpriteCreate( "effects/blueblackflash.vmt", startpos, false );
	if(sprite)
	{
		m_hFireSprite.Set(sprite->BaseEntity());
		int iAttachment = LookupAttachment( "BellyGun" );
		sprite->SetTransparency( kRenderGlow, 255, 255, 255, 0, kRenderFxNoDissipation );
		sprite->SetAttachment( BaseEntity(), iAttachment );
		sprite->SetBrightness( 255 );
		sprite->SetScale(1.0f);
	}
}

void CNPC_CombineGunship::RemoveFireSprite()
{
	if(m_hFireSprite)
	{
		UTIL_Remove(m_hFireSprite);
		m_hFireSprite.Set(NULL);
	}
}

void CNPC_CombineGunship::CreateFireBeam(const Vector &endpos)
{
	...
	if(beam)
	{
		...
		beam->PointsInit(startpos, endpos);
		beam->SetStartEntity(this);
		beam->SetStartAttachment( iAttachment );
		beam->RelinkBeam();
