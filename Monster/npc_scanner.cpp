...
void CNPC_CScanner::Spawn(void)
{
	...
	m_hSpotlight.Set(NULL);
	m_hSpotlightTarget.Set(NULL);
...
void CNPC_CScanner::Activate()
{
	...
	m_pEyeFlash = CE_CSprite::SpriteCreate( "sprites/blueflare1.vmt", GetLocalOrigin(), FALSE );
	m_pEyeFlash->SetTransparency( kRenderGlow, 255, 255, 255, 0, kRenderFxNoDissipation );
	m_pEyeFlash->SetAttachment( BaseEntity(), LookupAttachment( SCANNER_ATTACHMENT_LIGHT ) );
...
void CNPC_CScanner::Gib( void )
{
...
#if defined(CE_SPAWN_SCANNERGIBS)
	// Spawn all gibs
	if( m_bIsClawScanner )
	{
	...
	if ( !HasSpawnFlags(SF_NPC_NO_WEAPON_DROP) && enginerandom->RandomFloat( 0.0f, 1.0f) < 0.3f )
	{
		CItem<CSode_Fix> *pBattery = (CItem<CSode_Fix>*)CreateEntityByName("item_battery");
		if ( pBattery )
		{
...
void CNPC_CScanner::Precache(void)
{
	...
	if( m_bIsClawScanner )
	{
		...
		//g_helpfunc.UTIL_PrecacheOther( "combine_mine" );
	}
	else
	{
...
void CNPC_CScanner::RequestInspectSupport(void)
{
	...
	if (pSquad)
	{
		...
		for (CBaseEntity *pSquadMember = pSquad->GetFirstMember( &iter ); pSquadMember; pSquadMember = pSquad->GetNextMember( &iter ) )
		{
			...
			if (cent_pSquadMember != this)
			{
				CAI_NPC *pNPC = dynamic_cast <CAI_NPC*>(cent_pSquadMember);
				if(pNPC)
				{
					if (GetTarget())
					{
						pNPC->DispatchInteraction(g_interactionScannerSupportEntity,((void *)((CBaseEntity*)GetTarget())),BaseEntity());
					}
					else
					{
						pNPC->DispatchInteraction(g_interactionScannerSupportPosition,((void *)m_vInspectPos.Base()),BaseEntity());
					}
				}
			}
		}
...
// Purpose:
//------------------------------------------------------------------------------
bool CNPC_CScanner::IsValidInspectTarget(CEntity *pEntity)
{
	// CE_TODO
#if 0
	...
	if (pSquad)
	{
		...
		for (CBaseEntity *pSquadMember = pSquad->GetFirstMember( &iter ); pSquadMember; pSquadMember = pSquad->GetNextMember( &iter ) )
		{
			CAI_NPC *pNPC = dynamic_cast <CAI_NPC*>(CEntity::Instance(pSquadMember));
			if(pNPC)
			{
				if (pNPC->GetTarget() == pEntity)
				{
					return false;
				}
			}
...
// Purpose:
//------------------------------------------------------------------------------
CEntity* CNPC_CScanner::BestInspectTarget(void)
{
	if ( !m_bShouldInspect )
		return NULL;
	...
	{
		vSearchOrigin	= m_hSpotlightTarget.Get()->GetAbsOrigin();
	...
	for ( i = 0; i < candidates.Count(); i++ )
	{
		...
		if ( ( pNPC && pNPC->Classify() == CLASS_CITIZEN_PASSIVE ) || pEntity->IsPlayer() )
		{
			...
			if ( !FVisible_Entity( pEntity->BaseEntity() ) )
				continue;
...
//------------------------------------------------------------------------------
void CNPC_CScanner::SetInspectTargetToEnt(CEntity *pEntity, float fInspectDuration)
{
	ClearInspectTarget();
	SetTarget(pEntity->BaseEntity());
...
//------------------------------------------------------------------------------
void CNPC_CScanner::SetInspectTargetToHint(CE_AI_Hint *pHint, float fInspectDuration)
{
	ClearInspectTarget();
	...
	trace_t tr;
	UTIL_TraceLine ( vHintOrigin, vHintEnd, MASK_BLOCKLOS, BaseEntity(), COLLISION_GROUP_NONE, &tr);
...
void CNPC_CScanner::DeployMine()
{
	...
	for ( child = FirstMoveChild(); child != NULL; child = child->NextMovePeer() )
	{
		if( FClassnameIs( child, "combine_mine" ) )
		{
			...
			child->SetOwnerEntity( this->BaseEntity() );
...
void CNPC_CScanner::InputEquipMine(inputdata_t &inputdata)
{
	...
	if( m_bIsClawScanner )
	{
		...
		if( attachment > -1 )
		{
			...
			pEnt->SetOwnerEntity( this->BaseEntity() );
			pEnt->SetParent( this->BaseEntity(), attachment );
	...
	if( !bPlacedMine )
	{
		...
		pEnt->SetOwnerEntity( this->BaseEntity() );
		pEnt->SetParent( this->BaseEntity() );
	}
...
{
	CEntity *pEnt = g_helpfunc.FindEntityGeneric( NULL, inputdata.value.String(), BaseEntity(), inputdata.pActivator );
...
void CNPC_CScanner::GatherConditions( void )
{
		...
		if ( pSound )
		{
			...
			if ( pSound->m_hOwner != NULL )
			{
				...
				if ( IRelationType( pOwner ) != D_LI )
				{
					...
					if ( FVisible_Entity( pOwner ) )
					{
						SetInspectTargetToEnt( CEntity::Instance(pOwner), SCANNER_SOUND_INSPECT_LENGTH );
					}
				}
			}
			else
			...
			m_nFlyMode = (enginerandom->RandomInt(0,2)==0) ? SCANNER_FLY_SPOT : SCANNER_FLY_PHOTO;
		}
	}
	...
	{
		CEntity *pBestEntity = BestInspectTarget();
		
		if ( pBestEntity != NULL )
		{
			SetInspectTargetToEnt( pBestEntity, SCANNER_CIT_INSPECT_LENGTH );
			m_nFlyMode = (enginerandom->RandomInt(0,3)==0) ? SCANNER_FLY_SPOT : SCANNER_FLY_PHOTO;
			SetCondition ( COND_CSCANNER_HAVE_INSPECT_TARGET );
		}
	}
	...
	if ( m_nFlyMode == SCANNER_FLY_PHOTO )
	{
		...
		if ( gpGlobals->curtime > m_fNextPhotographTime && HaveInspectTarget() && GetCurrentVelocity().LengthSqr() < (64*64) )
		{
			...
			if ( fInspectDist > SCANNER_PHOTO_NEAR_DIST && fInspectDist < SCANNER_PHOTO_FAR_DIST )
			{
				...
				{
					trace_t tr;
					IHandleEntity *pIgnore = GetTarget() ? GetTarget()->BaseEntity() : NULL;
					UTIL_TraceLine ( GetAbsOrigin(), InspectTargetPosition(), MASK_BLOCKLOS, pIgnore, COLLISION_GROUP_NONE, &tr);
...
//-----------------------------------------------------------------------------
Disposition_t CNPC_CScanner::IRelationType(CBaseEntity *pBaseTarget)
{
...
int CNPC_CScanner::SelectSchedule(void)
{
	...
	if ( HaveInspectTarget() )
	{
		...
		{
			m_nFlyMode = (enginerandom->RandomInt(0,1)==0) ? SCANNER_FLY_SPOT : SCANNER_FLY_PHOTO;
		}
...
void CNPC_CScanner::SpotlightCreate(void)
{
	...
	trace_t tr;
	UTIL_TraceLine ( GetAbsOrigin(), GetAbsOrigin() + m_vSpotlightDir * 2024, MASK_OPAQUE, BaseEntity(), COLLISION_GROUP_NONE, &tr );

	CE_CSpotlightEnd *pSpotlightTarget = (CE_CSpotlightEnd*)CreateEntityByName( "spotlight_end" );
	pSpotlightTarget->Spawn();
	pSpotlightTarget->SetLocalOrigin( tr.endpos );
	pSpotlightTarget->SetOwnerEntity( BaseEntity() );
	// YWB:  Because the scanner only moves the target during think, make sure we interpolate over 0.1 sec instead of every tick!!!
	pSpotlightTarget->SetSimulatedEveryTick( false );

	// Using the same color as the beam...
	pSpotlightTarget->SetRenderColor( 255, 255, 255 );
	pSpotlightTarget->m_Radius = m_flSpotlightMaxLength;

	CE_CBeam *pSpotlight = CE_CBeam::BeamCreate( "sprites/glow_test02.vmt", SPOTLIGHT_WIDTH );
	// Set the temporary spawnflag on the beam so it doesn't save (we'll recreate it on restore)
	pSpotlight->AddSpawnFlags( SF_BEAM_TEMPORARY );
	pSpotlight->SetColor( 255, 255, 255 ); 
	pSpotlight->SetHaloTexture( m_nHaloSprite );
	pSpotlight->SetHaloScale( 32 );
	pSpotlight->SetEndWidth( pSpotlight->GetWidth() );
	pSpotlight->SetBeamFlags( (FBEAM_SHADEOUT|FBEAM_NOTILE) );
	pSpotlight->SetBrightness( 32 );
	pSpotlight->SetNoise( 0 );
	pSpotlight->EntsInit( this, pSpotlightTarget );
	/////m_hSpotlight->SetHDRColorScale( 0.75f );	// Scale this back a bit on HDR maps
	// attach to light
	pSpotlight->SetStartAttachment( LookupAttachment( SCANNER_ATTACHMENT_LIGHT ) );
	
	m_hSpotlightTarget.Set(pSpotlightTarget->BaseEntity());
	m_hSpotlight.Set(pSpotlight->BaseEntity());
...
Vector CNPC_CScanner::SpotlightCurrentPos(void)
{
	...
	UTIL_TraceLine ( GetAbsOrigin(), vTraceEnd, MASK_OPAQUE, BaseEntity(), COLLISION_GROUP_NONE, &tr);
...
void CNPC_CScanner::TakePhoto(void)
{
	...
	CEntity *pTarget = GetTarget();
	if ( pTarget )
	{
		if ( pTarget->IsPlayer() )
		{
			m_OnPhotographPlayer.FireOutput( GetTarget(), this );
			BlindFlashTarget( pTarget );
		}
		
		if ( pTarget->MyNPCPointer() != NULL )
		{
			m_OnPhotographNPC.FireOutput( GetTarget(), this );
			GetTarget()->MyNPCPointer()->DispatchInteraction( g_interactionScannerInspectBegin, NULL, BaseEntity() );
		}
	}
...
// Input  : *pTarget - 
//-----------------------------------------------------------------------------
void CNPC_CScanner::BlindFlashTarget( CEntity *pTarget )
{
	...
	if( IsStriderScout() )
	{
		for ( int i = 0; i < nAIs; i++ )
		{
			...
			{
				ppAIs[ i ]->UpdateEnemyMemory( pTarget->BaseEntity(), pTarget->GetAbsOrigin(), BaseEntity() );
			}
		}
	}
	...
	if ( dotPr > 0.5f )
	{
		...
		trace_t tr;
		UTIL_TraceLine ( GetAbsOrigin(), pTarget->EyePosition(), MASK_OPAQUE, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
void CNPC_CScanner::AttackFlashBlind(void)
{
	...
	float fAttackDelay = enginerandom->RandomFloat(SCANNER_ATTACK_MIN_DELAY,SCANNER_ATTACK_MAX_DELAY);
...
void CNPC_CScanner::StartTask( const Task_t *pTask )
{
	...
	case TASK_CSCANNER_SPOT_INSPECT_ON:
	{
		...
		else
		{
			...
			{
				pNPC->DispatchInteraction(g_interactionScannerInspectBegin,NULL,BaseEntity());
	...
	case TASK_CSCANNER_SPOT_INSPECT_WAIT:
	{
		...
		else
		{
			...
			else
			{
				...
				pNPC->DispatchInteraction(g_interactionScannerInspectHandsUp,NULL,BaseEntity());
			}
			TaskComplete();
		}
		break;
	...
	case TASK_CSCANNER_SPOT_INSPECT_OFF:
	{
		...
		else
		{
			...
			{
				pNPC->DispatchInteraction(g_interactionScannerInspectDone,NULL,BaseEntity());
	...
	case TASK_CSCANNER_ATTACK_PRE_FLASH:
	{
		if( IsStriderScout() )
		{
			...
			g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER, vecScare, 256, 1.0, BaseEntity() );
		}
	...
	case TASK_GET_PATH_TO_TARGET:
	{
		...
		else 
		{
			...
			goal.pTarget = GetTarget() ? GetTarget()->BaseEntity() : NULL;
...
bool CNPC_CScanner::OverrideMove( float flInterval )
{
	...
	else
	{
		...
		{
			trace_t tr;
			UTIL_TraceHull( GetAbsOrigin(), vMoveTargetPos, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID_BRUSHONLY, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
bool CNPC_CScanner::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* pSourceEnt)
{
	...
	if (interactionType ==	g_interactionScannerSupportEntity)
	{
		...
		if (GetEnemy() == NULL && !HaveInspectTarget())
		{
			...
			CEntity *pTarget = CEntity::Instance((CBaseEntity*)data);
...
{
public:
	CE_DECLARE_CLASS( CNPC_ClawScanner, CNPC_CScanner );
