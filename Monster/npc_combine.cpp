...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC_CENTITY( CNPC_Combine )
...
void CNPC_Combine::InputThrowGrenadeAtTarget( inputdata_t &inputdata )
{
	...
	CFakeHandle *pCine = m_hCine;
	if ( m_NPCState == NPC_STATE_SCRIPT && pCine )
		return;
	...
	m_hForcedGrenadeTarget.Set(pEntity->BaseEntity());
...
void CNPC_Combine::DelaySquadAltFireAttack( float flDelay )
{
	...
	CAI_Squad *pSquad = m_pSquad;
	CBaseEntity *pSquadmate = pSquad ? pSquad->GetFirstMember( &iter ) : NULL;
	while ( pSquadmate )
	{
		CNPC_Combine *pCombine = dynamic_cast<CNPC_Combine*>(CEntity::Instance(pSquadmate));
...
bool CNPC_Combine::ShouldMoveAndShoot()
{
	...
	CAI_Squad *pSquad = m_pSquad;
	if( pSquad && IsCurSchedule( SCHED_COMBINE_TAKE_COVER1, false ) )
		m_flStopMoveShootTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.4f, 0.6f );
...
void CNPC_Combine::StartTask( const Task_t *pTask )
{
	...
	case TASK_COMBINE_SIGNAL_BEST_SOUND:
		if( IsInSquad() && GetSquad()->NumMembers() > 1 )
		{
			...
			if( pPlayer && OccupyStrategySlot( SQUAD_SLOT_EXCLUSIVE_HANDSIGN ) && pPlayer->FInAimCone_Entity( BaseEntity() ) )
			{
				CSound *pSound;
				pSound = GetBestSound();
			...
			if (GetHintGroup() == NULL_STRING)
			{
				...
				if ( pEntity )
				{
					...
					if ( !HasMemory(bits_MEMORY_PLAYER_HURT) && pEntity->IsPlayer() && pEntity->GetHealth() <= 20 )
					{
						...
						if ( pSquad )
						{
							pSquad->SquadRemember(bits_MEMORY_PLAYER_HURT);
						}
...
int CNPC_Combine::SelectCombatSchedule()
{
	...
	if ( HasCondition( COND_NEW_ENEMY ) )
	{
		...
		float flTimeSinceFirstSeen = gpGlobals->curtime - GetEnemies()->FirstTimeSeen( pEnemy->BaseEntity() );
		...
		if ( pSquad && pEnemy )
		{
			...
			if ( pSquad->IsLeader( this ) || ( pSquad->GetLeader() && pSquad->GetLeader()->GetEnemy() != pEnemy ) )
			{
	...
	if ( IRelationType( GetEnemy()->BaseEntity() ) == D_FR )
	{
		if (HasCondition( COND_SEE_ENEMY )	|| 
		...
		AI_EnemyInfo_t *pMemory = GetEnemies()->Find( GetEnemy()->BaseEntity() );
...
int CNPC_Combine::SelectSchedule( void )
{
	...
	if ( m_hForcedGrenadeTarget )
	{
		if ( m_flNextGrenadeCheck < gpGlobals->curtime )
		{
			...
			if ( IsElite() )
			{
				if ( FVisible_Entity( m_hForcedGrenadeTarget->BaseEntity() ) )
				{
					m_vecAltFireTarget = vecTarget;
					m_hForcedGrenadeTarget.Set(NULL);
				...
				if ( pSound)
				{
					if (pSound->m_iType & SOUND_DANGER)
					{
						...
						if ( pSoundOwner )
						{
							...
							{
								if ( IRelationType( pGrenade->GetThrower()->BaseEntity() ) != D_LI )
								{
	...
	case NPC_STATE_ALERT:
		{
			...
			if( HasCondition( COND_HEAR_COMBAT ) )
			{
				...
				{
					CAI_Squad *pSquad = m_pSquad;
					if( pSquad && pSquad->GetSquadMemberNearestTo( pSound->GetSoundReactOrigin() ) == this && OccupyStrategySlot( SQUAD_SLOT_INVESTIGATE_SOUND ) )
					{
...
int CNPC_Combine::SelectScheduleAttack()
{
	...
	if ( GetEnemy() && GetEnemy()->Classify() == CLASS_COMBINE && FClassnameIs(GetEnemy(), "npc_turret_floor") )
	{
		...
		float flTimeAtFirstHand = GetEnemies()->TimeAtFirstHand(GetEnemy()->BaseEntity());
	...
	if ( ShouldChargePlayer() )
	{
		...
		if ( HasCondition(COND_SEE_ENEMY) && !IsUnreachable( GetEnemy()->BaseEntity() ) )
		{
	...
	if ( GetEnemy() && !HasCondition(COND_SEE_ENEMY) )
	{
		...
		flTime = gpGlobals->curtime - GetEnemies()->LastTimeSeen( GetEnemy()->BaseEntity() );
		flDist = ( GetEnemy()->GetAbsOrigin() - GetEnemies()->LastSeenPosition( GetEnemy()->BaseEntity() ) ).Length();
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
		{
			...
			if ( pSquad )
			{
				...
				{
					if ( ShouldChargePlayer() && !IsUnreachable( GetEnemy()->BaseEntity() ) )
						return SCHED_COMBINE_CHARGE_PLAYER;
		...
		case COMBINE_AE_GREN_LAUNCH:
			{
				...
				CEntity *pGrenade = CreateNoSpawn( "npc_contactgrenade", Weapon_ShootPosition(), vec3_angle, BaseEntity() );
				if(pGrenade)
				{
					servertools->SetKeyValue(pGrenade->BaseEntity(), "velocity", m_vecTossVelocity);
					pGrenade->Spawn();
				}
		...
		case COMBINE_AE_KICK:
			{
				...
				CBaseEntity *pHurtBase = CheckTraceHullAttack_Float( 70, -Vector(16,16,18), Vector(16,16,18), 0, DMG_CLUB );
				CEntity *pHurt = CEntity::Instance(pHurtBase);
...
bool CNPC_Combine::CheckCanThrowGrenade( const Vector &vecTarget )
{
	...
	if( FInViewCone_Vector( vecTarget ) && CEntity::FVisible_Vector( vecTarget ) )
	{
...
int CNPC_Combine::MeleeAttack1Conditions ( float flDot, float flDist )
{
	...
	CBaseEntity *pEnemy = GetEnemy() ? GetEnemy()->BaseEntity() : NULL;
	if( tr.m_pEnt != pEnemy )
	{
		return COND_NONE;
	}
...
WeaponProficiency_t CNPC_Combine::CalcWeaponProficiency( CBaseEntity *pWeaponBase )
{
	...
	return BaseClass::CalcWeaponProficiency( pWeaponBase );
}
...
{
	if( GetActiveWeapon() && GetActiveWeapon()->ClassMatches(s_iszShotgunClassname) )
	{
		return true;
	}
