...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_crow, cycler, CNPC_Crow );
LINK_ENTITY_TO_CUSTOM_CLASS( npc_seagull, cycler, CNPC_Seagull );
LINK_ENTITY_TO_CUSTOM_CLASS( npc_pigeon, cycler, CNPC_Pigeon );
...
void CNPC_Crow::Spawn( void )
{
	...
	m_flGroundIdleMoveTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.0f, 5.0f );

	SetBloodColor( BLOOD_COLOR_RED );
	m_NPCState = NPC_STATE_NONE;

	m_nMorale = enginerandom->RandomInt( 0, 12 );
...
{
	m_flEnemyDist = (GetLocalOrigin() - CEntity::Instance(pEnemy)->GetLocalOrigin()).Length();
...
void CNPC_Crow::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if( pEvent->event == AE_CROW_HOP )
	{
		...
		m_nMorale -= enginerandom->RandomInt( 1, 6 );
		if ( m_nMorale <= 0 )
		{
			m_nMorale = 0;
		}
	...
	if( pEvent->event == AE_CROW_FLY )
	{
		...
		m_bSoar = false;
		m_flSoarTime = gpGlobals->curtime + enginerandom->RandomFloat( 3, 5 );
...
//
	...
	if ( fRandomize )
	{
		SetCycle( enginerandom->RandomFloat( 0.0, 0.75 ) );
	}
}
...
void CNPC_Crow::InputFlyAway( inputdata_t &inputdata )
{
	...
	{
		CEntity *pEnt = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, sTarget );

		if ( pEnt )
		{
			trace_t tr;
			UTIL_TraceLine ( EyePosition(), pEnt->GetAbsOrigin(), MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );

			if ( tr.fraction != 1.0f )
				return;

			// Find the npc's initial target entity, stash it
			SetGoalEnt( pEnt->BaseEntity() );
		}
	}
	else
		SetGoalEnt( NULL );
...
Activity CNPC_Crow::NPC_TranslateActivity( Activity eNewActivity )
{
	...
	if ( eNewActivity == ACT_FLY )
	{
		if ( m_flSoarTime < gpGlobals->curtime )
		{
			...
			if ( enginerandom->RandomInt( 0, 100 ) <= 50 && m_bSoar == false && GetAbsVelocity().z < 0 )
			{
				m_bSoar = true;
				m_flSoarTime = gpGlobals->curtime + enginerandom->RandomFloat( 1, 4 );
			}
			else
			{
				m_bSoar = false;
...
void CNPC_Crow::MoveCrowFly( float flInterval )
{
	...
	if ( GetHintNode() )
	{
		...
		GetMoveProbe()->MoveLimit( NAV_FLY, GetAbsOrigin(), GetNavigator()->GetCurWaypointPos(), MASK_NPCSOLID, cent, &moveTrace );
	...
	flRoll = clamp( flRoll, -45.0f, 45.0f );
...
bool CNPC_Crow::Probe( const Vector &vecMoveDir, float flSpeed, Vector &vecDeflect )
{
	...
	//
	trace_t tr;
	UTIL_TraceHull( GetAbsOrigin(), GetAbsOrigin() + vecMoveDir * flSpeed, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, BaseEntity(), HL2COLLISION_GROUP_CROW, &tr );
	if ( tr.fraction < 1.0f )
	{
		//
...
void CNPC_Crow::SetFlyingState( FlyState_t eState )
{
	if ( eState == FlyState_Flying )
	{
		...
		m_flGroundIdleMoveTime = gpGlobals->curtime + enginerandom->RandomFloat( 5.0f, 10.0f );
	}
...
void CNPC_Crow::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
...
{
	AI_NavGoal_t goal( GOALTYPE_PATHCORNER, CEntity::Instance(pTargetEnt)->GetAbsOrigin(),
					   ACT_FLY,
...
//		}
		...
		{
			if ( enginerandom->RandomInt( 1, 4 ) == 1 )
			{
				AlertSound();
			}
		...
		case TASK_CROW_PICK_EVADE_GOAL:
		{
			if ( GetEnemy() != NULL )
			{
				...
				//
				// Pick a hop goal a enginerandom distance along a vector away from our enemy.
				//
				m_vSavePosition = GetAbsOrigin() - vecEnemyOrigin;
				VectorNormalize( m_vSavePosition );
				m_vSavePosition = GetAbsOrigin() + (*(m_vSavePosition)) * ( 32 + enginerandom->RandomInt( 0, 32 ) );
		...
		case TASK_FIND_HINTNODE:
		{
			...
			if ( !GetHintNode() )
			{
				CE_AI_Hint *hint = CAI_HintManager::FindHint( this, HINT_CROW_FLYTO_POINT, bits_HINT_NODE_NEAREST | bits_HINT_NODE_USE_GROUP, 10000 );
				SetHintNode(hint);
			}
		...
		case TASK_GET_PATH_TO_HINTNODE:
		{
			...
			CEntity *cent = GetGoalEnt();
			if ( cent )
			{
				SetFlyingState( FlyState_Flying );
				StartTargetHandling( cent->BaseEntity() );
			...
			//
			// We have failed to fly normally. Pick a enginerandom "up" direction and fly that way.
			//
		case TASK_CROW_FLY:
		{
			float flYaw = UTIL_AngleMod( enginerandom->RandomInt( -180, 180 ) );

			Vector vecNewVelocity( cos( DEG2RAD( flYaw ) ), sin( DEG2RAD( flYaw ) ), enginerandom->RandomFloat( 0.1f, 0.5f ) );
			...
			m_bSoar = false;
			m_flSoarTime = gpGlobals->curtime + enginerandom->RandomFloat( 2, 5 );
		...
		{
			m_vSavePosition = GetLocalOrigin() + Vector( enginerandom->RandomFloat( -48.0f, 48.0f ), enginerandom->RandomFloat( -48.0f, 48.0f ), 0 );
			TaskComplete();
			break;
		}
	...
	switch ( pTask->iTask )
	{
		...
		case TASK_CROW_HOP:
		{
			...
			if ( ( GetAbsOrigin().z < m_flHopStartZ ) && ( !( GetFlags() & FL_ONGROUND ) ) )
			{
				...
				//
				trace_t tr;
				UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() + Vector( 0, 0, -32 ), MASK_SOLID, BaseEntity(), HL2COLLISION_GROUP_CROW, &tr );
				if ( tr.fraction == 1.0f )
				{
					//
		...
		case TASK_CROW_WAIT_FOR_BARNACLE_KILL:
		{
			...
			{
				m_flNextFlinchTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.5f, 2.0f );
				// dvs: TODO: squirm
	...
	switch ( m_NPCState )
	{
		...
		case NPC_STATE_COMBAT:
		{
			if ( !IsFlying() )
			{
				...
				{
					m_flGroundIdleMoveTime = gpGlobals->curtime + enginerandom->RandomFloat( 10.0f, 20.0f );
...
//-----------------------------------------------------------------------------
bool CNPC_Crow::HandleInteraction( int interactionType, void *data, CBaseEntity *sourceEnt )
{
