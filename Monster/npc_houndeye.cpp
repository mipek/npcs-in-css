...
#define HL2COLLISION_GROUP_HOUNDEYE 25
static int g_nBeamLaser = 0;
...
{
	CE_INIT_CUSTOM_AI(CNPC_Houndeye);
	...
	g_interactionHoundeyeGroupAttack				= CCombatCharacter::GetInteractionID();
	g_interactionHoundeyeGroupRetreat				= CCombatCharacter::GetInteractionID();
	g_interactionHoundeyeGroupRalley				= CCombatCharacter::GetInteractionID();
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_houndeye, cycler, CNPC_Houndeye );
...
int CNPC_Houndeye::RangeAttack1Conditions ( float flDot, float flDist )
{
	...
	UTIL_TraceHull( GetAbsOrigin(), GetAbsOrigin() + Vector(0,0,0.1), 
					...
					MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	if (tr.startsolid)
	{
...
void CNPC_Houndeye::HandleAnimEvent( animevent_t *pEvent )
{
	switch ( pEvent->event )
	{
		...
		case HOUND_AE_THUMP:
			// emit the shockwaves
			SonicAttack();
			m_flNextAttack = gpGlobals->curtime + enginerandom->RandomFloat( 5.0, 8.0 );
			break;
...
void CNPC_Houndeye::Spawn()
{
	...
	m_bLoopClockwise		= enginerandom->RandomInt(0,1) ? true : false;
...
void CNPC_Houndeye::Precache()
{
	...
	g_nBeamLaser = PrecacheModel("sprites/laser.vmt");
...
void CNPC_Houndeye::AlertSound ( void )
{
	...
	if ( IsInSquad() && !GetSquad()->IsLeader( this ) )
	{
		return; 
	}
...
//=========================================================
// GetBeamColor - get beam color based on the size of the group. 
//=========================================================
void CNPC_Houndeye::GetBeamColor ( int &bRed, int &bGreen, int &bBlue )
{
	if ( IsInSquad() )
	{
		switch ( GetSquad()->NumMembers() )
		{
		case 2:
...
void CNPC_Houndeye::NPCThink(void)
{
	...
	else 
	{
		...
		trace_t tr;
		UTIL_TraceHull( GetAbsOrigin(), GetAbsOrigin() + Vector(0,0,1), 
						GetHullMins(), GetHullMaxs(),
						MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
		if (!tr.startsolid)
		{
...
{
	if (IsInSquad() && enginerandom->RandomInt(0,10) == 10)
	{
		EmitSound( "NPC_Houndeye.Retreat" );
		m_flSoundWaitTime = gpGlobals->curtime + 1.0;

		GetSquad()->BroadcastInteraction( g_interactionHoundeyeGroupRetreat, NULL, this );
	}
...
void CNPC_Houndeye::SonicAttack ( void )
{
	...
	/*void BeamRingPoint( IRecipientFilter& filer, float delay,
	...
	GetBeamColor(r, g, b);
	te->BeamRingPoint(filter, 0.0f, EyePosition(), HOUNDEYE_MIN_ATTACK_RADIUS, HOUNDEYE_MAX_ATTACK_RADIUS+10.f, g_nBeamLaser, 0, 0, 1,
		0.35f, 40.f, 1, 0.5f, r, g, b, 200, 10, 0);
	...
	for ( CEntitySphereQuery sphere( GetAbsOrigin(), HOUNDEYE_MAX_ATTACK_RADIUS ); pEntity = sphere.GetCurrentEntity(); sphere.NextEntity() )
	{
		...
		if ( pEntity->m_takedamage != DAMAGE_NO || pPhysicsObject)
		{
			...
			{
				CTakeDamageInfo info( BaseEntity(), BaseEntity(), flDamageAdjuster * sk_Houndeye_dmg_blast.GetFloat(), DMG_SONIC | DMG_ALWAYSGIB );
				...
				if ( pEntity->IsPlayer() )
				{
					...
					pEntity->ViewPunch( QAngle(enginerandom->RandomInt(-20,20), 0, enginerandom->RandomInt(-20,20)) );
				}
			}
			// ------------------------------
			//  Apply physics foces
...
void CNPC_Houndeye::PrescheduleThink ( void )
{
	...
	if ( m_NPCState == NPC_STATE_COMBAT && ( GetActivity() == ACT_RUN ) && enginerandom->RandomFloat( 0, 1 ) < 0.2 )
	{
		WarnSound();
	}
	...
	if ( !m_fDontBlink )
	{
		if ( ( m_nSkin == 0 ) && enginerandom->RandomInt(0,0x7F) == 0 )
		{// start blinking!
			m_nSkin = HOUNDEYE_EYE_FRAMES - 1;
		}
		else if ( m_nSkin != 0 )
...
bool CNPC_Houndeye::IsAnyoneInSquadAttacking( void )
{
	...
	for (CBaseEntity *pSquadMember = GetSquad()->GetFirstMember( &iter ); pSquadMember; pSquadMember = GetSquad()->GetNextMember( &iter ) )
	{
		...
		if (cent_pSquadMember->IsCurSchedule(SCHED_HOUND_RANGE_ATTACK1))
		{
			return true;
		}
	}
	...
	case NPC_STATE_COMBAT:
		{
			...
			if ( HasCondition( COND_LIGHT_DAMAGE ) | 
				 HasCondition( COND_HEAVY_DAMAGE ) )
			{
				if ( enginerandom->RandomFloat( 0 , 1 ) <= 0.4 )
				{
					trace_t tr;
					Vector forward;
					AngleVectors( GetAbsAngles(), &forward );
					UTIL_TraceHull( GetAbsOrigin(), GetAbsOrigin() + forward * -128, 
						GetHullMins(), GetHullMaxs(),
						MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
			...
			{
				if (IsInSquad() && enginerandom->RandomInt(0,4) == 0)
				{
					if (!IsAnyoneInSquadAttacking())
					{
						...
						GetSquad()->BroadcastInteraction( g_interactionHoundeyeGroupAttack, NULL, this );
						return SCHED_HOUND_GROUP_ATTACK;
					}
				}
				//<<TEMP>>comment
			...
			{
				if (IsInSquad() && enginerandom->RandomInt(0,5) == 0)
				{
					if (!IsAnyoneInSquadAttacking())
					{
						...
						GetSquad()->BroadcastInteraction( g_interactionHoundeyeGroupRalley, NULL, this );
...
//-----------------------------------------------------------------------------
bool CNPC_Houndeye::HandleInteraction(int interactionType, void *data, CCombatCharacter* sourceEnt)
{
	...
	else if (interactionType == g_interactionHoundeyeGroupRalley)
	{
		...
		SetTarget(sourceEnt ? sourceEnt->BaseEntity() : NULL);
