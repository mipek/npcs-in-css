...
#ifndef FLASHLIGHT_NPC_CHECK_INTERVAL
#define FLASHLIGHT_NPC_CHECK_INTERVAL 0.4
#endif //FLASHLIGHT_NPC_CHECK_INTERVAL
...
BEGIN_DATADESC_CENTITY( CNPC_Barnacle )
...
void CNPC_Barnacle::Spawn()
{
	...
	UTIL_SetSize( BaseEntity(), Vector(-16, -16, -40), Vector(16, 16, 0) );
	...
	SetPlaybackRate( enginerandom->RandomFloat( 0.8f, 1.2f ) );
...
void CNPC_Barnacle::PlayerHasIlluminatedNPC( CBasePlayer *pPlayer, float flDot )
{
	...
	if( IsAlive() )
	{
 		g_helpfunc.CSoundEnt_InsertSound( SOUND_MOVE_AWAY | SOUND_CONTEXT_ALLIES_ONLY, m_vecTip, 60.0f, FLASHLIGHT_NPC_CHECK_INTERVAL );
	}
}
...
void CNPC_Barnacle::InitTonguePosition( void )
{
	...
	m_vecTip = ( m_vecRoot - Vector(0,0,(float)m_flAltitude) );
...
void CNPC_Barnacle::BarnacleThink ( void )
{
	...
	if ( m_hRagdoll )
	{
		...
		else if ( m_flDigestFinish )
		{
			...
			if ( m_flDigestFinish > gpGlobals->curtime )
			{
				...
				if ( enginerandom->RandomInt(0,25) == 0 )
				{
	...
	else
	{
		...
		GetCollideable()->WorldSpaceSurroundingBounds( &vecSurroundMins, &vecSurroundMaxs );
		if ( !g_helpfunc.UTIL_FindClientInPVS_VecVec( vecSurroundMins, vecSurroundMaxs ) )
		{
			SetNextThink( gpGlobals->curtime + enginerandom->RandomFloat(1,1.5) );	// Stagger a bit to keep barnacles from thinking on the same frame
		}
		...
		if ( m_cGibs && enginerandom->RandomInt(0,99) == 1 )
		{
			// cough up a gib.
			//CGib::SpawnSpecificGibs( this, 1, 50, 1, "models/gibs/hgibs_rib.mdl");
			m_cGibs--;
		...
		if ( m_flAltitude >= flLength )
		{
			...
			if ( ( pTouchEnt != NULL ) && ( pTouchEnt != m_hLastSpitEnemy.Get() ) )
			{
				...
				CCombatCharacter *pBCC = dynamic_cast<CCombatCharacter *>(pTouchEnt);

				if( pBCC && CanPickup( pBCC ) )
				{
					Vector vecGrabPos = pTouchEnt->EyePosition();
					if( !pBCC || pBCC->DispatchInteraction( g_interactionBarnacleVictimGrab, &vecGrabPos, BaseEntity() ) )
					{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Barnacle::CanPickup( CCombatCharacter *pBCC )
{
...
void CNPC_Barnacle::LiftNPC( float flBiteZOffset )
{
	...
	if ( GetAbsOrigin().z - m_vecTip.z < flBiteZOffset )
	{
		m_bLiftingPrey = false;
...
void CNPC_Barnacle::LiftRagdoll( float flBiteZOffset )
{
	...
	else
	{
		...
		if ( GetEnemy()->IsAlive() )
		{
			CAnimating *pAnimating = dynamic_cast<CAnimating*>( GetEnemy() );
			Assert(pAnimating);
			...
			*/
			...
			// Get the current bone matrix
			matrix3x4_t pBoneToWorld[MAXSTUDIOBONES];
			g_helpfunc.SetupBones( pAnimating->BaseEntity(), pBoneToWorld, BONE_USED_BY_ANYTHING );
			...
			g_helpfunc.SetupBones( pAnimating->BaseEntity(), m_pRagdollBones, BONE_USED_BY_ANYTHING );
		}
	}
}
...
void CNPC_Barnacle::LiftPhysicsObject( float flBiteZOffset )
{
	...
	// Figure out when the prey has reached our bite range
	if ( GetAbsOrigin().z - m_vecTip.z < flBiteZOffset ) // then yes, let's chomp
	{
		if ( m_hTongueTip )
		{
			m_hTongueTip->m_pSpring->SetSpringConstant( BARNACLE_TONGUE_SPRING_CONSTANT_HANGING );
...
void CNPC_Barnacle::LiftPrey( void )
{
	...
	trace_t tr;
	UTIL_TraceLine( WorldSpaceCenter(), pVictim->WorldSpaceCenter(), MASK_SOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	
	bool bEnemyIsNPC = IsEnemyAnNPC() && !IsEnemyARagdoll();
	CBaseEntity *pVictimBase = pVictim->BaseEntity();
	CBaseEntity *pRagdollBase = (m_hRagdoll == NULL) ? NULL : m_hRagdoll->BaseEntity();
	if ( ( bEnemyIsNPC && !pVictim->IsAlive() ) || (tr.fraction < 1.0 && tr.m_pEnt != pVictimBase && tr.m_pEnt != pRagdollBase) )
	{
		if ( !GetEnemy()->IsPlayer() )
		{
			// ignore the object so we don't get into a loop of trying to pick it up.
			m_hLastSpitEnemy.Set(GetEnemy()->BaseEntity());
		}
		LostPrey( false );
		return;
	}
...
//-----------------------------------------------------------------------------
CE_CRagdollProp *CNPC_Barnacle::AttachRagdollToTongue( CAnimating *pAnimating )
{
	// Find his head bone
	...
	CBaseEntity *pBaseRagdoll = g_helpfunc.CreateServerRagdollAttached( (CBaseAnimating*)pAnimating->BaseEntity(), vec3_origin, -1, COLLISION_GROUP_NONE,
		pTonguePhysObject, (CBaseAnimating*)m_hTongueTip->BaseEntity(), 0, vecBonePos, m_iGrabbedBoneIndex, vec3_origin );

	CE_CRagdollProp *pRagdoll = (CE_CRagdollProp*)CEntity::Instance(pBaseRagdoll);
	if ( pRagdoll )
	{
#if HL2_EPISODIC
...
//-----------------------------------------------------------------------------
void CNPC_Barnacle::AttachTongueToTarget( CEntity *pTouchEnt, Vector vecGrabPos )
{
	...
	if ( /* pTouchEnt->IsPlayer() || */ pTouchEnt->MyNPCPointer() )
	{
		...
		CTraceFilterSkipTwoEntities traceFilter( BaseEntity(), pTouchEnt->BaseEntity(), COLLISION_GROUP_NONE );
...
#if BARNACLE_USE_TONGUE_OFFSET
			VectorRotate( m_svPlayerHeldTipOffset, pTouchEnt->EntityToWorldTransform(), m_vecTipDrawOffset );
			m_vecTipDrawOffset.z = m_svPlayerHeldTipOffset.z;
#endif
...
You can use this stanza to try to counterplace the constraint on the player's head so he gets hauled sideways to the right place on the barnacle, but it is better to just move the tongue before attachment.
	...
	CE_CRagdollProp *pRagProp = AttachRagdollToTongue( pAnimating );
	pRagProp->SetDamageEntity( pAnimating->BaseEntity() );
	m_hRagdoll.Set(pRagProp->BaseEntity());
...
void CNPC_Barnacle::BitePrey( void )
{
	...
	pVictim->TakeDamage( CTakeDamageInfo( BaseEntity(), BaseEntity(), nDamage, iDamageType | DMG_CRUSH ) );
...
#endif
		g_helpfunc.DetachAttachedRagdollsForEntity( m_hRagdoll->BaseEntity() );
		if ( bDestroyRagdoll )
		{
			UTIL_Remove( m_hRagdoll );
		}
...
void CNPC_Barnacle::LostPrey( bool bRemoveRagdoll )
{
 	...
 	if ( pEnemy )
	{
		...
		if ( pVictim )
		{
			pVictim->DispatchInteraction( g_interactionBarnacleVictimReleased, NULL, BaseEntity() );
			pVictim->RemoveEFlags( EFL_IS_BEING_LIFTED_BY_BARNACLE );
...
void CNPC_Barnacle::Event_Killed( const CTakeDamageInfo &info )
{
	...
	else if ( m_bSwallowingPrey && m_hRagdoll )
	{
		...
		//m_hRagdoll->RecheckCollisionFilter(); CE_TODO: impl me?
#if defined(BARNACLE_SWALLOW)
		if ( npc_barnacle_swallow.GetBool() )
		{
			m_hRagdoll->SetThink( NULL );
	...
	bool spawnGibs = ( !HasSpawnFlags( SF_BARNACLE_CHEAP_DEATH ) || enginerandom->RandomInt( 0, 1 ) );
	...
	trace_t bloodTrace;
	UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() - Vector( 0, 0, 256 ), MASK_SOLID_BRUSHONLY, BaseEntity(), COLLISION_GROUP_NONE, &bloodTrace);
...
void CNPC_Barnacle::WaitTillDead ( void )
{
	...
	trace_t tr;
	UTIL_TraceLine( m_vecRoot, m_vecRoot - Vector( 0, 0, 256 ), MASK_SOLID_BRUSHONLY, BaseEntity(), COLLISION_GROUP_NONE, &tr );

	if ( tr.fraction < 1.0 )
	{
		float distToFloor = ( m_vecRoot - tr.endpos ).Length();
	...
	float flDist = m_vecRoot.z - m_vecTip.z;
	...
	if ( IsActivityFinished() && bTongueInPosition )
	{
		...
		m_hTongueTip.Set(NULL);
		m_hTongueRoot.Set(NULL);
...
{
public:
	CTongueEntitiesEnum( CEntity **pList, int listMax );
...
CTongueEntitiesEnum::CTongueEntitiesEnum( CEntity **pList, int listMax )
{
	m_pList = pList;
	m_nListMax = listMax;
	m_nCount = 0;
...
bool CTongueEntitiesEnum::AddToList( CEntity *pEntity )
{
...
{
	CEntity *pEnt = CEntity::Instance(pHandleEntity->GetRefEHandle());
	if ( pEnt )
	{
		if ( !AddToList( pEnt ) )
...
//-----------------------------------------------------------------------------
class CBarnacleTongueFilter : public CE_CTraceFilterSimple
{
	DECLARE_CLASS( CBarnacleTongueFilter, CE_CTraceFilterSimple );

public:
	CBarnacleTongueFilter( CEntity *pLastEnemy, const IHandleEntity *passedict, int collisionGroup ) : 
		CE_CTraceFilterSimple( passedict, collisionGroup )
	{
		m_pLastEnemy = pLastEnemy;
		m_pBarnacle = const_cast<CEntity*>( CE_EntityFromEntityHandle( passedict ) );
	}

	virtual bool ShouldHitEntity( IHandleEntity *pServerEntity, int contentsMask )
	{
		if ( CE_EntityFromEntityHandle(pServerEntity) == m_pLastEnemy )
			return true;
...
private:
	CEntity *m_pLastEnemy;
	CEntity *m_pBarnacle;
};


#define BARNACLE_CHECK_SPACING	12
CEntity *CNPC_Barnacle::TongueTouchEnt ( float *pflLength )
{
	trace_t		tr;
	float		length;
	...
	CBarnacleTongueFilter tongueFilter( m_hLastSpitEnemy, BaseEntity(), COLLISION_GROUP_NONE );
	...
	for ( int i = 0; i < nCount; i++ )
	{
		...
		{
			CEntity *pDriver = CEntity::Instance(pTest->GetServerVehicle()->GetPassenger());
			if ( pDriver )
			{
				Vector vecPrevDriverPos = pTest->GetVelocity();
		...
		// NPCs + players
		CCombatCharacter *pVictim = ToBaseCombatCharacter( pTest );
		if ( !pVictim )
			continue;

		// only clients and monsters
		if ( pTest != this && 
			IRelationType( pTest->BaseEntity() ) == D_HT && 
...
//===============================================================================================================================
// Crane tip
LINK_ENTITY_TO_CUSTOM_CLASS( npc_barnacle_tongue_tip, item_sodacan, CBarnacleTongueTip );
...
void CBarnacleTongueTip::Spawn( void )
{
	...
	SetSolid( SOLID_BBOX );
	AddSolidFlags( FSOLID_NOT_STANDABLE );
	UTIL_SetSize( BaseEntity(), Vector(-16, -16, -5), Vector(16, 16, 5) );
	BaseClass::Spawn();
...
//-----------------------------------------------------------------------------
bool CBarnacleTongueTip::CreateSpring( CAnimating *pTongueRoot )
{
...
//-----------------------------------------------------------------------------
CBarnacleTongueTip *CBarnacleTongueTip::CreateTongueTip( CNPC_Barnacle *pBarnacle, CAnimating *pTongueRoot, const Vector &vecOrigin, const QAngle &vecAngles )
{
	CBarnacleTongueTip *pTip = (CBarnacleTongueTip *)CEntity::Create( "npc_barnacle_tongue_tip", vecOrigin, vecAngles );
	if ( !pTip )
		return NULL;

	pTip->VPhysicsDestroyObject();
	IPhysicsObject *pPhysObj = pTip->VPhysicsInitNormal( pTip->GetSolid(), pTip->GetSolidFlags(), false );
		//pTip->GetSolid(), pTip->GetSolidFlags(), false );
	Assert(pPhysObj);
