...
public:
	...
	virtual void OnMinigunStartShooting( CEntity *pTarget ) = 0;
	virtual void OnMinigunStopShooting( CEntity *pTarget ) = 0;
	virtual CAI_NPC *GetEntity() = 0;
};
...
{
public:
	CE_DECLARE_CLASS( CNPC_Strider, CE_Cycler_Fix );
	...
	float			StriderEnemyDistance( CEntity *pEnemy );
	...
	bool			GetWeaponLosZ( const Vector &vOrigin, float minZ, float maxZ, float increment, CEntity *pTarget, float *pResult );
	...
	void			RagdollDeathEffect( CE_CRagdollProp *pRagdoll, float flDuration );
	...
	bool			IsUnusableNode(int iNodeID, CBaseEntity *pHint); // Override for special NPC behavior
	...
	void			OnMinigunStartShooting( CEntity *pTarget ) {};
	void			OnMinigunStopShooting( CEntity *pTarget );
	...
	CEntity		   *GetCannonTarget();
	bool			HasCannonTarget() const;
	bool			IsCannonTarget( CEntity *pTarget ) const;
	bool			AimCannonAt( CEntity *pEntity, float flInterval );
	void			FireCannon();
	void			CannonHitThink();
	...
	void	StriderBusterAttached( CEntity *pAttached );
	void	StriderBusterDetached( CEntity *pAttached );
...
public:
	...
	Vector			CalculateStompHitPosition( CEntity *pEnemy );
	bool			IsLegBoneFollower( CE_CBoneFollower *pFollower );
	CE_CBoneFollower	*GetBoneFollowerByIndex( int nIndex );
...
private:
	...
	void	CreateFireSprite(const Vector &position);
	void	ClearFireSprite();
	void	CreateFireBeam(const Vector &startpos, const Vector &endpos);
	bool	m_bExploding;
	...
	static int		gm_MiniGunAttachment;
...
struct StriderMinigunAnimController_t
{
	...
	{
		target = enginerandom->RandomFloat( minTarget, maxTarget );
		rate = enginerandom->RandomFloat( minRate, maxRate );
	}
};
...
public:
	DECLARE_DATADESC();
	virtual ~CStriderMinigun() {}

	void		Init();
	void		SetTarget( IStriderMinigunHost *pHost, CEntity *pTarget, bool bOverrideEnemy = false );
	CEntity		*GetTarget() { return m_hTarget; }
	...
	void 		AimAtTarget( IStriderMinigunHost *pHost, CEntity *pTarget, bool bSnap = false );
	void 		ShootAtTarget( IStriderMinigunHost *pHost, CEntity *pTarget, float shootTime );
	void 		StartShooting( IStriderMinigunHost *pHost, CEntity *pTarget, float duration );
	void 		ExtendShooting( float timeExtend );
	void 		SetShootDuration( float duration );
	void 		StopShootingForSeconds( IStriderMinigunHost *pHost, CEntity *pTarget, float duration );
	bool 		IsPegged( int dir = MINIGUN_PEGGED_DONT_CARE );
	bool 		CanStartShooting( IStriderMinigunHost *pHost, CEntity *pTargetEnt );
...
class CSparkTrail : public CE_InfoTarget_Fix
{
public:
	CE_DECLARE_CLASS( CSparkTrail, CE_InfoTarget_Fix );
	void Spawn( void );
