...
//=========================================================
//=========================================================
class CNPC_MissileDefense : public CE_Cycler_Fix
{
public:
	CE_DECLARE_CLASS( CNPC_MissileDefense, CE_Cycler_Fix );
	DECLARE_DATADESC();
	...
	void	AimGun( void );
	void	EnemyShootPosition(CEntity* pEnemy, Vector *vPosition);
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_missiledefense, cycler, CNPC_MissileDefense );
...
void CNPC_MissileDefense::FireCannons( void )
{
	...
	// ----------------------------------------------
	trace_t tr;
	UTIL_TraceLine( GetEnemy()->EyePosition(), GetAbsOrigin(), MASK_SOLID_BRUSHONLY, BaseEntity(), COLLISION_GROUP_NONE, &tr);
	if (tr.fraction < 1.0)
	{
		return;
	}
	...
	vecDir.x += enginerandom->RandomFloat( -NOISE, NOISE );
	vecDir.y += enginerandom->RandomFloat( -NOISE, NOISE );

	Vector vecStart = vecGun + vecDir * 110;
	Vector vecEnd	= vecGun + vecDir * 4096;
	UTIL_Tracer( vecStart, vecEnd, 0, TRACER_DONT_USE_ATTACHMENT, 3000 + enginerandom->RandomFloat( 0, 2000 ), fSound );

	vecDir = vecTarget - vecCenter;
	VectorNormalize(vecDir);
	vecDir.x += enginerandom->RandomFloat( -NOISE, NOISE );
	vecDir.y += enginerandom->RandomFloat( -NOISE, NOISE );
	vecDir.z += enginerandom->RandomFloat( -NOISE, NOISE );

	GetAttachment( MD_AP_RGUN, vecGun, vecAng );
	vecStart = vecGun + vecDir * 110;
	vecEnd = vecGun + vecDir * 4096;
	UTIL_Tracer( vecStart, vecEnd, 0, TRACER_DONT_USE_ATTACHMENT, 3000 + enginerandom->RandomFloat( 0, 2000 ) );
	...
	if( flDamage > 0 )
	{
		...
		CTakeDamageInfo info( BaseEntity(), BaseEntity(), flDamage, DMG_MISSILEDEFENSE );
...
void CNPC_MissileDefense::Spawn( void )
{
	...
	UTIL_SetSize( BaseEntity(), Vector( -36, -36 , 0 ), Vector( 36, 36, 64 ) );
...
void CNPC_MissileDefense::Gib(void)
{
	...
	// Smoke
	UTIL_Smoke(GetAbsOrigin(), enginerandom->RandomFloat(10.0, 15.0), 10);
	...
	// Throw manhackgibs
	CE_CGib::SpawnSpecificGibs( BaseEntity(), MD_GIB_COUNT, 300, 500, MD_GIB_MODEL);
}
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_MissileDefense::EnemyShootPosition(CEntity* pEnemy, Vector *vPosition)
{
