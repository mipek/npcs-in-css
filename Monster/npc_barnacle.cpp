...
int	CNPC_Barnacle::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if( hl2_episodic->GetBool() && attacker && attacker->Classify() == CLASS_PLAYER_ALLY_VITAL )
	{
		if( FClassnameIs( attacker, "npc_alyx" ) )
		{
...
void CNPC_Barnacle::LiftPrey( void )
{
	...
	if ( ( bEnemyIsNPC && !pVictim->IsAlive() ) || (tr.fraction < 1.0 && tr.m_pEnt != pVictim->BaseEntity() && m_hRagdoll.IsValid() && tr.m_pEnt != m_hRagdoll->BaseEntity()) )
	{
...
void CNPC_Barnacle::RemoveRagdoll( bool bDestroyRagdoll )
{
	...
	if ( m_hRagdoll )
	{
		...
		if ( npc_barnacle_swallow.GetBool() )
		{
			...
			// CE_TODO: Implement SetBlendWeight (LOOK FOR OTHER CALLS TO THE FUNC IN HERE!)
			//m_hRagdoll->SetBlendWeight( 1.0f );
		}
...
{
	// CE_TODO: impl dem gibs finally...
	m_cGibs = 0; // <-- remove this when doing the CE_TODO
#if 0
...
public:
		...
		CE_CTraceFilterSimple( passedict, collisionGroup )
	{
		...
		m_pBarnacle = CE_EntityFromEntityHandle( passedict );
	}
...
CEntity *CNPC_Barnacle::TongueTouchEnt ( float *pflLength )
{
	...
	CBarnacleTongueFilter tongueFilter( m_hLastSpitEnemy.Get(), BaseEntity(), COLLISION_GROUP_NONE );
