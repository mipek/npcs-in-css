...
LINK_ENTITY_TO_CUSTOM_CLASS( info_target_helicopter_crash, info_target, CE_InfoTarget_Fix );
...
class CGrenadeHelicopter : public CE_Grenade
{
public:
	CE_DECLARE_CLASS( CGrenadeHelicopter, CE_Grenade );
	DECLARE_DATADESC();
...
private:
	...
	void AnimateThink();
	void ExplodeConcussion( CEntity *pOther );
	void BecomeActive();
...
// The bombs the attack helicopter drops
//-----------------------------------------------------------------------------
class CBombDropSensor : public CEntity
{
	DECLARE_DATADESC();

public:
	CE_DECLARE_CLASS( CBombDropSensor, CEntity );
...
//-----------------------------------------------------------------------------
class CBombSuppressor : public CE_InfoTarget_Fix
{
	DECLARE_DATADESC();

public:
	CE_DECLARE_CLASS( CBombSuppressor, CE_InfoTarget_Fix );
...
private:
	typedef CFakeHandle BombSuppressorHandle_t;
...
//-----------------------------------------------------------------------------
class CHelicopterChunk : public CSoda_Fix
{
	DECLARE_DATADESC();

public:
	CE_DECLARE_CLASS( CHelicopterChunk, CSoda_Fix );
	...
	CEFakeHandle<CHelicopterChunk>	m_hMaster;
...
{
public:
	CE_DECLARE_CLASS( CNPC_AttackHelicopter, CBaseHelicopter );
	DECLARE_DATADESC();
	DEFINE_CUSTOM_AI;
...
private:
	...
	CEntity *FindTrackBlocker( const Vector &vecViewPoint, const Vector &vecTargetPos );
	...
	int BuildMissTargetList( int nCount, CEntity **ppMissCandidates );
	...
	void AimCloseToTargetButMiss( CEntity *pTarget, float flMinDist, float flMaxDist, const Vector &shootOrigin, Vector *pResult );
	...
	// Prototype zapper
	bool IsValidZapTarget( CEntity *pTarget );
	void CreateZapBeam( const Vector &vecTargetPos );
	void CreateEntityZapEffect( CEntity *pEnt );
	...
	CEntity *GetCrashPoint()	{ return m_hCrashPoint; }
...
private:
	...
	CEFakeHandle<CBombDropSensor>	m_hSensor;
	float		m_flAvoidMetric;
	AngularImpulse m_vecLastAngVelocity;
	CEFakeHandle<CEntity>	m_hSmokeTrail[MAX_SMOKE_TRAILS];
	...
	// Blinking lights
	CEFakeHandle<CE_CSprite> m_hLights[MAX_HELICOPTER_LIGHTS];
	bool		m_bShortBlink;
...
void CNPC_AttackHelicopter::StopLoopingSounds()
{
	...
	{
		g_SoundController->SoundDestroy( m_pGunFiringSound );
...
void CNPC_AttackHelicopter::Precache( void )
{
	...
	{
		g_helpfunc.UTIL_PrecacheOther( "grenade_helicopter" );
		g_helpfunc.UTIL_PrecacheOther( "env_fire_trail" );
...
{
	CEntity *pEnt = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_iszTransitionTarget );
...
void CNPC_AttackHelicopter::Spawn( void )
{
	...
	m_iMaxHealth = sk_helicopter_health.GetInt();
	m_iHealth = sk_helicopter_health.GetInt();
	...
	CEntity *cent = CreateEntityByName( "npc_helicoptersensor" );
	m_hSensor.Set(cent->BaseEntity());
	cent->Spawn();
	cent->SetParent( BaseEntity(), nBombAttachment );
...
void CNPC_AttackHelicopter::Startup()
{
	...
	if ( HasSpawnFlags( SF_HELICOPTER_LIGHTS ) )
	{
		for ( int i = 0; i < MAX_HELICOPTER_LIGHTS; ++i )
		{
			...
			if ( nAttachment == 0 )
			{
				m_hLights[i].Set(NULL);
				continue;
			}

			CE_CSprite *sprite = CE_CSprite::SpriteCreate( CHOPPER_RED_LIGHT_SPRITE, vec3_origin, false );

			if ( !m_hLights[i] )
				continue;

			m_hLights[i].Set(sprite->BaseEntity());

			sprite->SetParent( BaseEntity(), nAttachment );
			sprite->SetLocalOrigin( vec3_origin );
			sprite->SetLocalVelocity( vec3_origin );
			sprite->SetMoveType( MOVETYPE_NONE );
			sprite->SetTransparency( kRenderTransAdd, 255, 255, 255, 200, kRenderFxNone );
...
{
	UTIL_ImpactTrace( &tr, nDamageType, "Impact" );
}
...
void CNPC_AttackHelicopter::InitializeRotorSound( void )
{
	if ( !m_pRotorSound )
	{
		...
		if ( HasSpawnFlags( SF_HELICOPTER_LOUD_ROTOR_SOUND ) )
		{
			m_pRotorSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_AttackHelicopter.RotorsLoud" );
		}
		else
		{
			m_pRotorSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_AttackHelicopter.Rotors" );
		}

		m_pRotorBlast = g_SoundController->SoundCreate( filter, entindex(), "NPC_AttackHelicopter.RotorBlast" );
		m_pGunFiringSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_AttackHelicopter.FireGun" );
		if(m_pGunFiringSound) {
			g_SoundController->Play( m_pGunFiringSound, 0.0, 100 );
		}
	}
	else
	{
...
void CNPC_AttackHelicopter::InputDisablePathVisibilityTests( inputdata_t &inputdata )
{
	...
	GetEnemies()->SetUnforgettable( GetEnemy_CBase(), true );
}

void CNPC_AttackHelicopter::InputEnablePathVisibilityTests( inputdata_t &inputdata )
{
	m_bIgnorePathVisibilityTests = false;
	GetEnemies()->SetUnforgettable( GetEnemy_CBase(), false );
}
...
void CNPC_AttackHelicopter::InputSelfDestruct( inputdata_t &inputdata )
{
	...
	CTakeDamageInfo info( BaseEntity(), BaseEntity(), Vector(0, 0, 1), WorldSpaceCenter(), GetMaxHealth(), CLASS_MISSILE );
	TakeDamage( info );
}
...
//-----------------------------------------------------------------------------
class CTraceFilterChopper : public CE_CTraceFilterSimple
{
	DECLARE_CLASS( CTraceFilterChopper, CE_CTraceFilterSimple );
...
{
	CEntity *pEnt = CEntity::Instance(static_cast<IServerUnknown*>(pServerEntity)->GetBaseEntity());
	if ( pEnt )
	{
...
// Enemy visibility check
//-----------------------------------------------------------------------------
CEntity *CNPC_AttackHelicopter::FindTrackBlocker( const Vector &vecViewPoint, const Vector &vecTargetPos )
{
	if ( m_bIgnorePathVisibilityTests )
		return NULL;

	CTraceFilterChopper chopperFilter( BaseEntity(), COLLISION_GROUP_NONE );

	trace_t	tr;
	UTIL_TraceHull( vecViewPoint, vecTargetPos, -Vector(4,4,4), Vector(4,4,4), MASK_SHOT, &chopperFilter, &tr );
...
// More Enemy visibility check
//-----------------------------------------------------------------------------
bool CNPC_AttackHelicopter::FVisible_Entity( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	CEntity *cent = CEntity::Instance(pEntity);
	if ( cent->GetFlags() & FL_NOTARGET )
		return false;
	...
	Vector vecTargetOrigin = cent->EyePosition();
...
Vector CNPC_AttackHelicopter::GetAttackSpread( CBaseEntity *pWeapon, CBaseEntity *pTarget )
{
...
// Find interesting nearby things to shoot
//------------------------------------------------------------------------------
int CNPC_AttackHelicopter::BuildMissTargetList( int nCount, CEntity **ppMissCandidates )
{
	...
	for ( int i = 0; i < numEnts; i++ )
	{
		...
		CEntity *cent = pEnts[i];
		// Miss candidates cannot include the player or his vehicle
		if ( cent == GetEnemyVehicle() || cent == GetEnemy() )
			continue;

		// See if it's a good target candidate
		if ( FClassnameIs( cent, "prop_dynamic" ) ||
			 FClassnameIs( cent, "prop_physics" ) ||
			 FClassnameIs( cent, "physics_prop" ) )
		{
			ppMissCandidates[numMissCandidates++] = cent;
		}
	}
...
// Gets a vehicle the enemy is in (if any)
//------------------------------------------------------------------------------
CEntity *CNPC_AttackHelicopter::GetEnemyVehicle()
{
	CEntity *cent = GetEnemy();
	if ( !cent )
		return NULL;
	...
	return CEntity::Instance(static_cast<CPlayer*>(cent)->GetVehicleEntity());
}
...
void CNPC_AttackHelicopter::ShootAtPlayer( const Vector &vBasePos, const Vector &vGunDir )
{
	...
	{
		int nSwap = enginerandom->RandomInt( 0, nActualTargets - 1 );
...
void CNPC_AttackHelicopter::PickDirectionToCircleOfDeath( const Vector &vBasePos, const Vector &vecFireAtPosition, Vector *pResult )
{
	*pResult = vecFireAtPosition;
	...
	do
	{
		x = enginerandom->RandomFloat( -1.0f, 1.0f );
		y = enginerandom->RandomFloat( -1.0f, 1.0f );
	} while ( (x * x + y * y) > 1.0f );
...
// Deliberately aims as close as possible w/o hitting
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::AimCloseToTargetButMiss( CEntity *pTarget, float flMinDist, float flMaxDist, const Vector &shootOrigin, Vector *pResult )
{
	Vector vecDirection;
	VectorSubtract( pTarget->WorldSpaceCenter(), shootOrigin, vecDirection );
	float flDist = VectorNormalize( vecDirection );
	float flRadius = pTarget->BoundingRadius() + enginerandom->RandomFloat( flMinDist, flMaxDist );
	...
	// Choose enginerandom points in a plane perpendicular to the shoot origin.
	Vector vecRandomDir;
...
void CNPC_AttackHelicopter::FireBullets( const FireBulletsInfo_t &info )
{
	...
	if ( !GetEnemyVehicle() && !IsDeadlyShooting() )
	{
		if ( m_nBurstHits >= m_nMaxBurstHits )
		{
			...
			actualInfo.m_pAdditionalIgnoreEnt = GetEnemy_CBase();
			BaseClass::FireBullets( actualInfo );
			return;
		}
	}

	CPlayer *pPlayer = assert_cast<CPlayer*>(GetEnemy());
...
{
	CEntity *pVehicle = GetEnemyVehicle();
...
bool CNPC_AttackHelicopter::DoGunIdle( const Vector &vGunDir, const Vector &vTargetDir )
{
	...
	else
	{
		...
		m_flChargeTime = gpGlobals->curtime + enginerandom->RandomFloat(flChargeTime - flVariance, flChargeTime + flVariance);
		m_nGunState = GUN_STATE_CHARGING;
		m_flAvoidMetric = 0.0f;
		m_vecLastAngVelocity.Init( 0, 0, 0 );
	}
...
bool CNPC_AttackHelicopter::DoGunCharging( )
{
	...
	switch( GetShootingMode() )
	{
		...
		case SHOOT_MODE_FAST:
		{
			...
			m_nRemainingBursts = enginerandom->RandomInt( nBurstCount, 2 * nBurstCount );
	...
	if ( !GetEnemyVehicle() )
	{
		m_nMaxBurstHits = !IsDeadlyShooting() ? enginerandom->RandomInt( 6, 9 ) : 200;
	...
	else
	{
		...
		m_nMaxBurstHits = enginerandom->RandomInt( nMin, nHitCount + 1 );

		int nNearShots = (int)(RemapVal( flTotal, 0.0f, 1.0f, 70, 5 ) + 0.5f);
		int nMinNearShots = nNearShots >= 5 ? nNearShots - 5 : 0;
		m_nMaxNearShots = enginerandom->RandomInt( nMinNearShots, nNearShots + 5 );
...
// Can we zap it?
//-----------------------------------------------------------------------------
bool CNPC_AttackHelicopter::IsValidZapTarget( CEntity *pTarget )
{
...
void CNPC_AttackHelicopter::CreateZapBeam( const Vector &vecTargetPos )
{
	...
	data.m_flScale = 5;
	g_helpfunc.DispatchEffect( "TeslaZap", data );
}

void CNPC_AttackHelicopter::CreateEntityZapEffect( CEntity *pEnt )
{
	CEffectData	data;
...
void CNPC_AttackHelicopter::FireElectricityGun( )
{
	...
	CEntity *ppCandidates[256];
	int nCandidateCount = 0;
	int i;
	for ( i = 0; i < nEntCount; i++ )
	{
		CEntity *cent = ppEnts[i];
		if (cent == NULL )
			continue;

		// Zap metal or flesh things.
		if ( !IsValidZapTarget( cent ) )
			continue;

		ppCandidates[ nCandidateCount++ ] = cent;
	}

	// First, put a bolt in front of the player, at enginerandom
	float flDist = 1024;
	if ( GetEnemy() )
	{
		Vector vecDelta;
		...
		for ( i = 0; i < nBoltCount; ++i )
		{
			...
			VectorMA( vecTargetPt, enginerandom->RandomFloat( flDist + 100, flDist + 500 ), vecDelta, vecTargetPt );
			VectorMA( vecTargetPt, enginerandom->RandomFloat( -500, 500 ), vecPerp, vecTargetPt );
			vecTargetPt.z += enginerandom->RandomFloat( -500, 500 );
			CreateZapBeam( vecTargetPt );
		}
	}

	// Next, choose the number of bolts...
	int nBoltCount = enginerandom->RandomInt( 8, 16 );
	for ( i = 0; i < nBoltCount; ++i )
	{
		if ( (nCandidateCount > 0) && enginerandom->RandomFloat( 0.0f, 1.0f ) < 0.6f )
		{
			--nCandidateCount;
		...
		{
			// Select enginerandom point *on* sphere
			Vector vecTargetPt;
			float flEffectRadius = enginerandom->RandomFloat( flRadius * 1.2, flRadius * 1.5f );
			float flTheta = enginerandom->RandomFloat( 0.0f, 2.0f * M_PI );
			float flPhi = enginerandom->RandomFloat( -0.5f * M_PI, 0.5f * M_PI );
	...
	// Finally, put a bolt right at the player, at enginerandom
	float flHitRatio = ClampSplineRemapVal( flDist, 128.0f, 512.0f, 0.75f, 0.0f );
	if ( enginerandom->RandomFloat( 0.0f, 1.0f ) < flHitRatio )
	{
		if ( GetEnemyVehicle() )
		{
			Vector vecTarget;
			...
			CTakeDamageInfo info( BaseEntity(), BaseEntity(), 5, DMG_SHOCK );
	...
	m_flNextAttack = gpGlobals->curtime + enginerandom->RandomFloat( 0.3f, 1.0f );
}
...
{
	if(m_pGunFiringSound) {
		float flVolume = g_SoundController->SoundGetVolume( m_pGunFiringSound );
		if ( flVolume != 1.0f )
		{
			g_SoundController->SoundChangeVolume( m_pGunFiringSound, 1.0, 0.01f );
		}
	}
	...
	if(m_pGunFiringSound) {
		g_SoundController->SoundChangeVolume( m_pGunFiringSound, 0.0, 0.01f );
	}
	float flIdleTime = CHOPPER_GUN_IDLE_TIME;
	float flVariance = flIdleTime * 0.1f;
	m_flNextAttack = gpGlobals->curtime + m_flIdleTimeDelay + enginerandom->RandomFloat(flIdleTime - flVariance, flIdleTime + flVariance);
	m_nGunState = GUN_STATE_IDLE;
	SetPauseState( PAUSE_NO_PAUSE );
	return true;
}
...
void CNPC_AttackHelicopter::CreateBomb( bool bCheckForFairness, Vector *pVecVelocity, bool bMegaBomb )
{
	...
	if ( !pVecVelocity )
	{
		...
		vecAcross *= enginerandom->RandomFloat( 10.0f, 30.0f );
		vecAcross *= enginerandom->RandomFloat( 0.0f, 1.0f ) < 0.5f ? 1.0f : -1.0f;
...
void CNPC_AttackHelicopter::InputDropBomb( inputdata_t &inputdata )
{
	...
	if ( ShouldDropBombs() )
	{
		m_flNextAttack = gpGlobals->curtime + 0.5f + enginerandom->RandomFloat( 0.3f, 0.6f );
	}
}
...
void CNPC_AttackHelicopter::InputDropBombAtTargetInternal( inputdata_t &inputdata, bool bCheckFairness )
{
	...
	CEntity *pBombEnt = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, strBombTarget );
	if ( pBombEnt == NULL )
	{
	...
	if ( ShouldDropBombs() )
	{
		m_flNextAttack = gpGlobals->curtime + 1.5f + enginerandom->RandomFloat( 0.1f, 0.2f );
	}
}
...
void CNPC_AttackHelicopter::DropBombs( )
{
	...
	if ( (m_nAttackMode != ATTACK_MODE_BULLRUSH_VEHICLE) )
	{
		...
		{
			m_nGrenadeCount = CHOPPER_BOMB_DROP_COUNT;
			m_flNextAttack += enginerandom->RandomFloat( 1.5f, 3.0f );
		}
	}
}
...
void CNPC_AttackHelicopter::BullrushBombs( )
{
	...
	else
	{
		...
		VectorNormalize( vecAcross );
		vecAcross *= enginerandom->RandomFloat( 300.0f, 500.0f );
...
void CNPC_AttackHelicopter::AddSmokeTrail( const Vector &vecPos )
{
	...
	{
		CE_CFireTrail *pFireTrail = CE_CFireTrail::CreateFireTrail();

		if ( pFireTrail == NULL )
			return;

		m_hSmokeTrail[m_nSmokeTrailCount].Set(pFireTrail->BaseEntity());

		pFireTrail->FollowEntity( this, UTIL_VarArgs( "damage%d", m_nSmokeTrailCount ) );
		pFireTrail->SetParent( BaseEntity(), nAttachment );
	...
	else
	{
		...
		m_hSmokeTrail[m_nSmokeTrailCount].Set(pSmokeTrail->BaseEntity());
		...
		pSmokeTrail->SetParent( BaseEntity(), nAttachment );
...
void Chopper_CreateChunk( CEntity *pChopper, const Vector &vecChunkPos, const QAngle &vecChunkAngles, const char *pszChunkName, bool bSmall )
{
	...
	CE_CGib *pChunk = (CE_CGib *)CreateEntityByName( "gib" );
	...
	pChunk->SetOwnerEntity( pChopper->BaseEntity() );

	if ( bSmall )
	{
		pChunk->m_lifeTime = enginerandom->RandomFloat( 0.5f, 1.0f );
	...
	QAngle angles;
	angles.x = enginerandom->RandomFloat( -70, 20 );
	angles.y = enginerandom->RandomFloat( 0, 360 );
	angles.z = 0.0f;
	AngleVectors( angles, &vecVelocity );

	vecVelocity *= enginerandom->RandomFloat( 550, 800 );
	...
	pFireTrail->SetParent( pChunk->BaseEntity(), 0 );
	pFireTrail->SetLocalOrigin( vec3_origin );
	pFireTrail->SetMoveType( MOVETYPE_NONE );
...
void CNPC_AttackHelicopter::ExplodeAndThrowChunk( const Vector &vecExplosionPos )
{
	...
	g_helpfunc.DispatchEffect( "HelicopterMegaBomb", data );
	...
	if(GetCrashPoint() != NULL)
	{
		...
		ExplosionCreate( vecExplosionPos, QAngle(0,0,1), BaseEntity(), 100, 128, false );
	}

	if ( enginerandom->RandomInt( 0, 4 ) )
	{
		for ( int i = 0; i < 2; i++ )
		{
	...
	{
		Chopper_CreateChunk( this, vecExplosionPos, RandomAngle(0, 360), s_pChunkModelName[enginerandom->RandomInt( 0, CHOPPER_MAX_SMALL_CHUNKS - 1 )], false );
	}
}
...
void CNPC_AttackHelicopter::DropCorpse( int nDamage )
{
	...
	CEntity *pGib = CreateRagGib( "models/combine_soldier.mdl", GetAbsOrigin(), GetAbsAngles(), vecForceVector );
	if ( pGib )
	{
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr , CDmgAccumulator *dump)
{
	CEntity *inf = CEntity::Instance(info.GetInflictor());
	CEntity *att = CEntity::Instance(info.GetAttacker());
	...
	if ( ( info.GetDamageType() & DMG_AIRBOAT ) ||
		 ( inf && inf->Classify() == CLASS_MISSILE ) ||
		 ( att && att->Classify() == CLASS_MISSILE ) )
	{
		BaseClass::BaseClass::TraceAttack( info, vecDir, ptr, dump );
	}
...
int CNPC_AttackHelicopter::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	// CE_TODO: missile and stuff
	/*if( inf != this )
	{
		if ( ( ( info.GetDamageType() & DMG_AIRBOAT ) == 0 ) &&
			 ( inf->Classify() != CLASS_MISSILE ) &&
			 ( att->Classify() != CLASS_MISSILE ) )
			return 0;
	}*/
	...
	CGrenadeHelicopter *pGren = dynamic_cast<CGrenadeHelicopter *>(inf);
	if ( pGren && att && att->IsPlayer() )
	{
		CTakeDamageInfo fudgedInfo = info;

		float damage;
		if( g_helpfunc.GameRules_IsSkillLevel(SKILL_EASY) )
		{
			damage = GetMaxHealth() / sk_helicopter_num_bombs1.GetFloat();
		}
		else if( g_helpfunc.GameRules_IsSkillLevel(SKILL_HARD) )
		{
...
int CNPC_AttackHelicopter::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( ( inf != NULL ) && ( inf->GetOwnerEntity() != NULL ) && ( inf->GetOwnerEntity() == this ) )
	{
	...
	if( info.GetDamageType() & DMG_BLAST )
	{
		...
		m_flGoalRollDmg = enginerandom->RandomFloat( 10, 30 );
	...
	if ( nPrevHealth != GetHealth() )
	{
		...
		{
			m_OnHealthChanged.Set( nCurrPercent, BaseEntity(), BaseEntity() );
		}
	}
...
void Chopper_BecomeChunks( CEntity *pChopper )
{
	...
	Chopper_CreateChunk( pChopper, vecChunkPos, RandomAngle( 0, 360 ), s_pChunkModelName[enginerandom->RandomInt( 0, CHOPPER_MAX_CHUNKS - 1 )], false );
	...
	pCockpitChunk->m_hMaster.Set(pBodyChunk->BaseEntity());
	...
	pTailChunk->m_hMaster.Set(pBodyChunk->BaseEntity());
...
void CNPC_AttackHelicopter::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if(m_pGunFiringSound)
	{
		g_SoundController->SoundChangeVolume( m_pGunFiringSound, 0.0, 0.1f );
	}

	if( GetCrashPoint() == NULL )
	{
		CEntity *pCrashPoint = g_helpfunc.FindEntityByClassname( (CBaseEntity *)NULL, "info_target_helicopter_crash" );
		if( pCrashPoint != NULL )
		{
			m_hCrashPoint.Set( pCrashPoint->BaseEntity() );
			SetDesiredPosition( pCrashPoint->GetAbsOrigin() );

			// Start the failing engine sound
			g_SoundController->SoundDestroy( m_pRotorSound );

			CPASAttenuationFilter filter( this );
			m_pRotorSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_AttackHelicopter.EngineFailure" );
			if(m_pRotorSound) {
				g_SoundController->Play( m_pRotorSound, 1.0, 100 );
			}
	...
	m_OnDeath->FireOutput( info.GetAttacker(), BaseEntity() );
}
...
void CNPC_AttackHelicopter::CreateChopperHusk()
{
	...
	CEntity *pCorpse = CreateEntityByName( "prop_physics" );
...
void CNPC_AttackHelicopter::PrescheduleThink( void )
{
	...
	switch( m_lifeState )
	{
		case LIFE_DYING:
		{
			...
			if ( enginerandom->RandomInt( 0, 4 ) == 0 )
			{
				Vector	explodePoint;
											 ...
											 float flAdditionalHeight, float flMinDistFromSegment, float flMaxDistFromSegment, Vector *pVecAccel )
{
	...
	if ( flMaxDistFromSegment != 0.0f )
	{
		...
		if ( flDistFromPath > flMaxDistFromSegment )
		{
			...
			flAmount = clamp( flAmount, 0.0f, 1.0f );
...
void CNPC_AttackHelicopter::ComputeAngularVelocity( const Vector &vecGoalUp, const Vector &vecFacingDirection )
{
	...
	if ( m_lifeState != LIFE_DYING || (m_lifeState == LIFE_DYING && GetCrashPoint() != NULL) )
	{
		...
		goalPitch = clamp( goalPitch, -30.0f, 45.0f );
		goalRoll = clamp( goalRoll, -45.0f, 45.0f );
		...
		goalAngAccel.x = clamp( goalAngAccel.x, -300.0f, 300.0f );
		//goalAngAccel.y = clamp( goalAngAccel.y, -60, 60 );
		goalAngAccel.y = clamp( goalAngAccel.y, -120.0f, 120.0f );
		goalAngAccel.z = clamp( goalAngAccel.z, -300.0f, 300.0f );
	}
	else
	{
		goalAngAccel.x	= 0;
		goalAngAccel.y = enginerandom->RandomFloat( 50, 120 );
		goalAngAccel.z	= 0;
	}
	...
	angAccelAccel.x = clamp( angAccelAccel.x, -1000.0f, 1000.0f );
	angAccelAccel.y = clamp( angAccelAccel.y, -1000.0f, 1000.0f );
	angAccelAccel.z = clamp( angAccelAccel.z, -1000.0f, 1000.0f );
	...
	angVel.y = clamp( angVel.y, -120.0f, 120.0f );

	// Fix up pitch and yaw to tend toward small values
	if ( m_lifeState == LIFE_DYING && GetCrashPoint() == NULL )
	{
		float flPitchDiff = enginerandom->RandomFloat( -5, 5 ) - GetAbsAngles().x;
		angVel.x = flPitchDiff * 0.1f;
		float flRollDiff = enginerandom->RandomFloat( -5, 5 ) - GetAbsAngles().z;
		angVel.z = flRollDiff * 0.1f;
	}

	SetLocalAngularVelocity( angVel );

	float flAmt = clamp( angVel.y, -30.0f, 30.0f );
...
void CNPC_AttackHelicopter::FlightDirectlyOverhead( void )
{
	...
	if ( HasEnemy() && FVisible_Entity( pEnemy->BaseEntity() ) )
	{
		if ( GetEnemy()->IsPlayer() )
		{
			CEntity *pEnemyVehicle = CEntity::Instance(assert_cast<CPlayer*>(GetEnemy())->GetVehicleEntity());
			if ( pEnemyVehicle )
			{
...
void CNPC_AttackHelicopter::UpdateEnemyLeading( void )
{
	...
	CEntity *pTarget = GetEnemyVehicle();
	if ( !pTarget )
	{
...
void CNPC_AttackHelicopter::SetTransmit( CCheckTransmitInfo *pInfo, bool bAlways )
{
	...
	{
		CEntity *cent = m_hSmokeTrail[i];
		//CE_FIX
		//cent->m_bInSetTransmit = false;
		cent->SetTransmit( pInfo, bAlways );
	}
...
//
//------------------------------------------------------------------------------
LINK_ENTITY_TO_CUSTOM_CLASS( npc_helicoptersensor, info_target, CBombDropSensor );
...
{
	BaseClass::Spawn();
	UTIL_SetSize(BaseEntity(), Vector(-30,-30,-30), Vector(30,30,30) );
	SetSolid(SOLID_BBOX);
...
void CBombDropSensor::InputDropBomb( inputdata_t &inputdata )
{
	...
	myVersion.pActivator = BaseEntity();
...
LINK_ENTITY_TO_CUSTOM_CLASS( grenade_helicopter, grenade, CGrenadeHelicopter );
...
void CGrenadeHelicopter::Spawn( void )
{
	...
	// Tumble in air
	QAngle vecAngVel( enginerandom->RandomFloat ( -100, -500 ), 0, 0 );
	...
	g_pNotify->AddEntity( BaseEntity(), BaseEntity() );
...
void CGrenadeHelicopter::UpdateOnRemove()
{
	if( m_pWarnSound )
	{
		g_SoundController->SoundDestroy( m_pWarnSound );
		m_pWarnSound = NULL;
	}


	g_pNotify->ClearEntity( BaseEntity() );
	BaseClass::UpdateOnRemove();
}
...
void CGrenadeHelicopter::BecomeActive()
{
	...
	if ( !bMegaBomb )
	{
		...
		m_pWarnSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_AttackHelicopterGrenade.Ping" );
		if(m_pWarnSound) {
			g_SoundController->Play( m_pWarnSound, 1.0, PITCH_NORM );
		}
	}
...
void CGrenadeHelicopter::RampSoundThink( )
{
	if ( m_pWarnSound )
	{
		g_SoundController->SoundChangePitch( m_pWarnSound, 140, BOMB_RAMP_SOUND_TIME );
	}
...
*/
	...
	// Frighten people
	g_helpfunc.CSoundEnt_InsertSound ( SOUND_DANGER, WorldSpaceCenter(), g_helicopter_bomb_danger_radius.GetInt(), 0.2f, BaseEntity(), SOUNDENT_CHANNEL_REPEATED_DANGER );
...
void CGrenadeHelicopter::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	if( hl2_episodic->GetBool() )
	{
		...
		if( flImpactSpeed > 400.0f && CEntity::Instance(pEvent->pEntities[ 1 ])->IsWorld() )
		{
...
{
	ExplosionCreate( GetAbsOrigin(), GetAbsAngles(), GetOwnerEntity() ? GetOwnerEntity()->BaseEntity() : BaseEntity(), sk_helicopter_grenadedamage.GetInt(),
					 ...
					 sk_helicopter_grenadeforce.GetFloat(), BaseEntity() );
...
static inline float GetActualGravity( CEntity *pEnt )
{
...
//------------------------------------------------------------------------------
void CGrenadeHelicopter::ExplodeConcussion( CEntity *pOther )
{
	if ( !pOther->IsSolid() )
		return;
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_heli_avoidsphere, info_target, CAvoidSphere );
...
//-----------------------------------------------------------------------------
CEntity *CreateHelicopterAvoidanceSphere( CBaseEntity *pParent, int nAttachment, float flRadius, bool bAvoidBelow )
{
...
void CAvoidSphere::Activate( )
{
	BaseClass::Activate();
	CFakeHandle handle;
	handle.Set(BaseEntity());
	s_AvoidSpheres.AddToTail( handle );
}
...
void CAvoidSphere::UpdateOnRemove( )
{
	CFakeHandle handle;
	handle.Set(BaseEntity());
	s_AvoidSpheres.FindAndRemove( handle );
	BaseClass::UpdateOnRemove();
}
...
// Where are how should we avoid?
//-----------------------------------------------------------------------------
void CAvoidSphere::ComputeAvoidanceForces( CEntity *pEntity, float flEntityRadius,
										   float flAvoidTime, Vector *pVecAvoidForce )
{
	...
	{
		CAvoidSphere *pSphere = (CAvoidSphere *)(s_AvoidSpheres[i].Get());
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_heli_avoidbox, info_target, CAvoidBox );
...
void CAvoidBox::UpdateOnRemove( )
{
	CFakeHandle handle;
	handle.Set(BaseEntity());
	s_AvoidBoxes.FindAndRemove( handle );
	BaseClass::UpdateOnRemove();
}
...
// Where are how should we avoid?
//-----------------------------------------------------------------------------
void CAvoidBox::ComputeAvoidanceForces( CEntity *pEntity, float flEntityRadius, float flAvoidTime, Vector *pVecAvoidForce )
{
	pVecAvoidForce->Init();
	...
	{
		CAvoidBox *pBox = (CAvoidBox *)(s_AvoidBoxes[i].Get());
		...
		vecBoxMin += pBox->CollisionProp_Actual()->OBBMins();
		vecBoxMax += pBox->CollisionProp_Actual()->OBBMaxs();
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_heli_nobomb, info_target, CBombSuppressor );
...
void CBombSuppressor::Activate( )
{
	...
	CFakeHandle handle;
	handle.Set(BaseEntity());
	s_BombSuppressors.AddToTail( handle );
}
...
void CBombSuppressor::UpdateOnRemove( )
{
	CFakeHandle handle;
	handle.Set(BaseEntity());
	s_BombSuppressors.FindAndRemove( handle );
	BaseClass::UpdateOnRemove();
}
...
bool CBombSuppressor::CanBomb( const Vector &vecPosition )
{
	...
	{
		CBombSuppressor *pBox = (CBombSuppressor *)(s_BombSuppressors[i].Get());
...
LINK_ENTITY_TO_CUSTOM_CLASS( helicopter_chunk,item_sodacan,  CHelicopterChunk );
...
void CHelicopterChunk::FallThink( void )
{
	...
	if ( enginerandom->RandomInt( 0, 8 ) == 0 )
	{
		CEffectData data;
...
void CHelicopterChunk::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	if ( m_bLanded == false )
	{
		...
		CEntity *pOther = CEntity::Instance(pEvent->pEntities[otherIndex]);
		if ( !pOther )
			return;
...
CHelicopterChunk *CHelicopterChunk::CreateHelicopterChunk( const Vector &vecPos, const QAngle &vecAngles, const Vector &vecVelocity, const char *pszModelName, int chunkID )
{
	...
	pFireTrail->SetParent( pChunk->BaseEntity(), 1 );
	pFireTrail->SetLocalOrigin( vec3_origin );
	pFireTrail->SetMoveType( MOVETYPE_NONE );
