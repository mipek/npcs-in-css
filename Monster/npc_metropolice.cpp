...
// Purpose
//------------------------------------------------------------------------------
CBaseEntity *CNPC_MetroPolice::CheckTraceHullAttack_Float( float flDist, const Vector &mins, const Vector &maxs, int iDamage, int iDmgType, float forceScale, bool bDamageAnyNPC )
{
	...
	return CheckTraceHullAttack_Vector( vStart, vEnd, mins, maxs, iDamage, iDmgType, forceScale, bDamageAnyNPC );
}
	...
	virtual bool ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
	{
		...
		if ( pEntity )
		{
			...
			if ( pEntity->GetServerVehicle() != NULL )
			{
				...
				if ( pDriver != NULL )
				{
					pEntity = CEntity::Instance(pDriver);
				}
			}
			...
			if ( pBCC && pVictimBCC )
			{
				...
				if ( m_bDamageAnyNPC || pBCC->IRelationType( pEntity->BaseEntity() ) == D_HT || pEntity->IsPlayer() )
				{
					if ( info.GetDamage() )
					{
						...
						{
							if ( pEntity->IsPlayer() && ((CPlayer *)pEntity)->IsSuitEquipped() )
							{
								info.ScaleDamage( .25 );
...
// Output  :
//------------------------------------------------------------------------------
CBaseEntity *CNPC_MetroPolice::CheckTraceHullAttack_Vector( const Vector &vStart, const Vector &vEnd, const Vector &mins, const Vector &maxs, int iDamage, int iDmgType, float flForceScale, bool bDamageAnyNPC )
{

	CTakeDamageInfo	dmgInfo( BaseEntity(), BaseEntity(), iDamage, DMG_SLASH );

	CTraceFilterMetroPolice traceFilter( BaseEntity(), COLLISION_GROUP_NONE, &dmgInfo, flForceScale, bDamageAnyNPC );
	...
	CEntity *pEntity = traceFilter.m_pHit;
	...
	return (pEntity)?pEntity->BaseEntity():NULL;
}
...
void CNPC_MetroPolice::NotifyDeadFriend( CBaseEntity* pFriend )
{
	...
	CEntity *cent_pFriend = CEntity::Instance(pFriend);
	if ( cent_pFriend == m_hManhack )
	{
	...
	if ( FClassnameIs( cent_pFriend, "npc_manhack" ) )
		return;
...
void CNPC_MetroPolice::PrescheduleThink( void )
{
	...
	if ( PlayerIsCriminal() == false )
	{
		...
		if ( pPlayer && ( pPlayer->WorldSpaceCenter() - WorldSpaceCenter() ).LengthSqr() < (128*128) )
		{
			...
			AddLookTarget_E( pPlayer->BaseEntity(), 0.75f, 5.0f );

			if ( ( m_PolicingBehavior.IsEnabled() == false ) && ( m_nNumWarnings >= METROPOLICE_MAX_WARNINGS ) )
			{
				m_flBatonDebounceTime = gpGlobals->curtime + enginerandom->RandomFloat( 2.5f, 4.0f );
				SetTarget( pPlayer->BaseEntity() );
				SetBatonState( true );
			}
		}
		else
...
void CNPC_MetroPolice::Spawn( void )
{
	...
	{
		if ( !Weapon_OwnsThisType( WEAPON_SMG1_REPLACE_NAME ) )
		{
...
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::Weapon_Equip( CBaseEntity *pWeapon )
{
...
// Speaking
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::AnnounceEnemyType( CEntity *pEnemy )
{
	if ( !pEnemy || !GetSquad() )
		return;
	...
	if ( pSquad->IsLeader( this ) || ( pSquad->GetLeader() && pSquad->GetLeader()->GetEnemy() != GetEnemy() ) )
	{
		...
		switch ( pEnemy->Classify() )
		{
			case CLASS_PLAYER:
			{
				CPlayer *pPlayer = assert_cast<CPlayer*>( pEnemy );
	...
	{
		if ( pSquad->GetLeader() && FOkToMakeSound( SENTENCE_PRIORITY_MEDIUM ) )
		{
...
// Speaking
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::AnnounceEnemyKill( CEntity *pEnemy )
{
	if ( !pEnemy )
		return;
...
void CNPC_MetroPolice::AnnounceTakeCoverFromDanger( CSound *pSound )
{
	...
	if ( pSoundOwner )
	{
		...
		if ( pGrenade )
		{
			CEntity *cent = pGrenade->GetThrower();
			if ( IRelationType( (cent)?cent->BaseEntity():NULL ) != D_LI )
			{
...
bool CNPC_MetroPolice::IsEnemyInAnAirboat() const
{
	...
	CEntity *pVehicle = CEntity::Instance(static_cast<CPlayer*>( GetEnemy() )->GetVehicleEntity());
	if ( !pVehicle )
		return false;
...
// Returns the airboat
//-----------------------------------------------------------------------------
CEntity *CNPC_MetroPolice::GetEnemyAirboat() const
{
	// Should this be a condition??
	if ( !GetEnemy() || !GetEnemy()->IsPlayer() )
		return nullptr;

	return CEntity::Instance(static_cast<CPlayer*>( GetEnemy() )->GetVehicleEntity());
}
...
//-----------------------------------------------------------------------------
CEntity *CNPC_MetroPolice::GetShootTarget()
{
	// Should this be a condition??
	...
	CEntity *pVehicle = CEntity::Instance(static_cast<CPlayer*>( pEnemy )->GetVehicleEntity());
...
void CNPC_MetroPolice::OnUpdateShotRegulator( )
{
	...
	if( Weapon_OwnsThisType( WEAPON_PISTOL_REPLACE_NAME ) )
	{
...
void CNPC_MetroPolice::AimBurstRandomly( int nMinCount, int nMaxCount, float flMinDelay, float flMaxDelay )
{
	...
	// Choose a enginerandom direction vector perpendicular to the delta position
	Vector vecRight, vecUp;
	VectorVectors( vecDelta, vecRight, vecUp );
	float flAngle = enginerandom->RandomFloat( 0.0f, 2 * M_PI );
...
//-----------------------------------------------------------------------------
// Choose a enginerandom vector somewhere between the two specified vectors
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::RandomDirectionBetweenVectors( const Vector &vecStart, const Vector &vecEnd, Vector *pResult )
{
	...
	float flAngle = acos(flCosAngle) * enginerandom->RandomFloat( 0.0f, 1.0f );
...
{
	CEntity *pShootTarget = GetShootTarget();
...
void CNPC_MetroPolice::AimBurstTightGrouping( float flShotTime )
{
	...
	{
		m_nMaxBurstHits = enginerandom->RandomInt( nHitCount, nHitCount + 1 );
	}
	else
	{
		m_nMaxBurstHits = enginerandom->RandomInt( 2 * nHitCount - 1, 2 * nHitCount + 1 );
	}
...
int CNPC_MetroPolice::AimBurstAtSetupHitCount( float flDistToTarget, float flCurrentSpeed )
{
	...
	if ( IsEnemyInAnAirboat() )
	{
		...
		if (( nu < AIM_AT_SHOT_SPEED_COUNT - 1 ) && ( nv < AIM_AT_SHOT_DIST_COUNT - 1 ))
		{
			...
			m_nMaxBurstHits += enginerandom->RandomInt( nExtraShots, nExtraShots + 1 );
			return nExtraShots;
		}
	}
...
void CNPC_MetroPolice::AimBurstAtEnemy( float flReactionTime )
{
	...
	if ( bIsInVehicle )
	{
		...
		VectorMA( vecShootAt, enginerandom->RandomFloat( flMinAdd, 100.0f ), vecAcross, vecShootAt );
	}
...
void CNPC_MetroPolice::AimBurstInFrontOfEnemy( float flReactionTime )
{
	...
	m_nMaxBurstHits = enginerandom->RandomInt( 3, 4 );
	m_nBurstMode = BURST_LOCK_ON_AFTER_HIT;
	m_nBurstSteerMode = BURST_STEER_WITHIN_LINE_OF_DEATH;
	...
	// Now add in some extra vel in a enginerandom direction + try to prevent that....
...
void CNPC_MetroPolice::AimBurstAlongSideOfEnemy( float flFollowTime )
{
	...
	float flDist = AIM_ALONG_SIDE_LINE_OF_DEATH_DISTANCE + enginerandom->RandomFloat( 0.0f, 50.0f );
	...
	float flExtraDist = enginerandom->RandomFloat( 25.0f, 50.0f );
	VectorNormalize( vecTargetToGun );
...
void CNPC_MetroPolice::SteerBurstWithinLineOfDeath( )
{
	...
	else
	{
		...
		if (( t < -0.1f ) || ( t > 1.1f ))
		{
			...
			vecBurstDir *= enginerandom->RandomFloat( -flLength * 0.5f, flLength * 0.5f );
...
Vector CNPC_MetroPolice::ComputeBurstTrajectory( const Vector &shootOrigin )
{
	...
	vecNoise *= enginerandom->RandomFloat( -flNoise, flNoise );
	vecPos += vecNoise;
...
//-----------------------------------------------------------------------------
Vector CNPC_MetroPolice::AimCloseToTargetButMiss( CEntity *pTarget, const Vector &shootOrigin )
{
...
void CNPC_MetroPolice::FireBullets( const FireBulletsInfo_t &info )
{
	...
	if ( bIsPlayer && IsCurrentlyFiringBurst() )
	{
		...
		{
			actualInfo.m_pAdditionalIgnoreEnt = pEnemy->BaseEntity();
			BaseClass::FireBullets( actualInfo );
		}
	}
	else
...
{
	CEntity *pGoal = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, inputdata.value.String() );
...
void CNPC_MetroPolice::IdleSound( void )
{
	...
	switch( m_nIdleChatterType )
	{
		...
		case METROPOLICE_CHATTER_ASK_QUESTION:
		{
			...
			int nQuestionType = enginerandom->RandomInt( 0, METROPOLICE_CHATTER_RESPONSE_TYPE_COUNT );
...
Disposition_t CNPC_MetroPolice::IRelationType(CBaseEntity *pTarget)
{
	...
	if ( CEntity::Instance(pTarget)->Classify() == CLASS_PLAYER )
	{
...
void CNPC_MetroPolice::OnAnimEventDeployManhack( animevent_t *pEvent )
{
	...
	if ( pPhysObj )
	{
		Vector	yawOff = right * enginerandom->RandomFloat( -1.0f, 1.0f );
...
{
	CEntity *pHurt = CEntity::Instance(CheckTraceHullAttack_Float( 16, Vector(-16,-16,-16), Vector(16,16,16), 15, DMG_CLUB, 1.0f, false ));
	...
	if ( pHurt )
	{
		...
		// Play a enginerandom attack hit sound
...
void CNPC_MetroPolice::OnAnimEventBatonOn( void )
{
	...
	CNPCWeapon_StunStick *pStick = dynamic_cast<CNPCWeapon_StunStick *>(GetActiveWeapon());
...
{
	const char *eventName = GetEventName(pEvent->event);
...
//-----------------------------------------------------------------------------
bool CNPC_MetroPolice::HandleInteraction(int interactionType, void *data, CBaseEntity* sourceEnt)
{
	if ( interactionType == g_interactionMetrocopStartedStitch )
	{
		...
		m_flValidStitchTime = gpGlobals->curtime + enginerandom->RandomFloat( METROPOLICE_SQUAD_STITCH_MIN_INTERVAL, METROPOLICE_SQUAD_STITCH_MAX_INTERVAL );
		return true;
	}
	...
	if ( interactionType == (*g_interactionHitByPlayerThrownPhysObj) )
	{
		...
		CEntity *pProp = CEntity::Instance((CBaseEntity*)data);
		if( pProp != NULL )
		{
			if( pProp->NameMatches("cupcop_can") )
				m_OnCupCopped.FireOutput( BaseEntity(), (CBaseEntity *)NULL );
		}
...
void CNPC_MetroPolice::ReleaseManhack( void )
{
	...
	if ( GetEnemy() )
	{
		m_hManhack->SetEnemy( GetEnemy_CBase() );
		m_hManhack->SetState( NPC_STATE_COMBAT );

		m_hManhack->UpdateEnemyMemory( GetEnemy_CBase(), GetEnemy()->GetAbsOrigin() );
	}

	// Place him into our squad so we can communicate
	if ( GetSquad() )
	{
		GetSquad()->AddToSquad( m_hManhack );
	}
}
...
int CNPC_MetroPolice::SelectScheduleInvestigateSound()
{
	...
	if ( PlayerIsCriminal() && !HasCondition( COND_SEE_ENEMY ) )
	{
		...
		{
			if ( GetSquad() && OccupyStrategySlot( SQUAD_SLOT_INVESTIGATE_SOUND ) )
			{
...
bool CNPC_MetroPolice::OnObstructionPreSteer( AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult )
{
	if ( pMoveGoal->directTrace.pObstruction )
	{
		...
		CE_CPhysicsProp *pProp = dynamic_cast<CE_CPhysicsProp*>( CEntity::Instance(pMoveGoal->directTrace.pObstruction) );
		if ( pProp && pProp->GetHealth() )
		{
			m_hBlockingProp.Set(pProp->BaseEntity());
		}
		else
		{
			m_hBlockingProp.Set(NULL);
		}
	}
...
int CNPC_MetroPolice::SelectScheduleNoDirectEnemy()
{
	...
	{
		SetTarget( m_hBlockingProp->BaseEntity() );
		m_hBlockingProp.Set(NULL);
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_MetroPolice::ShouldKnockOutTarget( CEntity *pTarget )
{
...
// Purpose: This is a bridge between stunstick, NPC and its behavior
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::KnockOutTarget( CEntity *pTarget )
{
	if ( m_PolicingBehavior.IsEnabled() )
	{
...
bool CNPC_MetroPolice::CanEnemySeeMe( )
{
	if ( GetEnemy()->IsPlayer() )
	{
		if ( static_cast<CPlayer*>(GetEnemy())->FInViewCone_Entity( BaseEntity() ) )
		{
			return true;
		}
	}
...
int CNPC_MetroPolice::SelectStitchSchedule()
{
	...
	int i;
	float flRand = enginerandom->RandomFloat( 0.0f, flWeight[STITCH_SCHEDULE_COUNT - 1] );
...
// TraceAttack
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
...
void CNPC_MetroPolice::AnnounceHarrassment( void )
{
	...
	m_Sentences.Speak( pWarnings[ enginerandom->RandomInt( 0, ARRAYSIZE(pWarnings)-1 ) ], SENTENCE_PRIORITY_MEDIUM, SENTENCE_CRITERIA_NORMAL );
}
...
void CNPC_MetroPolice::IncrementPlayerCriminalStatus( void )
{
	...
	if ( pPlayer )
	{
		AddLookTarget_E( pPlayer->BaseEntity(), 0.8f, 5.0f );
	...
	m_flBatonDebounceTime = gpGlobals->curtime + enginerandom->RandomFloat( 2.0f, 4.0f );
...
void CNPC_MetroPolice::AdministerJustice( void )
{
	...
	if ( !IsInAScript() && (m_NPCState != NPC_STATE_SCRIPT) && HasSpawnFlags( SF_METROPOLICE_ALLOWED_TO_RESPOND ) )
	{
		...
		if(pPlayer)
		{
			...
			UpdateEnemyMemory( pPlayer->BaseEntity(), pPlayer->GetAbsOrigin() );
		}
	}
	else
	{
	...
	if ( !BehaviorSelectSchedule() )
	{
		...
		switch( m_NPCState )
		{
			...
			case NPC_STATE_COMBAT:
				if (!IsEnemyInAnAirboat() || !Weapon_OwnsThisType( WEAPON_SMG1_REPLACE_NAME ) )
				{
...
void CNPC_MetroPolice::EnemyResistingArrest()
{
	...
	{
		AISquadIter_t iter;
		CAI_NPC *pSquadmate = (CAI_NPC *)CEntity::Instance(squad->GetFirstMember( &iter ));
		while ( pSquadmate )
		{
			pSquadmate->RemoveSpawnFlags( SF_METROPOLICE_ARREST_ENEMY );
			pSquadmate->SetCondition( COND_METROPOLICE_ENEMY_RESISTING_ARREST );
			pSquadmate = (CAI_NPC *)CEntity::Instance(squad->GetNextMember( &iter ));
		}
	}
}
	...
	switch( pTask->iTask )
	{
		...
		case TASK_METROPOLICE_ARREST_ENEMY:
		{
			...
			if ( pTask->iTask == TASK_METROPOLICE_LEAD_ARREST_ENEMY )
			{
				...
				if ( nArrestCount == GetSquad()->NumMembers() )
				{
					TaskComplete();
					break;
				}
...
int CNPC_MetroPolice::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if (info.GetAttacker() == GetEnemy_CBase())
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
WeaponProficiency_t CNPC_MetroPolice::CalcWeaponProficiency( CBaseEntity *pWeapon )
{
	CEntity *cent_pWeapon = CEntity::Instance(pWeapon);
	if( FClassnameIs( cent_pWeapon, WEAPON_PISTOL_REPLACE_NAME ) )
	{
		return WEAPON_PROFICIENCY_POOR;
	}

	if( FClassnameIs( cent_pWeapon, WEAPON_SMG1_REPLACE_NAME ) )
	{
...
void CNPC_MetroPolice::GatherConditions( void )
{
	...
	else if ( (distToPlayerSqr < (42.0f*42.0f) && FVisible_Entity(pPlayer->BaseEntity())) )
	{
...
bool CNPC_MetroPolice::HasBaton( void )
{
	...
	if ( pWeapon )
		return FClassnameIs( pWeapon, WEAPON_STUNSTICK_REPLACE_NAME );
...
void CNPC_MetroPolice::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	CEntity *pHitEntity = CEntity::Instance(pEvent->pEntities[otherIndex]);

	if ( pEvent->pObjects[otherIndex]->GetGameFlags() & FVPHYSICS_PLAYER_HELD )
	{
		CPlayer *pPlayer = UTIL_GetNearestPlayer(pHitEntity->GetAbsOrigin());
...
// Input  : *pTarget -
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::StunnedTarget( CEntity *pTarget )
{
