...
public:
	...
	CBaseEntity *CheckTraceHullAttack_Float( float flDist, const Vector &mins, const Vector &maxs, int iDamage, int iDmgType, float forceScale, bool bDamageAnyNPC );
	CBaseEntity *CheckTraceHullAttack_Vector( const Vector &vStart, const Vector &vEnd, const Vector &mins, const Vector &maxs, int iDamage, int iDmgType, float flForceScale, bool bDamageAnyNPC );
	...
	bool	ShouldKnockOutTarget( CEntity *pTarget );
	void	KnockOutTarget( CEntity *pTarget );
	void	StunnedTarget( CEntity *pTarget );
...
private:
	...
	Vector AimCloseToTargetButMiss( CEntity *pTarget, const Vector &shootOrigin );
...
private:
	...
	CEFakeHandle<CAI_NPC>			m_hManhack;
	CEFakeHandle<CE_CPhysicsProp>	m_hBlockingProp;
