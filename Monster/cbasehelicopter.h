...
{
public:
	CE_DECLARE_CLASS( CBaseHelicopter, CAI_TrackPather );
	...
	virtual void FlyTouch( CEntity *pOther );
	virtual void CrashTouch( CEntity *pOther );
	...
	virtual CEntity *GetTrackPatherTargetEnt();
...
protected:
	...
	void			DoWashPushOnAirboat( CEntity *pAirboat, const Vector &vecWashToAirboat, float flWashAmount );
...
protected:
	...
	CFakeHandle		m_hRotorWash;	// Attached rotorwash entity
...
// should avoid. 
//-----------------------------------------------------------------------------
class CAvoidSphere : public CE_InfoTarget_Fix
{
	DECLARE_DATADESC();

public:
	CE_DECLARE_CLASS( CAvoidSphere, CE_InfoTarget_Fix );

	void Init( float flRadius );
	virtual void Activate();
	virtual void UpdateOnRemove();

	static void ComputeAvoidanceForces( CEntity *pEntity, float flEntityRadius, float flAvoidTime, Vector *pVecAvoidForce );

private:
	typedef CFakeHandle AvoidSphereHandle_t;
...
// should avoid. 
//-----------------------------------------------------------------------------
class CAvoidBox : public CE_InfoTarget_Fix
{
	DECLARE_DATADESC();

public:
	CE_DECLARE_CLASS( CAvoidBox, CE_InfoTarget_Fix );
...
private:
	typedef CFakeHandle AvoidBoxHandle_t;	
