...
int CNPC_Strider::gm_MiniGunAttachment;
...
void CNPC_Strider::PostConstructor()
{
...
void CNPC_Strider::Spawn()
{
	...
	MoveToGround( &origin, BaseEntity(), mins, maxs );
...
void CNPC_Strider::SetupGlobalModelData()
{
	...
	gm_MiniGunAttachment = LookupAttachment( "MiniGun" );
...
void CNPC_Strider::CalculateIKLocks( float currentTime )
{
	...
	if ( temp_m_pIk && temp_m_pIk->m_target.Count() )
	{
		...
		{
			target = temp_m_pIk->m_target[i];

			if (!target.IsActive())
				continue;

			m_vecIKTarget[i] = target.est.pos;
		}
	}
}
...
void CNPC_Strider::PrescheduleThink()
{
	...
	if( GetHealth() <= 50 && enginerandom->RandomInt( 0, 20 ) == 0 )
	{
		CEntity *pTrail = CreateEntityByName( "sparktrail" );
...
void CNPC_Strider::GatherConditions()
{
	...
	if( IsUsingAggressiveBehavior() )
	{
		if( GetEnemy() )
		{
			...
			{
				// Keep setting up to play my hunt sound at some enginerandom time after losing sight of my enemy.
				m_flTimeNextHuntSound = gpGlobals->curtime + 1.0f;
			}
			else
			{
	...
	if( !m_bCrouchLocked && !m_bDontCrouch )
	{
		if( m_hCannonTarget != NULL )
		{
			if( !IsStriderCrouching() && !IsStriderStanding() )
			{
				...
				{
					GatherHeightConditions( GetAdjustedOrigin(), m_hCannonTarget->BaseEntity() );
				}
			}
		}
		else if( GetEnemy() )
		{
				...
				if( GetEnemy() && m_pMinigun->IsShooting() && GetTimeEnemyAcquired() != gpGlobals->curtime )
				{
					if( m_pMinigun->IsOnTarget( 3 ) && !FClassnameIs( GetEnemy(), "npc_bullseye" ) )
					{
						if( m_iVisibleEnemies > 1 )
						{
							...
							GetEnemies()->SetTimeValidEnemy( GetEnemy_CBase(), gpGlobals->curtime + ( STRIDER_IGNORE_TARGET_DURATION * m_iVisibleEnemies ) );
							SetEnemy( NULL, false );
							ChooseEnemy();
						}
			...
			if ( GetEnemy() ) // Can go null above
			{
				...
				{
					GatherHeightConditions( GetAdjustedOrigin(), GetEnemy_CBase() );
				}
			}
		}
		else
...
void CNPC_Strider::GatherHeightConditions( const Vector &vTestPos, CBaseEntity *pEntity )
{
	...
	if( FInViewCone_Entity( pEntity ) && GetWeaponLosZ( vTestPos, minZ, maxZ, GetHeightRange() * .1, cent, &newHeight ) )
	{
		...
		if ( m_LowZCorrectionTimer.Expired() )
		{
			...
			{
				Vector muzzlePos;
				Vector targetPos = cent->BodyTarget( GetAdjustedOrigin() );

				GetAttachment( "minigun", muzzlePos );

				trace_t tr;
				UTIL_TraceLine( muzzlePos, targetPos, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr);

				if ( ( tr.m_pEnt != pEntity) && tr.fraction != 1.0 && !CanShootThrough( tr, targetPos ) )
				{
					if ( !GetWeaponLosZ( vTestPos, minZ + GetHeightRange() * .33, maxZ, GetHeightRange() * .1, cent, &newHeight ) )
					{
						return;
					}
					newHeight = GetMaxHeightModel() - ( GetAbsOrigin().z - newHeight);
		...
		if ( !strider_always_use_procedural_height.GetBool() )
		{
			...
			{
				if ( enginerandom->RandomInt(1, 10 ) <= 6 )
				{
	...
	switch ( pTask->iTask )
	{
		...
		case TASK_STRIDER_REFRESH_HUNT_PATH:
			...
			if ( GetGoalEnt() )
			{
				...
				TranslateNavGoal( GetGoalEnt()->BaseEntity(), goal.dest );
		...
		case TASK_STRIDER_GET_PATH_TO_CANNON_TARGET:
		{
			...
			TranslateNavGoal( m_hCannonTarget->BaseEntity(), goal.dest );
		...
		case TASK_STRIDER_BREAKDOWN:
			if ( IsActivityFinished() )
			{
				...
				if ( !IsMarkedForDeletion() )
				{
					...
					g_helpfunc.CreateServerRagdoll( BaseEntity(), 0, info, COLLISION_GROUP_NONE );
					TaskComplete();
					UTIL_Remove(this);
				}
			}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Strider::HandleInteraction( int interactionType, void *data, CBaseEntity* sourceEnt )
{
	if ( interactionType == g_interactionPlayerLaunchedRPG )
	{
		m_flTimePlayerMissileDetected = gpGlobals->curtime;
		m_hPlayersMissile.Set(sourceEnt);
		return true;
	}
...
void CNPC_Strider::HandleAnimEvent( animevent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		case STRIDER_AE_CANNONHIT:
			CreateConcussiveBlast( m_blastHit, m_blastNormal, BaseEntity(), 2.5 );
			break;

		case STRIDER_AE_SHOOTMINIGUN:
		{
			CEntity *pTarget = g_helpfunc.FindEntityGeneric( (CBaseEntity *)NULL, pEvent->options, BaseEntity(), BaseEntity() );
			if ( pTarget )
			{
				Vector vecTarget = pTarget->CollisionProp_Actual()->WorldSpaceCenter();
...
{
	if ( IsCannonTarget( CEntity::Instance(pTarget) ) )
		return D_HT;
...
{
	CEntity *cent = CEntity::Instance(pEntity);
	if ( nDisposition ==  D_HT && cent->ClassMatches("npc_bullseye") )
		UpdateEnemyMemory( pEntity, cent->GetAbsOrigin() );
...
{
	CEntity *cent = CEntity::Instance(pPathStart);
	m_strTrackName = cent->GetEntityName_String();
	if ( IsCurSchedule( SCHED_STRIDER_RANGE_ATTACK2, false ) )
	{
...
{
	CEntity *pTargetEntity = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, inputdata.value.String(), NULL, inputdata.pActivator, inputdata.pCaller );
...
void CNPC_Strider::InputSetCannonTarget( inputdata_t &inputdata )
{
	...
	if ( pTarget )
	{
		...
		{
			m_hCannonTarget.Set(pTarget->BaseEntity());
			m_AttemptCannonLOSTimer.Force();
			return;
		}
	}

	m_hCannonTarget.Set(NULL);
}
...
void CNPC_Strider::InputExplode( inputdata_t &inputdata )
{
	...
	killInfo.SetAttacker( BaseEntity() );
	killInfo.SetInflictor( BaseEntity() );
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::FVisible_Entity( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	bool bIsVisible = BaseClass::FVisible_Entity( pEntity, traceMask, ppBlocker );
	...
	if ( bIsVisible && cent == m_PlayerFreePass.GetPassTarget() )
	{
		...
		if( !bIsVisible && IsUsingAggressiveBehavior() && FInViewCone_Entity(pEntity) )
		{
			AlertSound();
		}
	}
...
bool CNPC_Strider::IsValidEnemy( CBaseEntity *pTarget )
{
	...
	CCombatCharacter *pEnemy = ToBaseCombatCharacter( cent );
	if ( pEnemy )
	{
		CE_CBaseFilter *filter = m_hEnemyFilter.ptr->Get();
		// Test our enemy filter
		if ( filter != NULL && filter->PassesFilter( BaseEntity(), pEnemy->BaseEntity() ) == false )
			return false;
...
{
	CEntity *cent = CEntity::Instance(pInformer);
	if( cent && FClassnameIs( cent, "npc_cscanner" ) )
	{
		...
		AddEntityRelationship( GetFocus()->BaseEntity(), D_HT, -1 );
		...
		BaseClass::UpdateEnemyMemory( GetFocus()->BaseEntity(), GetFocus()->GetAbsOrigin(), pInformer );
...
//---------------------------------------------------------
float CNPC_Strider::StriderEnemyDistance( CEntity *pEnemy )
{
	...
	CollisionProp_Actual()->WorldSpaceSurroundingBounds( &striderSurroundMins, &striderSurroundMaxs );
...
bool CNPC_Strider::FCanCheckAttacks()
{
	...
	if( FInViewCone_Entity( GetEnemy_CBase() ) )
	{
		return true;
	}
...
int CNPC_Strider::MeleeAttack1Conditions( float flDot, float flDist )
{
	...
	MoveToGround( &vecGround, BaseEntity(), Vector( -16, -16, -16 ), Vector( 16, 16, 16 ) );
...
bool CNPC_Strider::WeaponLOSCondition(const Vector &ownerPos, const Vector &targetPos, bool bSetConditions)
{
	...
	trace_t tr;
	UTIL_TraceLine( ownerPos + vRootOffset, targetPos, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr);

	// Hit the enemy, or hit nothing (traced all the way to a nonsolid enemy like a bullseye)
	if ( ( pTargetEnt && tr.m_pEnt == pTargetEnt->BaseEntity()) || tr.fraction == 1.0 || CanShootThrough( tr, targetPos ) )
	{
		Vector vBarrelPos;
		...
		UTIL_TraceLine( vBarrelPos, targetPos, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr);
		if ( ( pTargetEnt && tr.m_pEnt == pTargetEnt->BaseEntity()) || tr.fraction == 1.0 || CanShootThrough( tr, targetPos ) )
		{
			return true;
		}
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::IsValidShootPosition( const Vector &vecCoverLocation, CAI_Node *pNode, CBaseEntity const *pHint )
{
	CE_AI_Hint *cent = (CE_AI_Hint *)CEntity::Instance(pHint);
	if ( ( pNode && !cent ) || ( cent && cent->HintType() != HINT_STRIDER_NODE ) )
		return false;
...
bool CNPC_Strider::TestShootPosition(const Vector &vecShootPos, const Vector &targetPos )
{
	...
	if ( zDelta > 0 )
	{
		...
		BaseClass::m_bInTestShootPosition = false;
...
void CNPC_Strider::MakeTracer( const Vector &vecTracerSrc, const trace_t &tr, int iTracerType )
{
	...
	UTIL_Tracer( vecTracerSrc, tr.endpos, entindex(), gm_MiniGunAttachment, 5000, false, "StriderTracer" );
}
...
void CNPC_Strider::DoImpactEffect( trace_t &tr, int nDamageType )
{
	...
	if( cent && !cent->MyNPCPointer() )
	{
		...
		{
			UTIL_TraceLine( vecReTrace, vecReTrace - vecDir * 24, MASK_SHOT, NULL, COLLISION_GROUP_NONE, &retrace );

			BaseClass::m_bInDoImpactEffect = false;
...
bool CNPC_Strider::CanShootThrough( const trace_t &tr, const Vector &vecTarget )
{
	...
	UTIL_TraceLine( tr.endpos, vecTarget, MASK_SHOT, tr.m_pEnt, COLLISION_GROUP_NONE, &continuedTrace );

	if( continuedTrace.fraction != 1.0 )
	{
		if( continuedTrace.m_pEnt != GetEnemy_CBase() )
		{
			return false;
		}
	}
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::GetWeaponLosZ( const Vector &vOrigin, float minZ, float maxZ, float increment, CEntity *pTarget, float *pResult )
{
	Vector vTestPos;
...
void CNPC_Strider::HuntSound()
{
	if( m_PlayerFreePass.HasPass() )
	{
		...
		m_flTimeNextHuntSound = gpGlobals->curtime + enginerandom->RandomFloat( 8.0f, 12.0f );
	}
}

//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Strider::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
	...
	for ( int i = 0; i < m_hAttachedBusters.Count(); i++ )
	{
		...
		CEntity *cent = CEntity::Instance(m_hAttachedBusters[i]);
		flDistSqr = ( cent->WorldSpaceCenter() - inputInfo.GetDamagePosition() ).LengthSqr();
...
int CNPC_Strider::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( inflictor && inflictor->GetOwnerEntity() == this )
		return 0;

	// special interaction with combine balls
	if ( UTIL_IsCombineBall( inflictor ) )
	...
	if ( (info.GetDamageType() & DMG_BLAST) && info.GetMaxDamage() > 50 )
	{
		...
		if ( dist < 200 )
		{
			bool bPlayer = attacker->IsPlayer();
			if ( bPlayer )
			{
				m_PlayerFreePass.Revoke();
			...
			m_OnDamaged->FireOutput( info.GetAttacker(), BaseEntity());

			if( attacker->IsPlayer() )
			{
				m_OnDamagedByPlayer->FireOutput( info.GetAttacker(), BaseEntity() );

				// This also counts as being harmed by player's squad.
				m_OnDamagedByPlayerSquad->FireOutput( info.GetAttacker(), BaseEntity() );
			}
			else
			{
			...
			{
				m_OnHalfHealth->FireOutput(BaseEntity(), BaseEntity());
			}
...
void CNPC_Strider::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( info.GetInflictor() && UTIL_IsCombineBall( inflictor ) )
	{
		if ( m_lifeState == LIFE_DYING )
			return;
	...
	if( IsUsingAggressiveBehavior() )
	{
		...
		ExplosionCreate( vecExplode, QAngle(0,0,1), BaseEntity(), 100, 128, false );
	}
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Strider::RagdollDeathEffect( CE_CRagdollProp *pRagdoll, float flDuration )
{
	...
	for( i = 0 ; i < 2 ; i++ )
	{
		CSmokeTrail *pSmoke = CSmokeTrail::CreateSmokeTrail();
		if ( pSmoke )
		{
			if( i == 0 )
			{
...
bool CNPC_Strider::ShouldExplodeFromDamage( const CTakeDamageInfo &info )
{
	...
	if ( UTIL_IsCombineBall( pInflictor ) )
		return true;
...
bool CNPC_Strider::BecomeRagdoll( const CTakeDamageInfo &info, const Vector &forceVector )
{
	...
	else
	{
		...
		if ( pPlayer && mat_dxlevel->GetInt() > 0 )
		{
			...
			if ( maxRagdolls > 0 )
			{
				CUtlVectorFixed<CE_CRagdollProp *, 2> striderRagdolls;

				while ( ( pRagdoll = CEntity::NextEntByClass( pRagdoll ) ) != NULL )
				{
				...
				if ( striderRagdolls.Count() >= maxRagdolls )
				{
					...
					CE_CRagdollProp *pFurthest = NULL;
			...
			pRagdoll = assert_cast<CE_CRagdollProp *>( g_helpfunc.CreateServerRagdoll( BaseEntity(), m_nForceBone, info, HL2COLLISION_GROUP_STRIDER ) );
...
void CNPC_Strider::StartSmoking( void )
{
	...
	CSmokeTrail *smoke = CSmokeTrail::CreateSmokeTrail();

	if ( smoke )
	{
		m_hSmoke.Set(smoke->BaseEntity());

		smoke->m_SpawnRate			= 32;
		smoke->m_ParticleLifetime	= 3.0;
		smoke->m_StartSize			= 16;
		smoke->m_EndSize			= 64;
		smoke->m_SpawnRadius		= 20;
		smoke->m_MinSpeed			= 8;
		smoke->m_MaxSpeed			= 64;
		smoke->m_Opacity 			= 0.3;

		smoke->m_StartColor->Init( 0.25f, 0.25f, 0.25f );
		smoke->m_EndColor->Init( 0, 0, 0 );
		smoke->SetLifetime( 500.0f );
		smoke->FollowEntity( this, "MiniGunBase" );
	}
}
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::IsUnusableNode(int iNodeID, CBaseEntity *pHint)
{
	if ( !BaseClass::IsUnusableNode(iNodeID, pHint) )
	{
		CE_AI_Hint *cent = (CE_AI_Hint *)CEntity::Instance(pHint);
		if ( cent && cent->HintType() == HINT_STRIDER_NODE )
			return false;
	}
	return true;
}
...
{
	CEntity *cent = (CEntity *)CEntity::Instance(pEnemy);
	if ( cent )
	{
		if ( ! (cent->GetFlags() & FL_ONGROUND) )
		{
			MoveToGround( &chasePosition, pEnemy, cent->WorldAlignMins(), cent->WorldAlignMaxs() );
		}

		// move down to enemy's feet for enemy origin at chasePosition
		chasePosition.z += cent->WorldAlignMins().z;
...
{
	SetGoalEnt( NULL );
	CEntity *pGoalEnt = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_strTrackName );
	if ( pGoalEnt == NULL )
	{
	...
	CEntity *pClosestEnt = NULL;
	CEntity *pCurEnt = pGoalEnt;
	float distClosestSq = FLT_MAX;

	CUtlRBTree<CEntity *> visits;
	...
	ScheduledFollowPath( SCHED_IDLE_WALK, (pClosestEnt)?pClosestEnt->BaseEntity():NULL, ACT_WALK );
}
...
void CNPC_Strider::DoMuzzleFlash( void )
{
	...
	g_helpfunc.DispatchEffect( "StriderMuzzleFlash", data );*/
}
...
void CNPC_Strider::ShootMinigun( const Vector *pTarget, float aimError, const Vector &vecSpread )
{
	if ( pTarget )
	{
		...
		if( m_bMinigunUseDirectFire )
		{
			...
			CAI_NPC::FireBullets( 1, muzzlePos, vecShootDir, vecSpread, 8192, m_miniGunDirectAmmo, 1 );
		}
		else
		{
			// exactly on target w/tracer
			CAI_NPC::FireBullets( 1, muzzlePos, vecShootDir, vecSpread, 8192, m_miniGunAmmo, 1);
		}

		g_pEffects->MuzzleFlash( muzzlePos, muzzleAng, enginerandom->RandomFloat( 2.0f, 4.0f ) , MUZZLEFLASH_TYPE_STRIDER );
		DoMuzzleFlash();

		EmitSound( "NPC_Strider.FireMinigun" );

		//g_pEffects->MuzzleFlash(muzzlePos ,muzzleAng, 1.0f, 2);
	}
}
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Strider::OnMinigunStopShooting( CEntity *pTarget )
{
	// Stop hating the focus.
	if( GetFocus() && pTarget == GetFocus() )
	{
		AddEntityRelationship( GetFocus()->BaseEntity(), D_NU, 0 );
	}
...
float CNPC_Strider::GetMinigunOnTargetTime()
{
	if( IsUsingAggressiveBehavior() )
	{
		if( m_bMinigunUseDirectFire )
		{
			...
			// will be clipped, since shooting duration is enginerandom and
...
//---------------------------------------------------------
//---------------------------------------------------------
CEntity *CNPC_Strider::GetCannonTarget()
{
	CEntity *pTarget = m_hCannonTarget;
	return pTarget;
}
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::IsCannonTarget( CEntity *pTarget ) const
{
	CEntity *pCannonTarget = m_hCannonTarget;
...
//---------------------------------------------------------
bool CNPC_Strider::AimCannonAt( CEntity *pEntity, float flInterval )
{
	if ( !pEntity )
		return true;
...
void CNPC_Strider::FireCannon()
{
	...
	UTIL_TraceLine( vecShootPos, vecShootPos + vecShootDir * flDist, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	...
	Vector pos = m_hCannonTarget->GetAbsOrigin();
	CreateFireBeam(vecShootPos, pos);
...
void CNPC_Strider::VPhysicsShadowCollision( int index, gamevcollisionevent_t *pEvent )
{
	if ( !HasMemory( bits_MEMORY_PROVOKED ) )
	{
		...
		CBaseEntity *pPlayer = CEntity::Instance(pEvent->pEntities[!index])->HasPhysicsAttacker( 1 );
		if ( pPlayer )
		{
	...
	CEntity *cent = CEntity::Instance(pOther);
	if ( pOther && UTIL_IsCombineBall( cent ) )
	{
		Vector damagePos;
		...
		CBaseEntity *pPlayer = cent->HasPhysicsAttacker( 1.0 );
		if (pPlayer)
		{
		...
		m_nForceBone = 0;
		PhysCallbackDamage( BaseEntity(), dmgInfo, *pEvent, index );
		return;
	}
...
void CNPC_Strider::StompHit( int followerBoneIndex )
{
	...
	CEntity *pEnemy = GetEnemy();
	CAI_NPC *pNPC = pEnemy ? pEnemy->MyNPCPointer() : NULL;
	...
	CTakeDamageInfo damageInfo( BaseEntity(), BaseEntity(), 500, DMG_CRUSH );
	Vector forward;
	...
	CBaseEntity *pRagdollBase = g_helpfunc.CreateServerRagdollAttached( (CBaseAnimating*)pNPC->BaseEntity(), vec3_origin, -1, COLLISION_GROUP_DEBRIS, pLegPhys, (CBaseAnimating*)BaseEntity(), bone->boneIndex, vecStabPos, -1, localHit );
	CEntity *pRagdoll = CEntity::Instance(pRagdollBase);
	if ( pRagdoll )
	{
		// the strider might drag this through the world
		pRagdoll->AddSolidFlags( FSOLID_NOT_SOLID );

		m_hRagdoll.Set(pRagdoll->BaseEntity());
...
{
	trace_t tr;
	UTIL_TraceLine( origin + Vector(0, 0, 48), origin - Vector(0,0,100), MASK_SOLID_BRUSHONLY, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	float yaw = enginerandom->RandomInt(0,120);
	...
	if ( UTIL_PointContents( tr.endpos + Vector( 0, 0, 1 ) ) & MASK_WATER )
	{
		...
		data.m_flScale = enginerandom->RandomFloat( 10.0, 14.0 );
	...
	{
		ShakeRopes( tr.endpos, npc_strider_shake_ropes_radius.GetFloat(), npc_strider_shake_ropes_magnitude.GetFloat() );
	}
	...
	//
	g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER|SOUND_CONTEXT_EXCLUDE_COMBINE, tr.endpos, 512, 1.0f, BaseEntity() );
}

//---------------------------------------------------------
//---------------------------------------------------------
Vector CNPC_Strider::CalculateStompHitPosition( CEntity *pEnemy )
{
...
static void MoveToGround( Vector *position, CBaseEntity *ignore, const Vector &mins, const Vector &maxs )
{
	...
	floor.z -= 1024;
	UTIL_TraceHull( *position, floor, mins, maxs, MASK_NPCSOLID_BRUSHONLY, ignore, COLLISION_GROUP_NONE, &tr );
	if ( tr.fraction < 1 )
	{
...
void AdjustStriderNodePosition( CAI_Network *pNetwork, CAI_Node *pNode )
{
	...
	{
		CNPC_Strider *pStrider = (CNPC_Strider *)g_helpfunc.FindEntityByClassname( (CBaseEntity *)NULL, "npc_strider" );
		bool bCreated = false;
		if ( !pStrider )
		{
...
void CNPC_Strider::CNavigator::MoveCalcBaseGoal( AILocalMoveGoal_t *pMoveGoal )
{
	...
	if ( heightAdj < -1 )
	{
		heightAdj = clamp( heightAdj, -192.0f, 0.0f );
...
{
	DevMsg( 2, "Strider overriding DoFindPathToPos\n");
	if ( BaseClass::DoFindPathToPos() )
	{
		// Do special optimization on our first path segment
		DevMsg( 2, "Strider base pathfind worked\n");
		...
		if ( pLast->iNodeID == NO_NODE )
		{
			...
			if ( pLastNodeWaypoint )
			{
				...
				if ( pLastNodeWaypoint->iNodeID != NO_NODE )
				{
					...
					{
						DevMsg( 2, "Strider resetting goal to last node waypoint\n");
					...
					{
						DevMsg( 2, "Strider resetting goal to nearest point on graph\n");
						pLast->vecLocation = vNewEnd;
					}
					else
					{
						DevMsg( 2, "Strider resetting goal to nearest point on graph, on node\n");
			...
			DevMsg( 2, "Goal ended on node\n");
		return true;
	}
	else
		DevMsg( 2, "Strider base pathfind failed\n");
...
//-----------------------------------------------------------------------------
bool CNPC_Strider::IsLegBoneFollower( CE_CBoneFollower *pFollower )
{
...
//-----------------------------------------------------------------------------
CE_CBoneFollower *CNPC_Strider::GetBoneFollowerByIndex( int nIndex )
{
...
//-----------------------------------------------------------------------------
int CNPC_Strider::GetBoneFollowerIndex( CE_CBoneFollower *pFollower )
{
...
{
	if ( *(CEntity::Instance(pEntity)->m_iClassname) == m_iszStriderBusterName )
		return false;
...
//-----------------------------------------------------------------------------
void CNPC_Strider::StriderBusterAttached( CEntity *pAttached )
{
	// Add another to the list
	m_hAttachedBusters.AddToTail( (pAttached)?pAttached->BaseEntity():NULL );
	m_PlayerFreePass.Revoke();

	variant_t target;
	target.SetString( AllocPooledString( "!player" ) );
	g_CEventQueue->AddEvent( BaseEntity(), "UpdateEnemyMemory", target, 1.0, BaseEntity(), BaseEntity() );
}


void CNPC_Strider::StriderBusterDetached( CEntity *pAttached )
{
	int elem = m_hAttachedBusters.Find((pAttached)?pAttached->BaseEntity():NULL );
	if (elem >= 0)
	{
...
void CNPC_Strider::CreateFireSprite(const Vector &position)
{
	CE_CSprite *sprite = CE_CSprite::SpriteCreate( "effects/strider_pinch_dudv.vmt", position, FALSE );
	if(sprite)
	{
		sprite->SetAttachment( BaseEntity(), gm_CannonAttachment );
		sprite->SetTransparency( kRenderGlow, 255, 255, 255, 0, kRenderFxNoDissipation );
		sprite->SetBrightness( 255 );
		sprite->SetScale(0.25f);
		m_hFireSprite[0].Set(sprite->BaseEntity());
	}

	sprite = CE_CSprite::SpriteCreate( "effects/blueblackflash.vmt", position, FALSE );
	if(sprite)
	{
		sprite->SetTransparency( kRenderGlow, 255, 255, 255, 0, kRenderFxNoDissipation );
		sprite->SetAttachment( BaseEntity(), gm_CannonAttachment );
		...
		m_hFireSprite[1].Set(sprite->BaseEntity());
	}
}

void CNPC_Strider::ClearFireSprite()
{
	if(m_hFireSprite[0])
	{
		UTIL_Remove(m_hFireSprite[0]);
		m_hFireSprite[0].Set(NULL);
	}

	if(m_hFireSprite[1])
	{
		UTIL_Remove(m_hFireSprite[1]);
		m_hFireSprite[1].Set(NULL);
	}
}

void CNPC_Strider::CreateFireBeam(const Vector &startpos, const Vector &endpos)
{
	CE_CBeam *beam = CE_CBeam::BeamCreate("effects/blueblacklargebeam.vmt", 4.0f);
	if(beam)
	{
		beam->SetStartPos(startpos);
		beam->SetEndPos(endpos);
		beam->RelinkBeam();
		beam->LiveForTime(1.5f);
	}
}
...
{
	matrix3x4_t gunMatrix;
	CAI_NPC *pOwner = pHost->GetEntity();
...
//---------------------------------------------------------
//---------------------------------------------------------
void CStriderMinigun::AimAtTarget( IStriderMinigunHost *pHost, CEntity *pTarget, bool bSnap )
{
	if ( pTarget )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CStriderMinigun::ShootAtTarget( IStriderMinigunHost *pHost, CEntity *pTarget, float shootTime )
{
	if ( !pTarget )
		return;

	variant_t emptyVariant;
	pTarget->CustomAcceptInput( "InputTargeted", pHost->GetEntity()->BaseEntity(), pHost->GetEntity()->BaseEntity(), emptyVariant, 0 );
	Enable( NULL, true );
	if ( shootTime <= 0 )
	{
		shootTime = enginerandom->RandomFloat( 4, 8 );
	}
...
//---------------------------------------------------------
//---------------------------------------------------------
void CStriderMinigun::StartShooting( IStriderMinigunHost *pHost, CEntity *pTarget, float duration )
{
	...
	if( pTarget->IsPlayer() )
	{
		...
		if( !pPlayer->FInViewCone_Entity( pHost->GetEntity()->BaseEntity() ) )
		{
			...
			else
			{
				...
				// in the back!
				m_vecAnchor += Vector( 0, 0, enginerandom->RandomFloat( 160, 240 ) );

				// Move it to one side of the other randomly, just to get it off center.
				Vector right;
				pPlayer->GetVectors( NULL, &right, NULL );
				m_vecAnchor += right * enginerandom->RandomFloat( -100, 100 );
				bHasSetAnchor = true;
			}
		}
	}
	...
	if( !bHasSetAnchor )
	{
		...
		Vector offset = right * enginerandom->RandomFloat( 60, 120 );
		...
		m_vecAnchor.z -= enginerandom->RandomFloat( 80, 200 );
	}
...
//---------------------------------------------------------
//---------------------------------------------------------
void CStriderMinigun::StopShootingForSeconds( IStriderMinigunHost *pHost, CEntity *pTarget, float duration )
{
	if ( IsShooting() )
	{
		SetState( MINIGUN_OFF );
	...
	ClearOnTarget();
	m_nextTwitchTime = gpGlobals->curtime + enginerandom->RandomFloat( 2.0, 4.0 );
...
//---------------------------------------------------------
//---------------------------------------------------------
void CStriderMinigun::SetTarget( IStriderMinigunHost *pHost, CEntity *pTarget, bool bOverrideEnemy )
{
	m_hTarget.Set((pTarget)?pTarget->BaseEntity():NULL);;
	...
	if( pTarget )
	{
		...
		if( m_vecAnchor == vec3_invalid )
		{
			...
			m_vecAnchor += right * enginerandom->RandomFloat( -60.0f, 60.0f );
		}
	}
...
//---------------------------------------------------------
bool CStriderMinigun::CanStartShooting( IStriderMinigunHost *pHost, CEntity *pTargetEnt )
{
	if( !pTargetEnt )
		return false;
		...
		if( !pHost->GetEntity()->HasCondition( COND_SEE_ENEMY )  )
		{
			...
			if( !pStrider->WeaponLOSCondition( pStrider->GetAdjustedOrigin(), pTargetEnt->WorldSpaceCenter(), false ) )
			{
				if( pStrider->IsUsingAggressiveBehavior() && pTargetEnt->IsPlayer() && !pStrider->HasPass() )
				{
					...
					float flTimeSinceLastSeen = gpGlobals->curtime - pStrider->GetEnemies()->LastTimeSeen(pTargetEnt->BaseEntity());
...
void CStriderMinigun::Think( IStriderMinigunHost *pHost, float dt )
{
	...
	if( ShouldFindTarget( pHost ) )
	{
		...
		if( IsShooting() )
		{
			...
			ExtendShooting( STRIDER_SUBSEQUENT_TARGET_DURATION + enginerandom->RandomFloat( 0, 0.5 ) );
		}
	...
	if ( !GetTarget() && m_nextTwitchTime <= gpGlobals->curtime )
	{
		...
		m_nextTwitchTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.3, 2 );
	}

	CEntity *pTargetEnt = m_hTarget.Get();
	...
	else
	{
		...
		{
			StartShooting( pHost, pTargetEnt, pHost->GetMinigunShootDuration() + enginerandom->RandomFloat( 0, pHost->GetMinigunShootVariation() ) );
		}
	}
		...
		if( pTargetEnt && pTargetEnt == pHost->GetEntity()->GetEnemy() )
		{
			...
			if ( GetTarget() )
			{
				...
				if( flFactor <= 0.5 && !m_bWarnedAI )
				{
					...
					g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER | SOUND_CONTEXT_REACT_TO_SOURCE, pTargetEnt->EarPosition() + Vector( 0, 0, 1 ), 120, MAX( 1.0, flRemainingShootTime ), pHost->GetEntity()->BaseEntity() );
				}
			}
		}
		else
...
LINK_ENTITY_TO_CUSTOM_CLASS( sparktrail, info_target, CSparkTrail );
...
void CSparkTrail::Spawn()
{
	...
	m_iHealth = 20 + enginerandom->RandomInt( 0, 5 );
	UTIL_SetOrigin( this, GetOwnerEntity()->EyePosition() );

	Vector vecVelocity;

	vecVelocity.x = enginerandom->RandomFloat( 100, 400 );
	vecVelocity.y = enginerandom->RandomFloat( 100, 400 );
	vecVelocity.z = enginerandom->RandomFloat( 0, 100 );
	...
	UTIL_SetSize( BaseEntity(), Vector( 0, 0, 0 ), Vector( 0, 0, 0 ) );
