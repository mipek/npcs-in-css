...
void CNPC_BaseScanner::Spawn(void)
{
	...
	// Noise modifier
	Vector	bobAmount;
	bobAmount.x = enginerandom->RandomFloat( -2.0f, 2.0f );
	bobAmount.y = enginerandom->RandomFloat( -2.0f, 2.0f );
	bobAmount.z = enginerandom->RandomFloat( 2.0f, 4.0f );
	...
	m_flFlyNoiseBase = enginerandom->RandomFloat( 0, M_PI );
...
{
	// CE TODO: dive bomb seems to tend to crash..
#if defined(MONSTER_CRASHMODE)
	// ----------------------------------------------------
	...
	case TASK_SCANNER_SET_FLY_DIVE:
		{
			...
			else
			{
				...
				m_vecDiveBombDirection = forward + Vector( enginerandom->RandomFloat( -10, 10 ), enginerandom->RandomFloat( -10, 10 ), enginerandom->RandomFloat( -20, -10 ) );
			}
			VectorNormalize( m_vecDiveBombDirection );

			// Calculate a roll force.
			m_flDiveBombRollForce = enginerandom->RandomFloat( 20.0, 420.0 );
			if ( enginerandom->RandomInt( 0, 1 ) )
			{
...
void CNPC_BaseScanner::TakeDamageFromPhyscannon( CPlayer *pPlayer )
{
	CTakeDamageInfo info;
	info.SetDamageType( DMG_GENERIC );
	info.SetInflictor( BaseEntity() );
	info.SetAttacker( pPlayer->BaseEntity() );
...
void CNPC_BaseScanner::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	CPlayer *pPlayer = ToBasePlayer(CEntity::Instance(HasPhysicsAttacker( SCANNER_SMASH_TIME )));
	if( pPlayer )
	{
...
void CNPC_BaseScanner::Gib( void )
{
	...
	// Cover the gib spawn
	ExplosionCreate( WorldSpaceCenter(), GetAbsAngles(), BaseEntity(), 64, 64, false );
...
//			bPunting -
//-----------------------------------------------------------------------------
void CNPC_BaseScanner::OnPhysGunPickup( CBaseEntity *pPhysGunUser, PhysGunPickup_t reason )
{
...
// Input  : *pPhysGunUser -
//-----------------------------------------------------------------------------
void CNPC_BaseScanner::OnPhysGunDrop( CBaseEntity *pPhysGunUser, PhysGunDrop_t Reason )
{
	m_hPhysicsAttacker.Set(pPhysGunUser);
	m_flLastPhysicsInfluenceTime = gpGlobals->curtime;
...
//------------------------------------------------------------------------------
CBaseEntity *CNPC_BaseScanner::HasPhysicsAttacker( float dt )
{
	...
	{
		CPlayer *pPlayer = m_hPhysicsAttacker.Get();
		return pPlayer ? pPlayer->BaseEntity() : NULL;
	}
	return NULL;
}
...
{
	g_SoundController->SoundDestroy( m_pEngineSound );
	m_pEngineSound = NULL;
...
void CNPC_BaseScanner::AttackDivebombCollide(float flInterval)
{
	...
	UTIL_TraceHull( GetAbsOrigin(), checkPos, GetHullMins(), GetHullMaxs(), MASK_SOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	...
	if (tr.m_pEnt)
	{
		...
		if (pHitEntity && pHitEntity->Classify()!=CLASS_SCANNER)
		{
			if ( !pHitEntity->ClassMatches("item_battery") )
			{
				...
				{
					CTakeDamageInfo info( BaseEntity(), BaseEntity(), sk_scanner_dmg_dive.GetFloat(), DMG_CLUB );
	...
	if (tr.fraction != 1.0)
	{
		...
		CCombatCharacter* pBCC = ToBaseCombatCharacter( pHitEntity );
		...
		else
		{
			...
			UTIL_TraceLine( GetAbsOrigin(), checkPos,MASK_SOLID_BRUSHONLY, BaseEntity(), COLLISION_GROUP_NONE, &tr );
			if (tr.fraction != 1.0)
			{
...
void CNPC_BaseScanner::PlayFlySound(void)
{
	...
	if ( m_pEngineSound == NULL )
	{
		...
		g_SoundController->SoundChangeVolume( m_pEngineSound, 1.0f, 2.0f );
	}
	...
	g_SoundController->SoundChangePitch( m_pEngineSound, iPitch, 0.1f );
	g_SoundController->SoundChangeVolume( m_pEngineSound, flVolume, 0.1f );
}
...
void CNPC_BaseScanner::StartSmokeTrail( void )
{
	...
	m_pSmokeTrail = CSmokeTrail::CreateSmokeTrail();
	...
	if ( m_pSmokeTrail )
	{
		...
		m_pSmokeTrail->m_StartColor.ptr->Init( 0.5f, 0.5f, 0.5f );
		m_pSmokeTrail->m_EndColor.ptr->Init( 0, 0, 0 );
...
void CNPC_BaseScanner::MoveExecute_Alive(float flInterval)
{
	...
	float tailPerc = clamp( GetCurrentVelocity().z, -150.0f, 250.0f );
	...
	float speed	= GetCurrentVelocity().Length();
	float flDynamoSpeed = (maxSpeed > 0 ? speed / maxSpeed : 1.0f) * 60.0f;
...
// Input  : flInterval -
//-----------------------------------------------------------------------------
bool CNPC_BaseScanner::OverridePathMove( CEntity *pMoveTarget, float flInterval )
{
	// Save our last patrolling direction
	Vector lastPatrolDir = GetNavigator()->GetCurWaypointPos() - GetAbsOrigin();

	// Continue on our path
	if ( ProgressFlyPath( flInterval, pMoveTarget->BaseEntity(), (MASK_NPCSOLID|CONTENTS_WATER), false, 64 ) == AINPP_COMPLETE )
	{
...
bool CNPC_BaseScanner::OverrideMove( float flInterval )
{
	...
	else
	{
		...
		if ( pMoveTarget )
		{
			...
			if ( ( tr.m_pEnt == pMoveTarget->BaseEntity() ) || ( fTargetDist < 50 ) )
			{
...
Vector CNPC_BaseScanner::IdealGoalForMovement( const Vector &goalPos, const Vector &startPos, float idealRange, float idealHeightDiff )
{
	...
	trace_t	tr;
	UTIL_TraceHull( vIdealPos, vIdealPos - Vector( 0, 0, MinGroundDist() ), GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
{
	// CE_TODO: this is crashing because our vphysics object is invalid -> investigate!
	return;
...
// Purpose:
//-----------------------------------------------------------------------------
Vector CNPC_BaseScanner::VelocityToEvade(CCombatCharacter *pEnemy)
{
	if (pEnemy)
	{
		// -----------------------------------------
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
inline CEntity *CNPC_BaseScanner::EntityToWatch( void )
{
...
void CNPC_BaseScanner::ClampMotorForces( Vector &linear, AngularImpulse &angular )
{
	...
	if ( m_nFlyMode != SCANNER_FLY_DIVE )
	{
		linear.x = clamp( linear.x, -500.0f, 500.0f );
		linear.y = clamp( linear.y, -500.0f, 500.0f );
		linear.z = clamp( linear.z, -500.0f, 500.0f );
	}
