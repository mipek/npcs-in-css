...
class CCombineDropshipContainer : public CE_CPhysicsProp
{
public:
	CE_DECLARE_CLASS( CCombineDropshipContainer, CE_CPhysicsProp );
	DECLARE_DATADESC();
...
{
public:
	CE_DECLARE_CLASS( CNPC_CombineDropship, CBaseHelicopter );
	...
	Vector	GetDropoffFinishPosition( Vector vecOrigin, CAI_NPC *pNPC, Vector vecMins, Vector vecMaxs );
...
private:
	...
	// Misc Vars
	CEFakeHandle<CAnimating>	m_hContainer;
	CFakeHandle	m_hPickupTarget;
	...
	CFakeHandle	m_hLastTroopToLeave;
...
//
//------------------------------------------------------------------------------
LINK_ENTITY_TO_CUSTOM_CLASS( prop_dropship_container, physics_prop, CCombineDropshipContainer )
...
void CCombineDropshipContainer::CreateCorpse()
{
	...
	te->Explosion( filter, 0.0f, &vecAbsPoint, g_sModelIndexFireball,
				   enginerandom->RandomInt( 4, 10 ), enginerandom->RandomInt( 8, 15 ), TE_EXPLFLAG_NOPARTICLES, 100, 0 );

	// Break into chunks
	Vector angVelocity;
	QAngleToAngularImpulse( GetLocalAngularVelocity(), angVelocity );
	PropBreakableCreateAll( GetModelIndex(), VPhysicsGetObject(), GetAbsOrigin(), GetAbsAngles(), GetAbsVelocity(), angVelocity, 1.0, 250, COLLISION_GROUP_NPC, BaseEntity() );

	// Create flaming gibs
	int iChunks = enginerandom->RandomInt( 4, 6 );
...
void CCombineDropshipContainer::ThrowFlamingGib( void )
{
	...
	int nGib = enginerandom->RandomInt( 0, DROPSHIP_CONTAINER_MAX_CHUNKS - 1 );
	pChunk->Spawn( s_pChunkModelName[nGib] );
	pChunk->SetOwnerEntity( BaseEntity() );
	pChunk->m_lifeTime = enginerandom->RandomFloat( 6.0f, 8.0f );
	...
	if ( pPhysicsObject )
	{
		...
		QAngle angles;
		angles.x = enginerandom->RandomFloat( -20, 20 );
		angles.y = enginerandom->RandomFloat( 0, 360 );
		angles.z = 0.0f;
		AngleVectors( angles, &vecVelocity );

		vecVelocity *= enginerandom->RandomFloat( 300, 900 );
	...
	CE_CEntityFlame *pFlame = CE_CEntityFlame::Create( pChunk, false );
	if ( pFlame != NULL )
	{
	...
	if( pSmokeTrail )
	{
		...
		pSmokeTrail->SetParent( pChunk->BaseEntity(), 0 );
...
int CCombineDropshipContainer::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( nPrevHealth != GetHealth() )
	{
		...
		{
			ExplosionCreate( dmgInfo.GetDamagePosition(), vec3_angle, BaseEntity(), 1000, 500,
...
void CCombineDropshipContainer::AddSmokeTrail( const Vector &vecPos )
{
	...
	ExplosionCreate( vecPos, vec3_angle, BaseEntity(), 1000, 500, SF_ENVEXPLOSION_NODAMAGE |
...
void CNPC_CombineDropship::Spawn( void )
{
	...
	switch ( m_iCrateType )
	{
		...
		case CRATE_SOLDIER:
			p_m_hContainer = (CAnimating*)CreateEntityByName( "prop_dropship_container" );
			if ( p_m_hContainer )
			{
				m_hContainer.Set(p_m_hContainer->BaseEntity());
				p_m_hContainer->SetName( STRING(AllocPooledString("dropship_container")) );
				p_m_hContainer->SetAbsOrigin( GetAbsOrigin() );
				p_m_hContainer->SetAbsAngles( GetAbsAngles() );
				p_m_hContainer->SetParent(BaseEntity(), 0);
				p_m_hContainer->SetOwnerEntity(BaseEntity());
				p_m_hContainer->Spawn();

				IPhysicsObject *pPhysicsObject = p_m_hContainer->VPhysicsGetObject();
				if ( pPhysicsObject )
				{
					pPhysicsObject->SetShadow( 1e4, 1e4, false, false );
					pPhysicsObject->UpdateShadow( p_m_hContainer->GetAbsOrigin(), p_m_hContainer->GetAbsAngles(), false, 0 );
				}

				p_m_hContainer->SetMoveType( MOVETYPE_PUSH );
		...
		case CRATE_STRIDER:
			p_m_hContainer = (CAnimating*)CreateEntityByName( "npc_strider" );
			m_hContainer.Set(p_m_hContainer->BaseEntity());
			p_m_hContainer->SetAbsOrigin( GetAbsOrigin() - Vector( 0, 0 , 100 ) );
			p_m_hContainer->SetAbsAngles( GetAbsAngles() );
			p_m_hContainer->SetParent(BaseEntity(), 0);
			p_m_hContainer->SetOwnerEntity(BaseEntity());
			p_m_hContainer->Spawn();
		...
		{
			m_soldiersToDrop = 0;
			p_m_hContainer = (CAnimating*)g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_iszAPCVehicleName );
			if ( !p_m_hContainer )
			{
			...
			p_m_hContainer->Teleport( &apcPosition, &apcAngles, NULL );

			m_iContainerMoveType = p_m_hContainer->GetMoveType();
			...
			p_m_hContainer->SetMoveType( MOVETYPE_PUSH );
			p_m_hContainer->SetGroundEntity( NULL );
			p_m_hContainer->UpdatePhysicsShadowToCurrentPosition(0);
		}
			break;

		case CRATE_JEEP:
			p_m_hContainer = (CAnimating*)CreateEntityByName( "prop_dynamic_override" );
			if ( p_m_hContainer )
			{
				m_hContainer.Set(p_m_hContainer->BaseEntity());
				p_m_hContainer->SetModel( "models/buggy.mdl" );
				p_m_hContainer->SetName( STRING(AllocPooledString("dropship_jeep")) );

				p_m_hContainer->SetAbsOrigin( GetAbsOrigin() );//- Vector( 0, 0 , 25 ) );
				...
				p_m_hContainer->SetParent(BaseEntity(), 0);
				p_m_hContainer->SetOwnerEntity(BaseEntity());
				p_m_hContainer->SetSolid( SOLID_VPHYSICS );
				p_m_hContainer->Spawn();
			}
			break;
	...
	if ( m_hContainer )
	{
		UTIL_SetSize( BaseEntity(), DROPSHIP_BBOX_CRATE_MIN, DROPSHIP_BBOX_CRATE_MAX );
		SetIdealActivity( (Activity)ACT_DROPSHIP_FLY_IDLE_CARGO );
	}
	else
	{
		UTIL_SetSize( BaseEntity(), DROPSHIP_BBOX_MIN, DROPSHIP_BBOX_MAX );
	...
	m_hPickupTarget.Set(NULL);
	m_hLandTarget.Set(NULL);
...
void CNPC_CombineDropship::Precache( void )
{
	...
	{
		case CRATE_SOLDIER:
			g_helpfunc.UTIL_PrecacheOther( "prop_dropship_container" );
			...
			for ( int i = 0; i < DROPSHIP_MAX_SOLDIERS; i++ )
			{
				if ( m_sNPCTemplate[i] != NULL_STRING )
				{
					...
					if ( m_sNPCTemplateData[i] != NULL_STRING )
					{
						...
						g_helpfunc.MapEntity_ParseEntity( pEntity, STRING(m_sNPCTemplateData[i]), NULL );
						if ( pEntity != NULL )
						{
							pEntity->Precache();
...
void CNPC_CombineDropship::Flight( void )
{
	...
	//We assume CRATE_NONE means the dropship just picked up some enginerandom phys object.
	...
	if ( bRunFlight )
	{
		...
		accel.z = clamp( accel.z, 384.0f * 0.2f, 384.0f * 2.0f );
		...
		goalPitch = clamp( goalPitch, -45.0f, 60.0f );
...
void CNPC_CombineDropship::InitializeRotorSound( void )
{
	...
	m_pRotorSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_CombineDropship.RotorLoop" );
	m_pNearRotorSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_CombineDropship.NearRotorLoop" );
	m_pRotorOnGroundSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_CombineDropship.OnGroundRotorLoop" );
	m_pDescendingWarningSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_CombineDropship.DescendingWarningLoop" );
	m_pCannonSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_CombineDropship.FireLoop"  );

	// NOTE: m_pRotorSound is started up by the base class
	if ( m_pCannonSound )
	{
		g_SoundController->Play( m_pCannonSound, 0.0, 100 );
	}

	if ( m_pDescendingWarningSound )
	{
		g_SoundController->Play( m_pDescendingWarningSound, 0.0, 100 );
	}

	if ( m_pRotorOnGroundSound )
	{
		g_SoundController->Play( m_pRotorOnGroundSound, 0.0, 100 );
	}

	if ( m_pNearRotorSound )
	{
		g_SoundController->Play( m_pNearRotorSound, 0.0, 100 );
	}
...
void CNPC_CombineDropship::StopLoopingSounds()
{
	if ( m_pCannonSound )
	{
		g_SoundController->SoundDestroy( m_pCannonSound );
		m_pCannonSound = NULL;
	}

	if ( m_pRotorOnGroundSound )
	{
		g_SoundController->SoundDestroy( m_pRotorOnGroundSound );
		m_pRotorOnGroundSound = NULL;
	}

	if ( m_pDescendingWarningSound )
	{
		g_SoundController->SoundDestroy( m_pDescendingWarningSound );
		m_pDescendingWarningSound = NULL;
	}

	if ( m_pNearRotorSound )
	{
		g_SoundController->SoundDestroy( m_pNearRotorSound );
...
void CNPC_CombineDropship::UpdateRotorWashVolume( CSoundPatch *pRotorSound, float flVolume, float flDeltaTime )
{
	...
	float flVolDelta = flVolume - g_SoundController->SoundGetVolume( pRotorSound );
	if ( flVolDelta )
	{
		...
		g_SoundController->SoundChangeVolume( pRotorSound, flVolume, flRampTime );
	}
}
...
void CNPC_CombineDropship::UpdateRotorSoundPitch( int iPitch )
{
	...
	if ( m_pRotorSound )
	{
		g_SoundController->SoundChangePitch( m_pRotorSound, iPitch + rotorPitch, 0.1 );
	}

	if ( m_pNearRotorSound )
	{
		g_SoundController->SoundChangePitch( m_pNearRotorSound, iPitch + rotorPitch, 0.1 );
	}

	if (m_pRotorOnGroundSound)
	{
		g_SoundController->SoundChangePitch( m_pRotorOnGroundSound, iPitch + rotorPitch, 0.1 );
	}
...
void CNPC_CombineDropship::LandCommon( bool bHover )
{
	...
	{
		CEntity *pTarget = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_iszLandTarget );
		if ( !pTarget )
		{
			Warning("npc_combinedropship %s couldn't find land target named %s\n", GetEntityName(), STRING(m_iszLandTarget) );
			return;
		}

		// Start heading to the point
		m_hLandTarget.Set(pTarget->BaseEntity());
	}
}
...
//	m_hContainer->SetOwnerEntity(NULL);
	...
	{
		CTakeDamageInfo dmgInfo( BaseEntity(), BaseEntity(), vec3_origin, m_hContainer->GetAbsOrigin(), m_hLastTroopToLeave->GetMaxHealth(), DMG_GENERIC );
	...
	else
	{
		...
		m_OnContainerShotDownBeforeDropoff.Set( iTroopsNotUnloaded, BaseEntity(), BaseEntity() );
	}
}
...
void CNPC_CombineDropship::InputPickup( inputdata_t &inputdata )
{
	...
	CEntity *pTarget = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, iszTargetName );
	if ( !pTarget )
	{
		Warning("npc_combinedropship %s couldn't find pickup target named %s\n", GetEntityName(), STRING(iszTargetName) );
		return;
	}

	// Start heading to the point
	m_hPickupTarget.Set(pTarget->BaseEntity());

	m_bHasDroppedOff = false;

	// Disable collisions to my target
	m_hPickupTarget->SetOwnerEntity(BaseEntity());
...
void CNPC_CombineDropship::SetLandingState( LandingState_t landingState )
{
	...
	if ( m_pDescendingWarningSound )
	{
		...
		{
			g_SoundController->SoundChangeVolume( m_pDescendingWarningSound, m_bSuppressSound ? 0.0f : 1.0f, 0.3f );
		}
		else
		{
			g_SoundController->SoundChangeVolume( m_pDescendingWarningSound, 0.0f, 0.0f );
		}
	}
...
#define ALTITUDE_CAP	512.0f
			...
			g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER, vecSpot, 400, 0.1, BaseEntity(), 0 );
			g_helpfunc.CSoundEnt_InsertSound( SOUND_PHYSICS_DANGER, vecSpot, 400, 0.1, BaseEntity(), 1 );
//			NDebugOverlay::Cross3D( vecSpot, -Vector(4,4,4), Vector(4,4,4), 255, 0, 255, false, 10.0f );
			...
			UTIL_TraceHull( vecBottom, pEndPoint, vecBBoxMin, vecBBoxMax, MASK_SOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );

			if ( tr.fraction < 1.0f )
			{
				// Damage anything that's blocking me
				CEntity *cent = CEntity::Instance(tr.m_pEnt);
				if ( cent && cent->m_takedamage != DAMAGE_NO )
				{
					CTakeDamageInfo info( BaseEntity(), BaseEntity(), 20 * dt, DMG_BURN );
					cent->TakeDamage( info );
				}
			}
			...
			else
			{
				...
				if ( flDistance < 50 )
				{
					...
					*/
					{
						// Grab the target
						m_hContainer.Set(m_hPickupTarget->BaseEntity());
						m_hPickupTarget.Set(NULL);
						...
						m_hContainer->SetParent(BaseEntity(), 0);
...
void CNPC_CombineDropship::SpawnTroop( void )
{
	...
	g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER, vecDeployEndPoint, 120, 2.0f, BaseEntity() );

	// Make sure there are no NPCs on the spot
	trace_t tr;
	CTraceFilterOnlyNPCsAndPlayer filter( BaseEntity(), COLLISION_GROUP_NONE );
	UTIL_TraceHull( vecDeployEndPoint, vecDeployEndPoint, vecNPCMins, vecNPCMaxs, MASK_SOLID, &filter, &tr );
	if ( tr.m_pEnt )
	{
	...
	CEntity *pEntity = NULL;
	g_helpfunc.MapEntity_ParseEntity( pEntity, STRING(m_sNPCTemplateData[m_iCurrentTroopExiting]), NULL );
	...
	g_CEventQueue->AddEvent( pBlocker->BaseEntity(), "Kill", 2.5, BaseEntity(), BaseEntity() );
	...
	DispatchSpawn( pNPC->BaseEntity() );
	pNPC->m_NPCState = NPC_STATE_IDLE;
	pNPC->Activate();

	// Spawn a scripted sequence entity to make the NPC run out of the dropship
	CEAI_ScriptedSequence *pSequence = (CEAI_ScriptedSequence*)CreateEntityByName( "scripted_sequence" );
	pSequence->DispatchKeyValue( "m_iszEntity", pNPC->GetEntityName() );
	pSequence->DispatchKeyValue( "m_iszPlay", "Dropship_Deploy" );
	pSequence->DispatchKeyValue( "m_fMoveTo", "4" );	// CINE_MOVETO_TELEPORT
	pSequence->DispatchKeyValue( "OnEndSequence", UTIL_VarArgs("%s,NPCFinishDustoff,%s,0,-1", GetEntityName(), pNPC->GetEntityName()) );
...
// Input  : vecOrigin -
//-----------------------------------------------------------------------------
Vector CNPC_CombineDropship::GetDropoffFinishPosition( Vector vecOrigin, CAI_NPC *pNPC, Vector vecMins, Vector vecMaxs )
{
	...
	trace_t tr;
	UTIL_TraceHull( vecOrigin + Vector(0,0,32), vecOrigin, vecMins, vecMaxs, MASK_SOLID, (pNPC)?pNPC->BaseEntity():NULL, COLLISION_GROUP_NONE, &tr );
	if ( tr.fraction < 1.0 )
	{
...
{
	CEntity *pEnt = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, inputdata.value.StringID(), NULL, inputdata.pActivator, inputdata.pCaller );
	if ( !pEnt )
		return;
	...
	CEntity *pDustoff = NULL;
	if ( m_sDustoffPoints[m_iCurrentTroopExiting-1] != NULL_STRING )
	{
		pDustoff = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_sDustoffPoints[m_iCurrentTroopExiting-1] );
		if ( !pDustoff )
		{
			Warning("npc_combinedropship %s couldn't find dustoff target named %s\n", GetEntityName(), STRING(m_sDustoffPoints[m_iCurrentTroopExiting-1]) );
		}
	}
	...
	if ( !pDustoff )
	{
		...
		g_helpfunc.CSoundEnt_InsertSound( SOUND_MOVE_AWAY | SOUND_CONTEXT_COMBINE_ONLY, pNPC->GetAbsOrigin(), 128, 0.1 );
	}
	else
	{
		// Tell the NPC to move to the dustoff position
		pNPC->SetState( NPC_STATE_ALERT );
		pNPC->ScheduledMoveToGoalEntity( SCHED_DROPSHIP_DUSTOFF, pDustoff->BaseEntity(), ACT_RUN );
...
float CNPC_CombineDropship::GetAltitude( void )
{
	...
	UTIL_TraceEntity( this, vecBottom, vecBottom - Vector(0,0,4096), MASK_SOLID_BRUSHONLY, &tr );
...
{
	NPC_Rollermine_DropFromPoint( GetAbsOrigin(), BaseEntity(), STRING(m_sRollermineTemplateData) );
}
...
void CNPC_CombineDropship::UpdatePickupNavigation( void )
{
	...
	vecPickup.z += (m_hPickupTarget->CollisionProp_Actual()->OBBSize().z * 0.5);
...
bool CNPC_CombineDropship::FireCannonRound( void )
{
	...
	g_pEffects->MuzzleFlash( vecMuzzle, vecAimAngles, enginerandom->RandomFloat( 5.0f, 7.0f ), MUZZLEFLASH_TYPE_GUNSHIP );
	...
	CAI_NPC::FireBullets( 1, vecMuzzle, vecAimDir, VECTOR_CONE_2DEGREES, 8192, ammoType, 1, -1, -1, sk_npc_dmg_dropship.GetInt() );
...
{
	g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER | SOUND_CONTEXT_REACT_TO_SOURCE, tr.endpos, 120, 0.3f, BaseEntity() );
...
void CNPC_CombineDropship::StartCannon( void )
{
	...
	if ( m_pCannonSound )
	{
		g_SoundController->SoundChangeVolume(m_pCannonSound, 1.0, 0.0);
	}
}
...
void CNPC_CombineDropship::StopCannon( void )
{
	...
	if ( m_pCannonSound )
	{
		g_SoundController->SoundChangeVolume(m_pCannonSound, 0.0, 0.1);
	}
}
