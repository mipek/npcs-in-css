...
{
	CE_DECLARE_CLASS( CNPC_CeilingTurret, CNPCBaseInteractive<CE_Cycler_Fix> );
public:
	...
	Vector	GetAttackSpread( CBaseEntity *pWeapon, CBaseEntity *pTarget ) 
	{
		return VECTOR_CONE_5DEGREES * ((CCombatWeapon::GetDefaultProficiencyValues())[ WEAPON_PROFICIENCY_PERFECT ].spreadscale);
	}
...
{ 
	...
	m_pEyeGlow->SetAttachment( BaseEntity(), 2 );
...
int CNPC_CeilingTurret::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	if ( m_iHealth <= 0 )
	{
		...
		ExplosionCreate( GetAbsOrigin(), GetLocalAngles(), BaseEntity(), 100, 100, false );
		SetThink( &CNPC_CeilingTurret::DeathThink );

		StopSound( "NPC_CeilingTurret.Alert" );

		m_OnDamaged->FireOutput( info.GetInflictor(), BaseEntity() );
...
void CNPC_CeilingTurret::Retire( void )
{
	...
	if ( GetActivity() != ACT_CEILING_TURRET_CLOSE )
	{
		...
		if ( UpdateFacing() == false )
		{
			...
			m_OnRetire.FireOutput( (CBaseEntity *)NULL, this );
		}
	}
...
void CNPC_CeilingTurret::Deploy( void )
{
	...
	if ( GetActivity() != ACT_CEILING_TURRET_OPEN )
	{
		...
		m_OnDeploy.FireOutput( (CBaseEntity *)NULL, this );
	}
...
bool CNPC_CeilingTurret::FVisible( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	...
	if ( BaseClass::FVisible_Entity( pEntity, traceMask, &pHitEntity ) )
		return true;

	// If we hit something that's okay to hit anyway, still fire
	if ( pHitEntity )
	{
		CCombatCharacter *pBCC = CEntity::Instance(pHitEntity)->MyCombatCharacterPointer();
...
void CNPC_CeilingTurret::ActiveThink( void )
{
	...
	UpdateEnemyMemory( GetEnemy()->BaseEntity(), vecMidEnemy );

	//Look for our current enemy
	bool bEnemyVisible = FInViewCone_Entity( GetEnemy()->BaseEntity() ) && FVisible( GetEnemy()->BaseEntity() ) && GetEnemy()->IsAlive();	
...
void CNPC_CeilingTurret::Shoot( const Vector &vecSrc, const Vector &vecDirToEnemy )
{
	if ( m_spawnflags & SF_CEILING_TURRET_OUT_OF_AMMO )
	{
		...
		{
			m_flShotTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.5, 1.5 );
		}
		else
		{
