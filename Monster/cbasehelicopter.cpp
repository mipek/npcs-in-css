...
CEntity *CBaseHelicopter::GetTrackPatherTargetEnt()	
{ 
	return GetEnemy(); 
}
...
void CBaseHelicopter::HelicopterThink( void )
{
	...
	if( GetEnemy() != NULL && (!GetEnemy()->IsAlive() || GetEnemy()->GetFlags() & FL_NOTARGET || IRelationType( GetEnemy_CBase() ) == D_NU ) )
	{
		SetEnemy( NULL );
	}
...
void CBaseHelicopter::DrawRotorWash( float flAltitude, const Vector &vecRotorOrigin )
{
	...
	{
		ShakeRopes( GetAbsOrigin(), flAltitude, 128 );
	}
...
void CBaseHelicopter::DoWashPushOnAirboat( CEntity *pAirboat, 
	const Vector &vecWashToAirboat, float flWashAmount )
{
	...
	pAirboat->VPhysicsTakeDamage( CTakeDamageInfo( BaseEntity(), BaseEntity(), vecForce, vecWashOrigin, flWashAmount, DMG_BLAST ) );
}
...
bool CBaseHelicopter::DoWashPush( washentity_t *pWash, const Vector &vecWashOrigin )
{
	...
	flPushTime = clamp( flPushTime, 0.0f, BASECHOPPER_WASH_RAMP_TIME );
	...
	if ( pShooter )
	{
		...
		pEntity = CEntity::Instance(pShooter->DoWashPush( pWash->flWashStartTime, vecForce ));
		if ( !pEntity )
			return true;

		washentity_t Wash;
		Wash.hEntity.Set(pEntity->BaseEntity());
	...
	pEntity->VPhysicsTakeDamage( CTakeDamageInfo( BaseEntity(), BaseEntity(), vecForce, vecWashOrigin, flWashAmount, DMG_BLAST ) );
...
void CBaseHelicopter::DoRotorPhysicsPush( const Vector &vecRotorOrigin, float flAltitude )
{
	...
	CEntity *pLightestEntity = NULL;
	float flLightestMass = 9999;
	while ((pEntity = g_helpfunc.FindEntityInSphere(pEntity, vecPhysicsOrigin, BASECHOPPER_WASH_RADIUS )) != NULL)
	{
		IRotorWashShooter *pShooter = GetRotorWashShooter( pEntity );
		...
		if ( pShooter || pEntity->GetMoveType() == MOVETYPE_VPHYSICS || (pEntity->VPhysicsGetObject() && !pEntity->IsPlayer()) ) 
		{
			...
			{
				if ( m_hEntitiesPushedByWash[i].hEntity == pEntity->BaseEntity() )
				{
			...
			{
				UTIL_TraceLine( vecSpot, vecPhysicsOrigin, MASK_SOLID_BRUSHONLY, pEntity->BaseEntity(), COLLISION_GROUP_NONE, &tr );
				if ( tr.fraction != 1.0f )
					continue;
			}
			...
			washentity_t Wash;
			Wash.hEntity.Set(pLightestEntity->BaseEntity());
	...
	if ( !bWasPushingObjects && m_hEntitiesPushedByWash.Count() )
	{
		if ( m_pRotorBlast )
		{
			g_SoundController->SoundChangeVolume( m_pRotorBlast, 1.0, 1.0 );
		}
	}
	else if ( bWasPushingObjects && m_hEntitiesPushedByWash.Count() == 0 )
	{
		if ( m_pRotorBlast )
		{
			...
			g_SoundController->SoundChangeVolume( m_pRotorBlast, 0, 1.0 );
		}
	}
}
...
void CBaseHelicopter::UpdateEnemy()
{
	...
	if( m_lifeState == LIFE_ALIVE )
	{
		...
		if( HasEnemy() )
		{
			...
			GatherEnemyConditions( pEnemy->BaseEntity() );
			if ( FVisible_Entity( pEnemy->BaseEntity() ) )
			{
...
void CBaseHelicopter::UpdatePlayerDopplerShift( )
{
	...
	else
	{
		...
		// CE_TODO: do it for every player
...
void CBaseHelicopter::UpdateRotorWashVolume()
{
	...
	float flVolDelta = GetRotorVolume()	- g_SoundController->SoundGetVolume( m_pRotorSound );
	if ( flVolDelta )
	{
		...
		g_SoundController->SoundChangeVolume( m_pRotorSound, GetRotorVolume(), flRampTime );
	}
}
...
void CBaseHelicopter::InitializeRotorSound( void )
{
	...
	{
		// Get the rotor sound started up.
		g_SoundController->Play( m_pRotorSound, 0.0, 100 );
	...
	if ( m_pRotorBlast )
	{
		...
		g_SoundController->Play( m_pRotorBlast, 1.0, 100 );
		g_SoundController->SoundChangeVolume(m_pRotorBlast, 0, 0.0);
	}
...
void CBaseHelicopter::UpdateRotorSoundPitch( int iPitch )
{
	if (m_pRotorSound)
	{
		g_SoundController->SoundChangePitch( m_pRotorSound, iPitch, 0.1 );
		UpdateRotorWashVolume();
	}
}
...
// Output  :
//------------------------------------------------------------------------------
void CBaseHelicopter::FlyTouch( CEntity *pOther )
{
...
// Output  :
//------------------------------------------------------------------------------
void CBaseHelicopter::CrashTouch( CEntity *pOther )
{
...
// Output : 
//-----------------------------------------------------------------------------
void CBaseHelicopter::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
...
void CBaseHelicopter::StopLoopingSounds()
{
	...
	g_SoundController->SoundDestroy( m_pRotorBlast );
	m_pRotorSound = NULL;
	m_pRotorBlast = NULL;
...
void CBaseHelicopter::Event_Killed( const CTakeDamageInfo &info )
{
	...
	UTIL_SetSize( BaseEntity(), Vector( -32, -32, -64), Vector( 32, 32, 0) );
...
void CBaseHelicopter::ApplySidewaysDrag( const Vector &vecRight )
{
	...
	vecNewVelocity.x *= 1.0f - fabs( vecRight.x ) * 0.05f;
	vecNewVelocity.y *= 1.0f - fabs( vecRight.y ) * 0.05f;
	vecNewVelocity.z *= 1.0f - fabs( vecRight.z ) * 0.05f;
...
void CBaseHelicopter::GatherEnemyConditions( CBaseEntity *pEnemy )
{
	...
	// If enemy is dead
	// -------------------
	if ( !CEntity::Instance(pEnemy)->IsAlive() )
	{
