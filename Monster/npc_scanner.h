...
{
public:
	CE_DECLARE_CLASS( CNPC_CScanner, CNPC_BaseScanner );
...
public:
	...
	void			SetInspectTargetToEnt(CEntity *pEntity, float fInspectDuration);
	void			SetInspectTargetToPos(const Vector &vInspectPos, float fInspectDuration);
	void			SetInspectTargetToHint(CE_AI_Hint *pHint, float fInspectDuration);
	void			ClearInspectTarget(void);
	bool			HaveInspectTarget(void);
	Vector			InspectTargetPosition(void);
	bool			IsValidInspectTarget(CEntity *pEntity);
	CEntity*	BestInspectTarget(void);
	...
	CE_CSprite*		m_pEyeFlash;

	void			TakePhoto( void );
	void			BlindFlashTarget( CEntity *pTarget );
	...
	CEFakeHandle<CE_CBeam>	m_hSpotlight;
	CEFakeHandle<CE_CSpotlightEnd> m_hSpotlightTarget;
