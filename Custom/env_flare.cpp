...
LINK_ENTITY_TO_CUSTOM_CLASS( env_flare, item_sodacan, CFlare );
...
CFlare *CreateFlare( Vector vOrigin, QAngle Angles, CEntity *pOwner, float flDuration )
{
...
void KillFlare( CEntity *pOwnerEntity, CEntity *pEntity, float flKillTime )
{
...
CFlare::~CFlare()
{
	if(m_pBurnSound)
	{
		g_SoundController->SoundDestroy( m_pBurnSound );
		m_pBurnSound = NULL;
	}
...
void CFlare::Spawn( void )
{
	...
	UTIL_SetSize( BaseEntity(), Vector( -2, -2, -2 ), Vector( 2, 2, 2 ) );
...
void CFlare::StartBurnSound( void )
{
	if ( m_pBurnSound == NULL )
	{
		...
		m_pBurnSound = g_SoundController->SoundCreate( 
...
// Output : CFlare
//-----------------------------------------------------------------------------
CFlare *CFlare::Create( Vector vecOrigin, QAngle vecAngles, CEntity *pOwner, float lifetime )
{
	...
	pFlare->SetOwnerEntity( pOwner->BaseEntity() );
...
void CFlare::FlareThink( void )
{
	...
	if ( m_flTimeBurnOut != -1.0f )
	{
		...
		{
			m_bFading = true;
			g_SoundController->SoundChangePitch( m_pBurnSound, 60, deltaTime );
			g_SoundController->SoundFadeOut( m_pBurnSound, deltaTime );
		}
	...
	{
		//Shoot sparks
		if ( enginerandom->RandomInt( 0, 8 ) == 1 )
		{
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CFlare::FlareBurnTouch( CEntity *pOther )
{
	if ( pOther && pOther->m_takedamage && ( m_flNextDamage < gpGlobals->curtime ) )
	{
		pOther->TakeDamage( CTakeDamageInfo( BaseEntity(), m_pOwner->BaseEntity(), 1, (DMG_BULLET|DMG_BURN) ) );
...
void CFlare::FlareTouch( CEntity *pOther )
{
	...
	if ( pOther && pOther->m_takedamage )
	{
		...
		*/
		...
		pAnim = dynamic_cast<CAnimating*>(pOther);
		if( pAnim )
		{
			pAnim->Ignite( 30.0f );
		}
	...
	else
	{
		...
		const trace_t *tr = CEntity::GetTouchTrace();
...
void CFlare::Start( float lifeTime )
{
	...
	if ( m_pBurnSound != NULL )
	{		
		g_SoundController->Play( m_pBurnSound, 0.0f, 60 );
		g_SoundController->SoundChangeVolume( m_pBurnSound, 0.8f, 2.0f );
		g_SoundController->SoundChangePitch( m_pBurnSound, 100, 2.0f );
	}
