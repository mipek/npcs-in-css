...
#define CANNON_MAX_UP_PITCH			20.0f
#define CANNON_MAX_DOWN_PITCH		20.0f
#define CANNON_MAX_LEFT_YAW			90.0f
#define CANNON_MAX_RIGHT_YAW		90.0f
...
public:
	...
	bool		HandlePassengerExit( CBaseEntity *pPassenger );
...
LINK_ENTITY_TO_CUSTOM_CLASS( prop_vehicle_jeep, prop_vehicle_driveable, CPropJeep );
...
void CPropJeep::PostConstructor()
{
	BaseClass::PostConstructor();

	m_bHasGun = monster_jeep_gun.GetBool();
...
void CPropJeep::CreateServerVehicle( void )
{
	...
	CJeepFourWheelServerVehicle *ptr = new CJeepFourWheelServerVehicle();
	CFourWheelServerVehicle *temp = dynamic_cast<CFourWheelServerVehicle *>(ptr);
	m_pServerVehicle = temp;
	ptr->SetVehicle( BaseEntity() );
}
...
{
	// CE: fix crash when no model is set
	...
	BaseClass::Spawn();
	pServerVehicle = dynamic_cast<CFourWheelServerVehicle*>(GetServerVehicle());
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPropJeep::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
...
float CPropJeep::PassengerDamageModifier( const CTakeDamageInfo &info )
{
	...
	if ( inflictor && FClassnameIs( inflictor, "hunter_flechette" ) )
		return 0.1f;
...
int CPropJeep::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	CEntity *driver = CEntity::Instance(GetDriver());
...
void CPropJeep::AimGunAt( Vector *endPos, float flInterval )
{
	...
	trace_t	tr;
	UTIL_TraceLine( vecMuzzle, vecMuzzle + (vecMuzzleDir * MAX_TRACE_LENGTH), MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
{
	CEntity *pEntity = CEntity::Instance(GetDriver());
...
void CPropJeep::CreateSplash( const Vector &vecPosition )
{
	...
	data.m_flScale = 10.0f + enginerandom->RandomFloat( 0, 2 );
...
void CPropJeep::CreateRipple( const Vector &vecPosition )
{
	...
	// Create the ripple.
	g_helpfunc.DispatchEffect( "waterripple", data );
}
...
void CPropJeep::Think( void )
{
	...
	if ( m_bEngineLocked )
	{
		...
		if ( pPlayer != NULL )
		{
			pPlayer->m_iHideHUD |= HIDEHUD_VEHICLE_CROSSHAIR;
		}
	}
	else if ( m_bHasGun )
	{
	...
	CPlayer *cent_m_hPlayer = (CPlayer *)((CEntity *)m_hPlayer);
	// Aim gun based on the player view direction.
	if ( m_bHasGun == true && cent_m_hPlayer != NULL && !m_bExitAnimOn && !m_bEnterAnimOn )
	{
		Vector vecEyeDir, vecEyePos;
		cent_m_hPlayer->EyePositionAndVectors( &vecEyePos, &vecEyeDir, NULL, NULL );
		...
		trace_t	trace;
		UTIL_TraceLine( vecEyePos, vecEndPos, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &trace );
	...
	if ( IsSequenceFinished() && (m_bExitAnimOn || m_bEnterAnimOn) )
	{
		if ( m_bEnterAnimOn )
		{
			...
			//{
			//	g_CEventQueue->AddEvent( BaseEntity(), "ShowHudHint", 1.5f, BaseEntity(), BaseEntity() );
			//}
		}
...
void CPropJeep::DrawBeam( const Vector &startPos, const Vector &endPos, float width )
{
	...
	CE_CBeam *pBeam = CE_CBeam::BeamCreate( GAUSS_BEAM_SPRITE, 0.5 );
	...
	pBeam->SetColor( 255, 185+enginerandom->RandomInt( -16, 16 ), 40 );
	pBeam->RelinkBeam();
	pBeam->LiveForTime( 0.1f );

	//Draw electric bolts along shaft
	pBeam = CE_CBeam::BeamCreate( GAUSS_BEAM_SPRITE, 3.0f );

	pBeam->SetStartPos( startPos );
	pBeam->PointEntInit( endPos, this );
	pBeam->SetEndAttachment( LookupAttachment("Muzzle") );

	pBeam->SetBrightness( enginerandom->RandomInt( 64, 255 ) );
	pBeam->SetColor( 255, 255, 150+enginerandom->RandomInt( 0, 64 ) );
	pBeam->RelinkBeam();
	pBeam->LiveForTime( 0.1f );
	pBeam->SetNoise( 1.6f );
...
void CPropJeep::FireCannon( void )
{
	...
	CPlayer *pPlayer = (CPlayer *)((CEntity *)m_hPlayer);
	info.m_nFlags = FIRE_BULLETS_ALLOW_WATER_SURFACE_IMPACTS;
	info.m_pAttacker = (pPlayer)?pPlayer->BaseEntity():NULL;

	FireBullets( info );

	// CE: custom laser effect cuz we don't have hl2 tracers
	/*trace_t tr;
	UTIL_TraceLine ( m_vecGunOrigin, m_vecGunOrigin + aimDir * (10*10), MASK_SOLID, BaseEntity(), COLLISION_GROUP_PROJECTILE, &tr);
	CBroadcastRecipientFilter filter;
	te->BeamPoints(filter, 0, &m_vecGunOrigin, &tr.endpos, m_nBeamLaser,
				   0, 0, 0, 0.15f, 2.0f, 1.0f, 0, 12.0f,
				   255, 255, 150+enginerandom->RandomInt( 0, 64 ), 222, 10);*/
...
void CPropJeep::FireChargedCannon( void )
{
	...
	trace_t	tr;
	UTIL_TraceLine( m_vecGunOrigin, endPos, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	...
	{
		CTakeDamageInfo dmgInfo( BaseEntity(), GetDriver(), flDamage, DMG_SHOCK );
	...
	pPlayer = (CPlayer *)((CEntity *)m_hPlayer);
	// Register a muzzleflash for the AI
	if ( pPlayer )
	{
	...
	if ( penetrated == true )
	{
		RadiusDamage( CTakeDamageInfo( BaseEntity(), BaseEntity(), flDamage, DMG_SHOCK ), tr.endpos, 200.0f, CLASS_NONE, NULL );
	}
}
...
void CPropJeep::ChargeCannon( void )
{
	...
	if ( m_bCannonCharging == false )
	{
		...
		m_sndCannonCharge = g_SoundController->SoundCreate( filter, entindex(), CHAN_STATIC, "Jeep.GaussCharge", ATTN_NORM );
		...
		if ( m_sndCannonCharge != NULL )
		{
			...
			g_SoundController->SoundChangePitch( m_sndCannonCharge, 250, 3.0f );
		}
...
void CPropJeep::StopChargeSound( void )
{
	if ( m_sndCannonCharge != NULL )
	{
		g_SoundController->SoundFadeOut( m_sndCannonCharge, 0.1f );
	}

	CPlayer *pPlayer = (CPlayer *)((CEntity *)m_hPlayer);
	if( pPlayer )
	{
		pPlayer->RumbleEffect( RUMBLE_FLAT_LEFT, 0, RUMBLE_FLAG_STOP );
	}
}
...
void CPropJeep::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	UTIL_TraceLine( vecStart, vecStart + vecForward * 1024, MASK_SOLID | CONTENTS_DEBRIS | CONTENTS_HITBOX, pPlayer->BaseEntity(), COLLISION_GROUP_NONE, &tr );

	if ( tr.m_pEnt == BaseEntity() && tr.hitgroup == JEEP_AMMOCRATE_HITGROUP )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPropJeep::SetupMove( CBaseEntity *player, CUserCmd *ucmd, IMoveHelper *pHelper, CMoveData *move )
{
	...
	if ( m_flOverturnedTime > OVERTURNED_EXIT_WAITTIME )
	{
		if ( (ucmd->buttons & (IN_FORWARD|IN_BACK|IN_MOVELEFT|IN_MOVERIGHT|IN_SPEED|IN_JUMP|IN_ATTACK|IN_ATTACK2) ) && !m_bExitAnimOn )
		{
			...
			if(cent_m_hPlayer != NULL)
			{
				if ( !GetServerVehicle()->HandlePassengerExit(cent_m_hPlayer->BaseEntity()))
				{
					cent_m_hPlayer->PlayUseDenySound();
				}
			}
			return;
		}
...
//			*pMoveData - 
//-----------------------------------------------------------------------------
void CPropJeep::ProcessMovement( CBaseEntity *pPlayer, CMoveData *pMoveData )
{
...
void CPropJeep::CreateDangerSounds( void )
{
	...
	if ( fabs(speed) > 120 )
	{
		...
		g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER | SOUND_CONTEXT_PLAYER_VEHICLE, vecSpot, (int)radius, soundDuration, BaseEntity(), 0 );
		g_helpfunc.CSoundEnt_InsertSound( SOUND_PHYSICS_DANGER | SOUND_CONTEXT_PLAYER_VEHICLE, vecSpot, (int)radius, soundDuration, BaseEntity(), 1 );
	...
	g_helpfunc.CSoundEnt_InsertSound( SOUND_PLAYER | SOUND_CONTEXT_PLAYER_VEHICLE, GetAbsOrigin(), 800, soundDuration, BaseEntity(), 0 );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPropJeep::EnterVehicle( CBaseEntity *pPassenger )
{
	...
	//SetPointViewControl(pPlayer);
	//CreateDriverRagdoll(pPlayer);
...
void CPropJeep::ExitVehicle( int nRole )
{
	...
	if ( GlobalEntity_GetState( "no_seagulls_on_jeep" ) == GLOBAL_OFF )
	{
		...
		CE_AI_Hint *pHint = CAI_HintManager::FindHint( GetAbsOrigin(), hintCriteria );
		if ( pHint )
		{
...
void CPropJeep::JeepSeagullThink( void )
{
	...
	CEntity *cent = m_hSeagull;
	// Do we already have a seagull?
	if ( cent )
	{
		CNPC_Seagull *pSeagull = dynamic_cast<CNPC_Seagull *>(cent);
	...
	CEntity *cent_m_hLastPlayerInVehicle = m_hLastPlayerInVehicle;
	// Is the player visible?
	if ( FVisible_Entity( (cent_m_hLastPlayerInVehicle)?cent_m_hLastPlayerInVehicle->BaseEntity():NULL, MASK_SOLID_BRUSHONLY, &pBlocker ) )
	{
		m_flLastSawPlayerAt = gpGlobals->curtime;
		SetContextThink( &CPropJeep::JeepSeagullThink, gpGlobals->curtime + JEEP_SEAGULL_THINK_INTERVAL, g_pJeepThinkContext );
		return;
...
void CPropJeep::SpawnPerchedSeagull( void )
{
	...
	CNPC_Seagull *pSeagull = (CNPC_Seagull*)CEntity::Create("npc_seagull", vecOrigin, vecAngles, NULL );

	if ( !pSeagull )
		return;

	pSeagull->AddSpawnFlags( SF_NPC_FADE_CORPSE );
	pSeagull->SetGroundEntity( BaseEntity() );
	pSeagull->AddFlag( FL_ONGROUND );
	pSeagull->SetOwnerEntity( BaseEntity() );
	pSeagull->SetMoveType( MOVETYPE_FLY );
	pSeagull->m_bOnJeep = true;

	m_hSeagull.Set(pSeagull->BaseEntity());
...
void CPropJeep::AddSeagullPoop( const Vector &vecOrigin )
{
	...
	for ( int i = 0; i < iDecals; i++ )
	{
		...
		trace_t tr;
		UTIL_TraceLine( vecPoop, vecPoop - Vector(0,0,512), MASK_SHOT, (m_hSeagull)?m_hSeagull->BaseEntity():NULL, COLLISION_GROUP_NONE, &tr );
...
bool CJeepFourWheelServerVehicle::IsPassengerVisible( int nRole )
{
	return BaseClass::IsPassengerVisible(nRole);
}

bool CJeepFourWheelServerVehicle::HandlePassengerExit( CBaseEntity *pPassenger )
{
	bool ret = BaseClass::HandlePassengerExit(pPassenger);
	if(ret)
	{
		CPlayer *pPlayer = ToBasePlayer( CEntity::Instance(pPassenger) );
		if(pPlayer)
		{
			GetFourWheelVehicle()->RemovePointViewControl(pPlayer);
		}
	}
	return ret;
}

void CJeepFourWheelServerVehicle::HandleEntryExitFinish( bool bExitAnimOn, bool bResetAnim )
{
	if(!bExitAnimOn && bResetAnim)
	{
		BaseClass::HandleEntryExitFinish(bExitAnimOn, bResetAnim);
		// CE_TODO ??
		/*CPlayer *pPlayer = ToBasePlayer( CEntity::Instance(GetDriver()) );
...
int CJeepFourWheelServerVehicle::GetExitAnimToUse( Vector &vecEyeExitEndpoint, bool &bAllPointsBlocked )
{
	...
	CAnimating *pAnimating = dynamic_cast<CAnimating *>(m_pVehicle);
