...
class CPropThumper : public CSoda_Fix
{
public:
	CE_DECLARE_CLASS( CPropThumper, CSoda_Fix );
	DECLARE_DATADESC();
...
LINK_ENTITY_TO_CUSTOM_CLASS( prop_thumper, item_sodacan, CPropThumper );
...
void CPropThumper::Spawn( void )
{
	...
	if ( pRepellant )
	{
		...
		m_hRepellantEnt.Set(pRepellant->BaseEntity());
	}
...
void CPropThumper::InitMotorSound( void )
{
	...
	m_sndMotor = g_SoundController->SoundCreate( filter, entindex(), CHAN_STATIC, "coast.thumper_ambient" , ATTN_NORM );

	if ( m_sndMotor ) {
		g_SoundController->Play( m_sndMotor, 1.0f, 100 );
	}
}
...
void CPropThumper::HandleState( void )
{
	...
	if(m_sndMotor)
	{
		g_SoundController->Play( m_sndMotor, 1.0f, m_flPlaybackRate * 100 );
	}
}

void CPropThumper::MyThink( void )
{
	StudioFrameAdvance();
...
void CPropThumper::Thump ( void )
{
	if ( m_iHammerAttachment != -1 )
	{
		...
		g_helpfunc.DispatchEffect( "ThumperDust", data );
	...
	g_helpfunc.CSoundEnt_InsertSound ( SOUND_THUMPER, GetAbsOrigin(), (int)(THUMPER_RADIUS * m_flPlaybackRate), THUMPER_SOUND_DURATION, BaseEntity() );
...
void CPropThumper::InputDisable( inputdata_t &inputdata )
{
	...
	if ( m_hRepellantEnt )
	{
		...
		m_hRepellantEnt->AcceptInput( "Disable", BaseEntity(), BaseEntity(), emptyVariant, 0 );
	}
}
...
void CPropThumper::InputEnable( inputdata_t &inputdata )
{
	...
	{
		variant_t emptyVariant;
		m_hRepellantEnt->AcceptInput( "Enable", BaseEntity(), BaseEntity(), emptyVariant, 0 );
	}
}