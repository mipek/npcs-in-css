...
void CBounceBomb::SetMineState( int iState )
{
	...
	switch( iState )
	{
		case MINE_STATE_DORMANT:
		{
			if(m_pWarnSound)
			{
				g_SoundController->SoundChangeVolume( m_pWarnSound, 0.0, 0.1 );
			}
		...
		case MINE_STATE_CAPTIVE:
		{
			if(m_pWarnSound)
			{
				g_SoundController->SoundChangeVolume( m_pWarnSound, 0.0, 0.2 );
			}
			// Unhook
		...
		case MINE_STATE_TRIGGERED:
		{
			...
			// Scare NPC's
			g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER, GetAbsOrigin(), 300, 1.0f, BaseEntity() );
			...
			vecNudge.x = enginerandom->RandomFloat( -1, 1 );
			vecNudge.y = enginerandom->RandomFloat( -1, 1 );
			vecNudge.z = 1.5;
			vecNudge *= 350;
			...
			float x, y;
			x = 10 + enginerandom->RandomFloat( 0, 20 );
			y = 10 + enginerandom->RandomFloat( 0, 20 );
...
bool CBounceBomb::IsValidLocation()
{
	...
	else
	{
		...
		CEntity *pEntity = CEntity::Instance(servertools->FirstEntity());
		...
		while( pEntity )
		{
			...
			pEntity = CEntity::Instance(servertools->NextEntity(pEntity->BaseEntity()));
		}
	}
...
void CBounceBomb::BounceThink()
{
	...
	if ( pPhysicsObject != NULL )
	{
		...
		trace_t tr;
		UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() + Vector( 0, 0, MINE_MAX_JUMP_HEIGHT ), MASK_SHOT, BaseEntity(), COLLISION_GROUP_INTERACTIVE, &tr );
		...
		pPhysicsObject->ApplyTorqueCenter( AngularImpulse( enginerandom->RandomFloat( 5, 25 ), enginerandom->RandomFloat( 5, 25 ), 0 ) );
...
void CBounceBomb::CavernBounceThink()
{
	...
	if ( pPhysicsObject != NULL )
	{
		...
		pPhysicsObject->ApplyTorqueCenter( AngularImpulse( enginerandom->RandomFloat( 15, 40 ), enginerandom->RandomFloat( 15, 40 ), enginerandom->RandomFloat( 30, 60 ) ) );
...
void CBounceBomb::SettleThink()
{
	...
	if( !m_bDisarmed )
	{
		if( VPhysicsGetObject()->IsAsleep() && !(VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD) )
		{
			...
			trace_t tr;
			UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() - Vector( 0, 0, 1024 ), MASK_SHOT|CONTENTS_GRATE, BaseEntity(), COLLISION_GROUP_NONE, &tr );

			bool bHop = false;
			CEntity *cent = CEntity::Instance(tr.m_pEnt);
			if( cent )
			{
				IPhysicsObject *pPhysics = cent->VPhysicsGetObject();
				...
				else if( cent->m_takedamage != DAMAGE_NO )
				{
				...
				{
					Vector vecForce;
					vecForce.x = enginerandom->RandomFloat( -1000, 1000 );
					vecForce.y = enginerandom->RandomFloat( -1000, 1000 );
					vecForce.z = 2500;
...
void CBounceBomb::UpdateLight( bool bTurnOn, unsigned int r, unsigned int g, unsigned int b, unsigned int a )
{
	if( bTurnOn )
	{
		...
		if( !m_hSprite.Get() )
		{
			...
			// Light isn't on.
			CE_CSprite *pSprite = CE_CSprite::SpriteCreate( "sprites/glow01.vmt", GetAbsOrigin() + up * 10.0f, false );

			if( pSprite )
			{
				m_hSprite.Set(pSprite->BaseEntity());
				pSprite->SetParent( BaseEntity() );
		...
		{
			// Update color
			m_hSprite->SetTransparency( kRenderTransAdd, r, g, b, a, kRenderFxNone );
		}
	}
...
void CBounceBomb::Wake( bool bAwake )
{
	...
	if( !m_pWarnSound )
	{
		m_pWarnSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_CombineMine.ActiveLoop" );
	...
	if( bAwake )
	{
		...
		if( m_bFoeNearest )
		{
			...
			if(m_pWarnSound)
			{
				g_SoundController->SoundChangeVolume( m_pWarnSound, 1.0, 0.1 );
			}
		}
...
float CBounceBomb::FindNearestNPC()
{
	...
	for ( int i = 0; i < nAIs; i++ )
	{
		...
		if( pNPC->IsAlive() )
		{
			...
			if( flDist < flNearest )
			{
				...
				if( FVisible_Entity( pNPC->BaseEntity(), MASK_SOLID_BRUSHONLY ) )
				{
					flNearest = flDist;
					SetNearestNPC( pNPC );
				}
	...
	if( pPlayer && !(pPlayer->GetFlags() & FL_NOTARGET) )
	{
		...
		if( flDist < flNearest && FVisible_Entity( pPlayer->BaseEntity(), MASK_SOLID_BRUSHONLY ) )
		{
			flNearest = flDist;
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CBounceBomb::IsFriend( CEntity *pEntity )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CBounceBomb::ExplodeTouch( CEntity *pOther )
{
...
void CBounceBomb::ExplodeThink()
{
	...
	if (m_iModification == MINE_MODIFICATION_CAVERN)
	{
		ExplosionCreate( GetAbsOrigin(), GetAbsAngles(), (pThrower) ? pThrower : BaseEntity(), BOUNCEBOMB_EXPLODE_DAMAGE, BOUNCEBOMB_EXPLODE_RADIUS, true,
...
//---------------------------------------------------------
//---------------------------------------------------------
void CBounceBomb::OnPhysGunDrop( CBaseEntity *pPhysGunUser, PhysGunDrop_t Reason )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
CBaseEntity *CBounceBomb::HasPhysicsAttacker( float dt )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CBounceBomb::OnPhysGunPickup( CBaseEntity *pPhysGunUser, PhysGunPickup_t reason )
{
	m_hPhysicsAttacker.Set(pPhysGunUser);
	m_flLastPhysicsInfluenceTime = gpGlobals->curtime;
	...
	if( reason != PUNTED_BY_CANNON )
	{
		if( m_iMineState == MINE_STATE_ARMED )
		{
			...
			// ear level.
			g_helpfunc.CSoundEnt_InsertSound( SOUND_MOVE_AWAY, GetAbsOrigin() + Vector( 0, 0, 60), 32, 0.2f );
			return;
		}
		else
		{
...
LINK_ENTITY_TO_CUSTOM_CLASS( bounce_bomb, item_sodacan, CBounceBomb );
LINK_ENTITY_TO_CUSTOM_CLASS( combine_bouncemine, item_sodacan, CBounceBomb );
LINK_ENTITY_TO_CUSTOM_CLASS( combine_mine, item_sodacan, CBounceBomb );
