...
CPropCombineBall *CreateCombineBall( const Vector &origin, const Vector &velocity, float radius, float mass, float lifetime, CEntity *pOwner )
{
	...
	pBall->SetOwnerEntity( pOwner->BaseEntity() );
...
//-----------------------------------------------------------------------------
CBaseEntity *CPropCombineBall::HasPhysicsAttacker( float dt )
{
	// Must have an owner
	...
	return GetOwnerEntity()->BaseEntity();
}
...
//-----------------------------------------------------------------------------
LINK_ENTITY_TO_CUSTOM_CLASS( prop_combine_ball, item_sodacan, CPropCombineBall );
...
void CPropCombineBall::Spawn( void )
{
	...
	{
		m_pGlowTrail->FollowEntity( this->BaseEntity() );
...
void CPropCombineBall::StartAnimating( void )
{
	...
	SetContextThink( &CPropCombineBall::AnimThink, gpGlobals->curtime + enginerandom->RandomFloat( 0.0f, 0.1f), s_pAnimThinkContext );
	...
	g_helpfunc.ResetClientsideFrame(BaseEntity());
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CPropCombineBall::ShouldHitPlayer() 
{ 
	if ( GetOwnerEntity() ) 
	{
		CAI_NPC *pNPC = GetOwnerEntity()->MyNPCPointer();
...
void CPropCombineBall::InputKill( inputdata_t &inputdata )
{
	...
	if ( pOwner )
	{
		pOwner->DeathNotice( this->BaseEntity() );
		SetOwnerEntity( NULL );
	}
...
void CPropCombineBall::OnPhysGunPickup( CPlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	if ( reason != PUNTED_BY_CANNON )
	{
		...
		m_pHoldingSound = g_SoundController->SoundCreate( filter, entindex(), ep );
		g_SoundController->Play( m_pHoldingSound, 1.0f, 100 ); 
...
void CPropCombineBall::OnPhysGunDrop( CPlayer *pPhysGunUser, PhysGunDrop_t Reason )
{
	...
	if ( Reason != LAUNCHED_BY_CANNON )
	{
		...
		shotAng[PITCH] += enginerandom->RandomInt( -55, 55 );
		shotAng[YAW] += enginerandom->RandomInt( -55, 55 );
...
void CPropCombineBall::StopLoopingSounds()
{
	if ( m_pHoldingSound )
	{
		g_SoundController->Shutdown( m_pHoldingSound );
		g_SoundController->SoundDestroy( m_pHoldingSound );
...
void CPropCombineBall::DissolveRampSoundThink( )
{
	...
	if ( m_pHoldingSound )
	{
		g_SoundController->SoundChangePitch( m_pHoldingSound, 150, dt );
	}
...
{
	return PROP_COMBINE_BALL_HOLD_DISSOLVE_TIME;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CPropCombineBall::DissolveEntity( CEntity *pEntity )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPropCombineBall::OnHitEntity( CEntity *pHitEntity, float flSpeed, int index, gamevcollisionevent_t *pEvent )
{
	...
	CCombatCharacter *pBCC = pHitEntity->MyCombatCharacterPointer();

	if ( pBCC )
	{
		bShouldHit = pBCC->IRelationType( GetOwnerEntity()->BaseEntity() ) != D_LI;
	}
	...
	if ( !bIsDissolving && bShouldHit == true )
	{
		if ( pHitEntity->PassesDamageFilter( info ) )
		{
			...
			{
				if ( (m_nState == STATE_THROWN) && pHitEntity->IsNPC())
				{
					EmitSound( "NPC_CombineBall.KillImpact" );
				}
...
void CPropCombineBall::DoImpactEffect( const Vector &preVelocity, int index, gamevcollisionevent_t *pEvent )
{
	...
	if ( tr.fraction < 1.0f )
	{
		...
		g_helpfunc.DispatchEffect( "cball_bounce", data );
	}
...
//-----------------------------------------------------------------------------
bool CPropCombineBall::IsAttractiveTarget( CEntity *pEntity )
{
	if ( !pEntity->IsAlive() )
		return false;
	...
	else
	{
		...
		if( pEntity->GetTeam() == GetOwnerEntity()->GetTeam() )
			return false;

		// We must be able to hit them
		trace_t	tr;
		UTIL_TraceLine( WorldSpaceCenter(), pEntity->BodyTarget( WorldSpaceCenter() ), MASK_SOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );

		if ( tr.fraction < 1.0f && tr.m_pEnt != pEntity->BaseEntity() )
			return false;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CPropCombineBall::IsHittableEntity( CEntity *pHitEntity )
{
...
void CPropCombineBall::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	if ( IsInField() )
	{
		...
		//g_EventQueue.AddEvent( BaseEntity(), "FadeAndRespawn", 0.01, NULL, NULL );
		return;
	}
...
//
//-----------------------------------------------------------------------------
LINK_ENTITY_TO_CUSTOM_CLASS( func_combine_ball_spawner, info_target, CFuncCombineBallSpawner );
...
void CFuncCombineBallSpawner::SpawnBall()
{
	...
	float flSpeed = enginerandom->RandomFloat( m_flMinSpeed, m_flMaxSpeed );
...
//-----------------------------------------------------------------------------
void CFuncCombineBallSpawner::BallGrabbed( CEntity *pCombineBall )
{
...
// Fire ball grabbed output
//-----------------------------------------------------------------------------
void CFuncCombineBallSpawner::GrabBallTouch( CEntity *pOther )
{
...
{
	return enginerandom->RandomFloat( m_flMinSpeed, m_flMaxSpeed );
}
